...
function writeSession(filename: string, header: { id: string; createdAt: number }, calls: StreamChunk[][]): string {
  let seq = 0
  const events: SessionEvent[] = calls.map((chunks, step) => attemptEvent(SessionSeq(seq++), 1, step + 1, chunks))
...
describe('parseSessionLog', () => {
  ...
  it('ignores blank lines', () => {
    ...
    const ev = attemptEvent(SessionSeq(0), 1, 1, TEXT_CHUNKS)
    expect(parseSessionLog(`${header}\n\n${JSON.stringify(ev)}\n\n`)).toEqual([ev])
  })

  it('decodes a released packed chunk row and folds it into an embedded stream', () => {
    const header = JSON.stringify({ type: 'session', version: 0, id: 's1', createdAt: 0 })
    const row = JSON.stringify({
      type: 'text-chunks', seq0: 0, time0: 0,
...
describe('loadReplayScript', () => {
  it('derives from the session JSONL when no override is present', () => {
    writeFileSync(file, sessionJsonl([attemptEvent(SessionSeq(0), 1, 1, TEXT_CHUNKS)]), 'utf8')
  ...
  it('patches form: swaps the named call index and keeps derived siblings', () => {
    ...
    writeFileSync(file, sessionJsonl([
      attemptEvent(SessionSeq(seq++), 1, 1, TEXT_CHUNKS),
      attemptEvent(SessionSeq(seq++), 1, 2, callB),
    ]), 'utf8')
    const overrideFile = join(dir, 'replay.override.json')
...
describe('installLlmReplay (through the real LlmRuntime)', () => {
  function writeLog(...calls: StreamChunk[][]): void {
    ...
    calls.forEach((chunks, step) => {
      events.push(attemptEvent(SessionSeq(seq++), 1, step + 1, chunks))
    })
...
describe('parseSessionHeader', () => {
  ...
  it('reads a non-zero v0 seedLength as the migrated inherited event count', () => {
    const events = [
      attemptEvent(SessionSeq(0), 1, 1, TEXT_CHUNKS),
      attemptEvent(SessionSeq(1), 2, 1, TEXT_CHUNKS),
    ]
    ...
    expect(parseSessionHeader(header + events.map(e => JSON.stringify(e)).join('\n') + '\n'))
      .toEqual({ id: 'child', createdAt: 7, inheritedEventCount: 2 })
  })

  it('rejects a header without its id', () => {
    expect(() => parseSessionHeader('{"type":"session","version":0}\n'))
      .toThrow(/header id must be a non-empty string/)
  })
...
describe('loadSessionScripts', () => {
  ...
  it('derives a FORK child script from its OWN events only (skips the seeded parent prefix)', () => {
    ...
    const parentChunks: StreamChunk[] = [{ type: 'text-delta', index: 0, text: 'PARENT-RESPONSE' }, { type: 'finish', reason: { kind: 'stop' } }]
    ...
    // child's own turn. seedLength = 1 marks where the inherited prefix ends.
    const childEvents: SessionEvent[] = [
      attemptEvent(SessionSeq(0), 1, 1, parentChunks),
      attemptEvent(SessionSeq(1), 2, 1, childChunks),
    ]
    const childPath = join(dir, 'session.1.jsonl')
    writeFileSync(childPath, sessionJsonl(childEvents, { id: 'child', createdAt: 200, seedLength: 1 }), 'utf8')
...
describe('apply (the plugin entry)', () => {
  ...
  it('loads child fixtures from config.childFiles (per-session routing)', async () => {
    ...
    writeFileSync(file, sessionJsonl([attemptEvent(SessionSeq(0), 1, 1, TEXT_CHUNKS)], { id: 'p', createdAt: 1 }), 'utf8')
    const childFile = join(dir, 'session.1.jsonl')
    writeFileSync(childFile, sessionJsonl([attemptEvent(SessionSeq(0), 1, 1, childSecond)], { id: 'c', createdAt: 2 }), 'utf8')
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
  ...
  it('falls back to $DSH_SNAPSHOT_CHILD_FILES (path-delimited) when config omits childFiles', async () => {
    ...
    writeFileSync(file, sessionJsonl([attemptEvent(SessionSeq(0), 1, 1, TEXT_CHUNKS)], { id: 'p', createdAt: 1 }), 'utf8')
    const childFile = join(dir, 'session.1.jsonl')
    writeFileSync(childFile, sessionJsonl([attemptEvent(SessionSeq(0), 1, 1, childChunks)], { id: 'c', createdAt: 2 }), 'utf8')
    process.env.DSH_SNAPSHOT_FILE = file
