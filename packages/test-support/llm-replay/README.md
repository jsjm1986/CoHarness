...
`parseSessionLog` also expands JSONL range-encoded `sourceEventSeqs` values, so physical persistence rows and projected fixtures expose the same logical event metadata to replay consumers.
