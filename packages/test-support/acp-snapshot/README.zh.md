...
- **规范化器**：将已捕获内容转换为稳定文本或可移植 fixture 的纯函数：`normalizeStdout`（JSON-RPC id → 首次出现序列；平台相关的 ACP `usage_update.used` 计数 → `{{usedTokens}}`；UUID 以及生成 cwd 的每种原生／JavaScript 文件系统写法 → token，按最长优先；根据 cwd 的分隔符选择规范 `/` 或宿主原生形式；同时作为 stdout 纯度检查）、`normalizeSessionLog`（时间归零、规范化存储范围元数据并保留可用的 `seq`、使用同一 cwd 路径策略，并归零 compact assistant-attempt 时钟而保留回放载荷）、`normalizeSessionSnapshot`（擦除 request header、移除持久化 envelope、展开 source-event 范围，并把相邻打包 run 重新合并为单一规范 fixture 布局）、`tokenizeSessionFixtureCwd`（生成的 workspace 及其文件系统别名，包括已进行 token 化的 macOS `/private` 别名 → 单一规范 `{{cwd}}`；手工编写的临时路径保持不变）、`scrubSystemPrompts`（request header 和持久 非空 `system/message` 的提示词文本 → `{{system}}`，保留空的持久内容）、`scrubToolSchemas`（schema bulk → `{{tools}}`）、`scrubRequestHeaders`（每个 pin 之外的所有 header bulk → `{{system}}`/`{{tools}}`/`{{messagePrefix}}`，保留结构；见[header 固定 Agent Note](../../../.agents/notes/archived/testing/2026-07-06-pin-request-header-content-in-one-scenario.md)）和 `stabilizeFixtureMessageIds`（针对任意录制器已准备写入 fixture 的父级/子级日志，通过结构化方式仅改写 surface 和持久 inbox 中完整消息的 ID 字段，将已提交 UUID 带入未变化且双向唯一匹配的消息）。
...
签入仓库的会话 fixture 使用规范打包行；[临时仓库迁移器](../../../scripts/migrate-packed-session-fixtures.ts)（`pnpm run migrate:packed-session-fixtures`）会改写较旧的 fixture 布局，由其[移除提案](../../../.agents/notes/proposed/process/2026-07-26-remove-packed-session-fixture-migrator.zh.md)负责删除该迁移器。
...
自身作用域组合出不同请求的 child 会话按 fixture 索引单独声明：`pinsChildToolSchemas` 把该 child 的工具序列移入 `tool-schemas.<n>.expected.json`，`pinsChildSystemPrompts` 把其提示词移入 `system-prompt.<n>.expected.md`。两者都指名自己描述的 `session.<n>.jsonl` fixture，其余请求 header 字段仍归类别 pin 所有，并要求 sidecar 恰好在声明时存在。child 提示词 sidecar 还必须与其类别 pin 不同，因此冗余副本会直接失败，而不会悄悄漂移。作用域组合出自有请求的可继续 child——委派范围声明，或与父级不同的工具行——是两者的随附用例；自 `send_message` 取代 child 作用域的 `report` 工具后，工具序列与父级一致的 child 应删除其 `pinsChildToolSchemas` 条目，而不是钉住一份冗余副本。
...
使用 `dsh-acp-snapshot` 作为免密钥 ACP 快照层的共享机制：一个示例凭场景表加夹具目录即可获得完整快照套件，全部比较与防护机制集中于此而非逐例复制。


## 不变量

**运行时不变量：** 未发布配套入口。测试支持库；其比较与守护机制由消费它的快照套件覆盖。

## 模型体验

没有直接影响；测试夹具仅观察并规范化转录，不改变真实请求。
