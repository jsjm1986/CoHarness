 ...
 */
const PLACEHOLDER_CHILD_IDS = new Set(['33333333-3333-4333-8333-333333333333', '{{session:2}}'])
...
export function apply(ctx: Context): void {
  ...
  agents.create = async (options) => {
    const handle = await create.call(agents, options)
  ...
  persistence.stat = async (id, options) => {
    ...
    return stat.call(persistence, id, options)
  }
  ctx.effect(() => () => {
  ...
  const deliver = subagents.sendMessage
  ctx.effect(() => () => { subagents.sendMessage = deliver }, 'snapshot child identity mapping')
  subagents.sendMessage = (authority, childId, content, options) => {
    const mapped = PLACEHOLDER_CHILD_IDS.has(childId) && realChildId !== undefined
      ? SessionId(realChildId)
      : childId
    return deliver.call(subagents, authority, mapped, content, options)
  }
