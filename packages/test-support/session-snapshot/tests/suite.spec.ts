...
describe('stabilizeRefreshLog', () => {
  it('preserves one compaction identity across lifecycle declarations and checkpoint references', () => {
    const log = (id: string, endId = id): string => [
      { type: 'session', id: 'same', createdAt: 1, cwd: '/same' },
      { type: 'compaction/start', data: { compactionId: id, turn: 1 } },
      { type: 'compaction/summary', data: { compactionId: id } },
      { type: 'user/message', data: { source: { kind: 'plugin', plugin: 'compact', compactionId: id }, content: [] } },
      { type: 'compaction/end', data: { compactionId: endId, turn: 1 } },
    ].map(record => JSON.stringify(record)).join('\n') + '\n'
    const first = '11111111-1111-4111-8111-111111111111'
    const second = '22222222-2222-4222-8222-222222222222'
    const wrong = '33333333-3333-4333-8333-333333333333'
    expect(stabilize(log(first), log(second))).toBe(log(second))
    expect(stabilize(log(first), log('{{compaction:1}}'))).toBe(log('{{compaction:1}}'))
    expect(stabilize(log(first, wrong), log(second))).toContain(wrong)
  })
...
it('refreshes owned headers but refuses to overwrite a borrowed header', async () => {
  const root = mkdtempSync(join(tmpdir(), 'snapshot-header-owner-'))
  try {
    const owner = join(root, 'owner.json'), alias = join(root, 'borrowed.json')
    await writeOwnedHeaderSnapshot(owner, 'first\n')
    symlinkSync(owner, alias)
    await expect(writeOwnedHeaderSnapshot(alias, 'first\n')).resolves.toBeUndefined()
    await expect(writeOwnedHeaderSnapshot(alias, 'changed\n')).rejects.toThrow('borrowed header differs')
