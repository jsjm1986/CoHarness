...
/**
 * Update a local header attachment while keeping aliases to other owners read-only.
 * @param path - Declared prompt or schema attachment path.
 * @param content - Complete header text produced by the owning composition.
 * @returns Resolves after writing a local file or verifying the borrowed bytes.
 */
export async function writeOwnedHeaderSnapshot(path: string, content: string): Promise<void> {
  ...
  if (borrowed) {
    if (await readFile(path, 'utf8') !== content) {
      throw new Error(`snapshot: borrowed header differs from its owner: ${path}; refresh the owner or declare a distinct composition`)
    }
    return
  }
...
/** Normalize a complete log so declarations and their references share identity mappings. */
function normalizedRefreshRecords(
  records: readonly Record<string, unknown>[],
  context: NormalizeContext,
): Record<string, unknown>[] {
  const log = records.map(record => JSON.stringify(record)).join('\n')
  return parseJsonlRecords(normalizeSessionLog(log, context))
}
...
function collectNormalizedStringMappings(
  ...
  if (typeof normalizedFresh === 'string' && typeof normalizedExisting === 'string'
    && /^\{\{[a-z]+:[1-9]\d*\}\}$/.test(normalizedFresh)
    && /^\{\{[a-z]+:[1-9]\d*\}\}$/.test(normalizedExisting)
    && normalizedFresh !== normalizedExisting
    && typeof fresh === 'string' && !excludedStrings.has(fresh)
    && typeof existing === 'string' && !excludedStrings.has(existing)) return false
  if (
    typeof fresh !== 'string'
    || typeof existing !== 'string'
    || typeof normalizedFresh !== 'string'
...
function normalizedStringMappings(
  ...
  normalizedFresh: Record<string, unknown>[],
  normalizedExisting: Record<string, unknown>[],
...
export function stabilizeRefreshLog(
  ...
  const normalizedFresh = normalizedRefreshRecords(freshRecords, freshContext)
  const normalizedExisting = normalizedRefreshRecords(existingRecords, fixtureContext(existing))
    ...
    for (const scenario of scenarios) {
      ...
      it.skipIf(scenarioSkipped(scenario, RECORDING, process.platform, options.hasPwsh))(`snapshot: ${scenario.name} matches the expected outputs`, async ({ expect }) => {
        ...
        if (writesSessionFixtures) {
          ...
          for (const fixture of outputFixtures) parseSessionLog(fixture)
          ...
          if (scenario.pinsHeader === true) {
            ...
            await writeOwnedHeaderSnapshot(promptPath, promptSnapshot)
            ...
            await writeOwnedHeaderSnapshot(schemaPath, toolSchemasSnapshot)
          }
