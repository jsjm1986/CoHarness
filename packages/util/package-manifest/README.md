...
The package root only re-exports declarations from [`src/types.ts`](src/types.ts). Its required invariant companion is intentionally empty because the package has no runtime state or independently observable relationships.
