...
`dsh-webhook` 提供 Host 侧的 `ctx.webhookRuntime`：它既是受信任程序化 webhook 规则的注册表，也拥有唯一内置动作——在 Web Workspace 中创建普通根会话。使用 `register(rule)` 注册受信任规则；提供方身份验证属于适配器包。当受信任规则必须把外部事件变成新的 agent（智能体）会话时，请使用它。
...
`invoke(ruleId, delivery, signal?)` 只等待选中的规则，返回已接纳提示词的 Session id；规则不请求动作时返回 `null`。规则缺失、提供方不匹配、取消与接纳失败都会拒绝。它与 `dispatch()` 共享注册卸载处理，不等待模型输出，也不证明 Agent 成功。
...
受管创建在应用预设前，通过当前执行授权服务登记输入身份并核验权限选择。缺少身份、资格撤销、授权服务替换或取消都会阻止提示词接收。
...
每个匹配规则都会让模型看到 `WebhookSessionRequest.prompt` 返回的非空文本原文。通用运行时不增加私有框架；若规则包含外部文本，则由规则负责标明其信任属性。外部事件文本必须与规则的受信任指令保持区分。
...
- **无持久队列** — 崩溃会丢失尚未接纳提示词的规则调用；不存在队列、回放或重试。
