...
interface RuleRegistration {
  ...
  readonly active: Set<Promise<SessionId | null>>
  closing: boolean
...
export class WebhookRuntime extends Service {
  ...
  dispatch<K extends string>(delivery: VerifiedWebhookDelivery<K>): void {
    ...
    for (const registration of [...this.rules.values()]) {
      ...
      void this.startInvocation(registration, snapshot).catch((error: unknown) => {
        const invocation = `webhook: provider=${JSON.stringify(snapshot.kind)} source=${JSON.stringify(snapshot.source)} `
          + `delivery=${JSON.stringify(snapshot.deliveryId)} rule=${JSON.stringify(registration.rule.id)}`
  ...
  async invoke(ruleId: WebhookRuleId, delivery: VerifiedWebhookDelivery, signal?: AbortSignal): Promise<SessionId | null> {
    if (this.closing) throw new Error('webhook runtime is closing')
    const registration = this.rules.get(ruleId)
    if (registration === undefined || registration.rule.kind !== delivery.kind) throw new Error('webhook rule is absent or belongs to another provider')
    return this.startInvocation(registration, snapshotDelivery(delivery), signal)
  }

  /** Registration disposal owns every invocation, including those awaited by managed intake. */
  private startInvocation(
    registration: RuleRegistration, delivery: VerifiedWebhookDelivery, cancellation?: AbortSignal,
  ): Promise<SessionId | null> {
    const signal = cancellation === undefined
      ? registration.controller.signal : AbortSignal.any([registration.controller.signal, cancellation])
    const tracked = Promise.resolve().then(async () => {
      signal.throwIfAborted()
      const request = await registration.rule.run(delivery, signal)
      signal.throwIfAborted()
      if (request !== null) {
        return createWebhookSession(
          this.selfCtx,
          delivery,
