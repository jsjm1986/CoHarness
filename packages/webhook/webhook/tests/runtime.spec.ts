...
describe('WebhookRuntime', () => {
  it('awaits only the selected rule and propagates its refusal without logging it as success', async () => {
    const { runtime, ctx } = harness()
    const sibling = vi.fn(() => null)
    runtime.register({ id: WebhookRuleId('selected'), kind: 'fixture', run: () => null })
    runtime.register({ id: WebhookRuleId('sibling'), kind: 'fixture', run: sibling })
    runtime.register({ id: WebhookRuleId('refused'), kind: 'fixture', run: () => { throw new Error('not authorized') } })
    await expect(runtime.invoke(WebhookRuleId('selected'), delivery())).resolves.toBeNull()
    expect(sibling).not.toHaveBeenCalled()
    await expect(runtime.invoke(WebhookRuleId('refused'), delivery())).rejects.toThrow('not authorized')
    await expect(runtime.invoke(WebhookRuleId('missing'), delivery())).rejects.toThrow('absent')
    await expect(runtime.invoke(WebhookRuleId('selected'), { ...delivery(), kind: 'other' })).rejects.toThrow('another provider')
    await ctx.fiber.dispose()
    await expect(runtime.invoke(WebhookRuleId('selected'), delivery())).rejects.toThrow('closing')
  })

  it.each(['caller', 'registration'] as const)('drains an awaited invocation cancelled by its %s', async (owner) => {
    const { runtime } = harness()
    const caller = new AbortController()
    const entered = Promise.withResolvers<AbortSignal>()
    const finish = Promise.withResolvers<boolean>()
    const dispose = runtime.register({ id: WebhookRuleId('awaited'), kind: 'fixture', async run(value, signal) {
      expect(Object.isFrozen(value.event)).toBe(true)
      entered.resolve(signal)
      await finish.promise
      return null
    } })
    const result = runtime.invoke(WebhookRuleId('awaited'), delivery(), caller.signal)
    const rejected = expect(result).rejects.toThrow()
    const signal = await entered.promise
    let drained = false
    const disposal = owner === 'registration' ? dispose().then(() => { drained = true }) : undefined
    if (owner === 'caller') caller.abort(new Error('caller stopped'))
  ...
  it('does not enter a rule for an already cancelled invocation', async () => {
    const { runtime } = harness()
    const run = vi.fn(() => null)
    runtime.register({ id: WebhookRuleId('cancelled'), kind: 'fixture', run })
    await expect(runtime.invoke(WebhookRuleId('cancelled'), delivery(), AbortSignal.abort(new Error('cancelled')))).rejects.toThrow('cancelled')
