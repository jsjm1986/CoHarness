...
describe('webhook Session creation', () => {
  ...
  it('stamps the actual input and checks its permission selection before admission', async () => {
    const authority = {
      stamp: vi.fn<ExecutionAuthority['stamp']>(async (_session, message) => message),
      authorizeSelection: vi.fn<ExecutionAuthority['authorizeSelection']>(async () => {}),
    }
    const test = harness({ authority })
    authority.stamp.mockImplementation(async (_session, message) => {
      test.calls.push('stamp')
      return { ...message, id: 'verified-message' as typeof message.id }
    })
    authority.authorizeSelection.mockImplementation(async () => { test.calls.push('authorize') })
    await create(test)
    expect(authority.authorizeSelection).toHaveBeenCalledWith(test.agent, 'read-only')
    expect(test.calls.slice(-6)).toEqual(['attach', 'stamp', 'authorize', 'permission-set:read-only', 'title', 'followup'])
    expect(test.messages).toEqual([expect.objectContaining({ id: 'verified-message' })])
  })

  it.each(['stamp', 'authorizeSelection'] as const)('rolls back when managed %s refuses admission', async (stage) => {
    const authority = {
      stamp: vi.fn<ExecutionAuthority['stamp']>(async (_session, message) => message),
      authorizeSelection: vi.fn<ExecutionAuthority['authorizeSelection']>(async () => {}),
    }
    authority[stage].mockRejectedValue(new Error('not authorized'))
    const test = harness({ authority })
    await expect(create(test)).rejects.toThrow('not authorized')
    expect(test.messages).toEqual([])
    expect(test.calls).not.toContain('permission-set:read-only')
    expect(test.calls.slice(-2)).toEqual(['detach', 'dispose'])
  })

  it('rejects cancellation during authorization without publishing a prompt', async () => {
    const authority = {
      stamp: vi.fn<ExecutionAuthority['stamp']>(async (_session, message) => message),
      authorizeSelection: vi.fn<ExecutionAuthority['authorizeSelection']>(async () => {}),
    }
    const test = harness({ authority })
    authority.authorizeSelection.mockImplementation(async () => { test.controller.abort(new Error('cancelled')) })
    await expect(create(test)).rejects.toThrow('cancelled')
    expect(test.messages).toEqual([])
    expect(test.calls.slice(-2)).toEqual(['detach', 'dispose'])
  })

  it('refuses an authority replacement while permission validation is in flight', async () => {
    const options: HarnessOptions = { authority: {
      stamp: async (_session, message) => message,
      authorizeSelection: async () => { delete options.authority },
    } }
    const test = harness(options)
    await expect(create(test)).rejects.toThrow('authority changed')
    expect(test.messages).toEqual([])
    expect(test.calls.slice(-2)).toEqual(['detach', 'dispose'])
  })
