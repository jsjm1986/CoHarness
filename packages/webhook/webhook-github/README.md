...
The normal Web profile already owns `ctx.webServer`. Mount another `dsh-host-webserver` and this adapter inside a group that isolates only `webServer`; the adapter still inherits credentials and `webhookRuntime`. The [Webhook subsystem reference](../../../docs/subsystems/webhook.md) distinguishes this optional local listener from managed Gateway intake. Shipped profiles do not enable this route.
