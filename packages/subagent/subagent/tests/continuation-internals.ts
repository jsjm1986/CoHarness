...
export function dropContinuationActivation(ctx: Context, childId: SessionId): void {
  ...
  const dropped = registry.resident.get(childId)
  registry.resident.delete(childId)
  // An unmanaged entry has no settlement owner; closing its Inbox stops the
  // orphaned watcher before teardown releases the projection registration.
  void dropped?.inbox.close(() => Promise.resolve())
}
