...
async function setupWith(
  adapter: LlmAdapter,
  options: { persistence?: boolean; schedule?: boolean; subagent?: ConstructorParameters<typeof SubagentRuntime>[1] } = {},
) {
  ...
  if (options.persistence !== false) {
    ...
    cleanups.push(async () => {
      // Disposing the whole fiber closes every session's write lease; a bare
      // persistence-fiber dispose would leave agent-owned handles to GC.
...
describe('continuable activation capacity', () => {
  ...
  it('layers editable depth over composition and removes the section on disposal', async () => {
    ...
    try {
      ...
      await expect(ctx.settings.update(SUBAGENT_SETTINGS_NAMESPACE, { maxDepth: -0 })).rejects.toThrow()
      await expect(ctx.settings.update(SUBAGENT_SETTINGS_NAMESPACE, { maxDepth: -1 })).rejects.toThrow()
      await expect(ctx.settings.update(SUBAGENT_SETTINGS_NAMESPACE, { maxDepth: 1.5 })).rejects.toThrow()
      await expect(ctx.settings.update(SUBAGENT_SETTINGS_NAMESPACE, { maxActiveSubagents: 0 })).rejects.toThrow()
      expect(ctx.subagents.resolveMaxDepth()).toBe(0)
      await ctx.settings.replace(SUBAGENT_SETTINGS_NAMESPACE, {})
      expect(ctx.subagents.resolveMaxDepth()).toBe(4)
      await fiber.dispose()
      expect(ctx.settings.describe().some(section => section.ns === SUBAGENT_SETTINGS_NAMESPACE)).toBe(false)
    } finally {
      await ctx.fiber.dispose()
    }
  })
  ...
  it('applies capacity edits to an existing root without stopping resident children', async () => {
    ...
    const { ctx, parent } = await setupWith(adapter, { subagent: { maxActiveSubagents: 1 } })
    ...
    try {
      ...
      await ctx.settings.update(SUBAGENT_SETTINGS_NAMESPACE, { maxActiveSubagents: 2 })
      const second = await ctx.subagents.startContinuable(startSpec(parent))
      await ctx.settings.update(SUBAGENT_SETTINGS_NAMESPACE, { maxActiveSubagents: 1 })
      expect(ctx.agents.get(first.childId)).toBeDefined()
      expect(ctx.agents.get(second.childId)).toBeDefined()
      await expect(ctx.subagents.startContinuable(startSpec(parent))).rejects.toMatchObject({ code: 'ACTIVATION_LIMIT_REACHED' })
      await ctx.settings.update(SUBAGENT_SETTINGS_NAMESPACE, { maxActiveSubagents: 3 })
  ...
  it('shares slots across siblings, providers and nested children while isolating roots', async () => {
    ...
    const { ctx, parent } = await setupWith(adapter, { subagent: { maxActiveSubagents: 3 } })
  ...
  it('reserves the last slot before asynchronous creation and returns it after failure', async () => {
    const { ctx, parent } = await setupWith(new MockAdapter([textResponse('replacement')]), { subagent: { maxActiveSubagents: 1 } })
    parkParent(ctx, parent)
...
describe('SubagentRuntime.startContinuable', () => {
  ...
  it('enforces the global Activation limit across different parents', async () => {
    ...
    const otherParent = await ctx.agentLoop.create(SessionId('other-parent'), { provider: 'mock', model: 'mock' })
    const started = await ctx.subagents.startContinuable(startSpec(parent))
    await vi.waitFor(() => { expect(adapter.requests).toHaveLength(1) })
...
describe('direct-child Queue residency routing', () => {
  it('fails a cold follow-up when session persistence is unavailable', async () => {
    const { ctx, parent } = await setupWith(new MockAdapter([]), {
      persistence: false,
    })

    await expect(queuePrompt(ctx, parent, SessionId('cold-without-persistence'), message('continue')))
