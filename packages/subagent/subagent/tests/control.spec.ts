...
// registry, prompt admission through `subagent/prompt-admission`, and the
// stable failure codes each answers with. The durable listing, continuation,
// and interrupt primitives they wrap have their own specs, so each case
// scripts them.
...
const IMAGE_REF: ImageAttachmentRef = {
  attachmentId: AttachmentId('att'), mediaType: 'image/png', bytes: 2, width: 1, height: 1,
}
...
describe('subagent prompt Remote', () => {
  ...
  it('lets a prompt-admission listener own durable admission ahead of the store', async () => {
    const { ctx, subagents } = await bench({ [PARENT]: { status: 'idle' } })
    const saveImages = vi.fn()
    ctx.provide('attachments', Object.setPrototypeOf({ saveImages }, AttachmentStore.prototype) as never)
    const admitted: AdmittedPromptContentPart[] = [
      { type: 'text', text: 'see this' },
      { type: 'image', attachment: { ...IMAGE_REF, attachmentId: AttachmentId('att-hook') } },
    ]
    const listener = vi.fn(() => Promise.resolve(admitted))
    ctx.on('subagent/prompt-admission', listener)
    const delivery = promptDelivery(subagents).mockResolvedValue('m-hook' as MessageId)
    const content = [
    ...
    await expect(subagents.prompt({ ...promptRequest(), content }, signal))
      .resolves.toEqual({ messageId: 'm-hook' })
    expect(listener).toHaveBeenCalledWith({ status: 'idle' }, content)
    expect(saveImages).not.toHaveBeenCalled()
    expect(delivery.mock.calls[0]?.[2]).toEqual(admitted)
  })

  it('runs admission listeners in registration order until one returns content', async () => {
    const { ctx, subagents } = await bench({ [PARENT]: { status: 'idle' } })
    const admitted = [{ type: 'text' as const, text: 'admitted' }]
    const first = vi.fn(() => undefined)
    const second = vi.fn(() => Promise.resolve(admitted))
    const third = vi.fn(() => Promise.resolve([{ type: 'text' as const, text: 'unreached' }]))
    ctx.on('subagent/prompt-admission', first)
    ctx.on('subagent/prompt-admission', second)
    ctx.on('subagent/prompt-admission', third)
    const delivery = promptDelivery(subagents).mockResolvedValue('m-order' as MessageId)

    await expect(subagents.prompt(promptRequest(), signal))
      .resolves.toEqual({ messageId: 'm-order' })
  ...
  it('maps a listener attachment refusal to subagent/attachment-invalid', async () => {
    const { ctx, subagents } = await bench({ [PARENT]: { status: 'idle' } })
    ctx.on('subagent/prompt-admission', () => {
