...
The child gets the parent's working-directory/session lineage and inherits the provider and model from the parent's latest logged request target when one exists; otherwise it uses the parent's creation options. The output-token cap remains the parent's creation-scoped option. `request.agentOptions` overrides these inherited values. It gets a fresh flat registration scope: parent ownership does not import parent tool restrictions or establish an authority subset.
...
- A `tools/result` observer commits a staged value only after that execution's authoritative final tool result succeeds, including the enclosing `run_code` result for PTC mode sub-dispatch.
