...
子 agent 会获得父 agent 的工作目录／会话谱系；如果父 agent 已有最近一次记录的请求目标，就从该目标继承提供方和模型，否则使用父 agent 的创建参数。输出 token 上限仍取父 agent 创建时的选项。`request.agentOptions` 会覆盖这些继承值。它获得全新的扁平注册作用域：父级所有权不会导入父 agent 的工具限制，也不会建立权限子集。
...
- `tools/result` 观察器只会在该次执行的权威最终工具结果成功后提交暂存值；PTC mode 子分派外层的 `run_code` 结果也包括在内。
