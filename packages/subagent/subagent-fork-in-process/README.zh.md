...
在提供方和模型相同的前提下，子 agent 可以复用继承的逐字节相同前缀。persona、工具过滤、生成 SDK 或路由变化可能在继承历史之前使复用失效；后续子 agent 历史仅追加。可继续消息不会增加任何仅子 agent 可见的系统提示词 section 或工具 schema：父子双方收到相同的 `send_message` 定义与顺序，子 agent 的父级 id 与返回指引跟在继承历史之后、位于初始用户任务中，因此两种生命周期绑定都让前缀保持可复用（见 [缓存保持型 fork Agent Note](../../../.agents/notes/implemented/architecture/2026-08-10-fork-children-stay-one-shot.zh.md)）。
