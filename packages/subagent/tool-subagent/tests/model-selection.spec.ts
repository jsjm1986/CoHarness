...
describe('dsh-tool-subagent model selection', () => {
  ...
  it('requires an allowlist policy before honoring a model-facing route request', () => {
  ...
  it('detects explicit requests and rejects malformed option merges', () => {
    const parent: AgentOptions = { provider: 'parent', model: 'p' }
    expect(hasDelegationModelRequest({})).toBe(false)
    expect(hasDelegationModelRequest({ reasoning_effort: 'high' })).toBe(true)
    expect(() => requestedAgentOptions(parent, undefined, { provider: 'x' }, true))
      .toThrow('must be supplied together')
    expect(() => requestedAgentOptions(parent, undefined, { provider: '', model: 'x' }, true))
      .toThrow('non-empty')
    expect(() => requestedAgentOptions(parent, undefined, { model: 'x' }, false))
      .toThrow('disabled')
    expect(() => {
