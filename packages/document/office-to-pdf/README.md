...
Linux WASM deployments need usable fonts in the kit defaults or configured `fontDirectories`; a fontless image fails with `unavailable`. Latin and CJK rendering can be supplied by distribution packages such as DejaVu and Noto CJK. Installing the engine alone does not establish document rendering support.

Browsers call `workspaceFiles.renderOffice` through the existing [ApiProxy](../../host/apiproxy/README.md). This entry authorizes the Session and relative path before consulting conversion caches, reads bounded version-checked FS windows after queue admission, and rechecks access and source identity before returning. Responses contain the source-relative path, version, base64 PDF, missing fonts, and converter generation. The converter has no separate Remote file service; in-process `convert()` accepts already-authorized deferred reads.
...
The ApiProxy file reader owns request authorization and cancellation; the converter owns admitted reads, conversion, cache, and engine teardown. Source-read failures pass through, and conversion failures expose a classified reason without engine diagnostics.
...
- [Workspace Files](../../host/apiproxy/README.md) — Session file authorization and bounded reads.
