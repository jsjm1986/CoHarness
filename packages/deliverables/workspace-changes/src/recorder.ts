...
import type { PosixRecorderExecution } from './execution.ts'
import {
...
export class TurnRecorder {
  ...
  private async locate(paths: Paths, signal: AbortSignal): Promise<Repository | null> {
  ...
  private async record(state: TurnState, signal: AbortSignal): Promise<void> {
    ...
    for (const absolute of captured) {
      ...
      const after = await this.captureFile(absolute, signal)
