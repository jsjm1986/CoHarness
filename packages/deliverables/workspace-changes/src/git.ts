...
/** Snapshot storage in the same execution world as git. */
export interface GitStorage {
  canonical(path: string, signal?: AbortSignal): Promise<string>
  mkdir(path: string, signal?: AbortSignal): Promise<void>
  temporary(prefix: string, signal?: AbortSignal): Promise<string>
  copyIndex(source: string, destination: string, signal?: AbortSignal): Promise<void>
  remove(path: string): Promise<void>
}

const localStorage: GitStorage = {
  canonical: canonicalPath,
  mkdir: async (path) => { await mkdir(path, { recursive: true }) },
  temporary: prefix => mkdtemp(prefix),
  copyIndex: async (source, destination) => {
    await copyFile(source, destination).catch((error: unknown) => { if (!isMissing(error)) throw error })
  },
...
export class GitRunner {
  constructor(
    ...
    readonly storage: GitStorage = localStorage,
  ) {}
...
export async function locateGitWorkspace(
  ...
  const directory = await git.storage.canonical(await scratch(), signal)
  const objects = join(directory, 'objects')
  await git.storage.mkdir(objects, signal)
...
export async function snapshotTree(git: GitRunner, workspace: GitWorkspace, signal: AbortSignal): Promise<string> {
  const scratch = await git.storage.temporary(join(workspace.scratch, 'index-'), signal)
  try {
    ...
    await git.storage.remove(scratch)
  }
}
