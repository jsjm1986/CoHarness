...
export async function captureFile(absolute: string, directory: string, maxBytes: number): Promise<Capture | undefined> {
  ...
  return storeCapture(bytes, directory)
}

/**
 * Retain authorized bytes in the Host-owned content-addressed capture store.
 * @param bytes - bounded complete bytes from the owning filesystem.
 * @param directory - private directory holding captures.
 * @returns a stable captured copy independent of subsequent source edits.
 */
export async function storeCapture(bytes: Uint8Array, directory: string): Promise<Capture> {
