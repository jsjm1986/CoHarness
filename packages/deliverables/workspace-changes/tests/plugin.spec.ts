...
it.skipIf(process.platform === 'win32')('captures execution-target file edits when target git is unavailable', async () => {
  const cwd = await scratchDir('remote-workspace-no-git-', cleanups)
  const { ctx } = await boot()
  vi.spyOn(ctx.fs, 'processPathFromHostPath').mockReturnValue(undefined)
  vi.spyOn(PosixRecorderExecution.prototype, 'gitAvailable').mockResolvedValue(false)
  const session = ctx.sessions.create(SessionId('remote-no-git'), { meta: { cwd } })
  startTurn(session, 1)
...
it.skipIf(process.platform === 'win32')('awaits a retiring Session snapshot directory and reports cleanup failure', async () => {
  const cwd = await repository()
  const { ctx, fiber } = await boot()
  vi.spyOn(ctx.fs, 'processPathFromHostPath').mockReturnValue(undefined)
  const session = ctx.sessions.create(SessionId('remote-retiring'), { meta: { cwd } })
  startTurn(session, 1)
  await settle(ctx, session)
  const entered = Promise.withResolvers<undefined>()
  const finish = Promise.withResolvers<undefined>()
  const remove = Reflect.get(PosixRecorderExecution.prototype, 'remove')
  vi.spyOn(PosixRecorderExecution.prototype, 'remove').mockImplementation(async function (this: PosixRecorderExecution, path) {
  ...
  finish.reject(new Error('remote cleanup acknowledgement lost'))
  await disposal
  expect(warn.mock.calls.some(call => String(call[0]).includes('cleanup failed'))).toBe(true)
