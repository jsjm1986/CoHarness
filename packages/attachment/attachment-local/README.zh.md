...
实例压缩限流器会在带 head index 的 FIFO 中保存排队的变换，因此在突发工作期间仍保持配置的并发上限，不会反复复制数组头部。
