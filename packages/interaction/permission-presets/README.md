...
A deployment may provide asynchronous `authorizeSelection(agent, name)` and `authorizeDefault(name)` policies. The command awaits selection authorization before writing any permission event, and cancellation prevents a late grant from changing the Session. The `permission` settings write awaits default authorization before persistence; Gateway Full access defaults require a fresh administrator check. Managed deployments refuse writes while their execution authority is unavailable. Registration and restored settings do not run request-dependent write authorization.
