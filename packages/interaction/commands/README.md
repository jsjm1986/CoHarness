...
`definitionId` is an optional branded, plugin-namespaced identity independent of the command name, display copy, and per-execution `commandId`. When present, the effective descriptor carries only the selected definition's identity, so a scoped shadow never inherits the global definition's identity. Older and third-party definitions may omit it.
