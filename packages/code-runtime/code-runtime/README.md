...
| `language` | Readonly descriptor: the source language `run` expects. `'typescript'` and `'python'` are the well-known values — those `dsh-tools` presents; `'typescript'` has the shipped worker-thread backend, and `'python'` has the private [`dsh-experimental-code-runtime-python`](../../experimental/code-runtime-python/README.md) subprocess backend for opt-in compositions. Informational, not gating — a consumer that generates language-specific presentation switches on it and fails loud on a language it cannot present. |
...
- **A persistent REPL-style kernel is recorded future work** — the no-state-between-runs contract stands until a persistent-kernel backend brings its own logging story ([PTC mode Agent Note](../../../.agents/notes/implemented/feature/2026-06-15-ptc.md)).
