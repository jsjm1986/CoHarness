...
import type { RemoteStreamConnection } from '../src/client/remote-stream.ts'
import {
...
function hostSource(initiallyAvailable: boolean): {
  connection: RemoteStreamConnection
...
it('reports a readiness subscription failure without reopening the stream', async () => {
  const failure = new Error('readiness source failed')
  const stream = supervisor({ generation: {
    getSnapshot: () => undefined,
    subscribe: () => { throw failure },
  } }, [{ terminal: new RemoteStreamCarrierError('offline') }])
  await expect(stream[Symbol.asyncIterator]().next()).rejects.toMatchObject({ code: 'gateway/internal', cause: failure })
  await stream.dispose()
})

it('aborts a retry after its readiness subscription is established', async () => {
  const subscribed = Promise.withResolvers<undefined>()
  const disposed = vi.fn()
  const stream = supervisor({ generation: {
    getSnapshot: () => undefined,
    subscribe: () => { subscribed.resolve(undefined); return disposed },
  } }, [{ terminal: new RemoteStreamCarrierError('offline') }])
  const pending = stream[Symbol.asyncIterator]().next()
  await subscribed.promise
  await stream.dispose()
