...
/** Current physical readiness; undefined while the owning Connection is unavailable. */
export interface RemoteStreamConnection {
  readonly generation: { getSnapshot(): unknown; subscribe(listener: () => void): () => void }
}
...
export interface RemoteStreamOptions<Item> {
  /** Session whose runtime transport owns this stream. */
...
export class RemoteStream<Item> implements AsyncIterable<RemoteStreamItem<Item>> {
   ...
   */
  constructor(
    private readonly connection: RemoteStreamConnection,
