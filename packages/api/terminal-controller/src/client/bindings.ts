...
const PREFIX = 'dsh.terminal.binding.v2.'
...
export class TerminalBindings {
  ...
  constructor(private readonly ownerKey: (sessionId: SessionId) => string | undefined) {}
  ...
  set(sessionId: SessionId, contentId: string, id: WebTerminalId): void {
    const key = this.key(sessionId, contentId)
    if (key === undefined) throw new Error('Terminal ownership is not verified')
  ...
  private key(sessionId: SessionId, contentId: string): string | undefined {
    const owner = this.ownerKey(sessionId)
    return owner === undefined ? undefined : PREFIX + JSON.stringify([owner, contentId])
  }
}
