...
it('preserves other windows records through interleaved writes, closes and reloads', () => {
  const a = new TerminalBindings(ownerKey)
  const b = new TerminalBindings(ownerKey)
  expect(b.get(session, 'b')).toBeUndefined()
  a.set(session, 'a', first)
  b.set(session, 'b', second)
  a.delete(session, 'a')
  const reloaded = new TerminalBindings(ownerKey)
  ...
  expect(new TerminalBindings(ownerKey).get(session, 'b')).toBe(second)
  expect(new TerminalBindings(ownerKey).get('session.with.dot' as SessionId, 'b')).toBe(first)
})
...
it.each(['{broken', 'null', '42', '"not/a/terminal"'])('rejects malformed saved identity %s', (value) => {
  ...
  expect(new TerminalBindings(ownerKey).get(session, 'content')).toBeUndefined()
})
