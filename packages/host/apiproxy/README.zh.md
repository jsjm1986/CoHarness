...
`workspaceFiles.renderOffice` 复用文件域的授权、相对路径及版本检查。内容权限探测在查询转换缓存前执行；队列准入后进行有界字节窗口读取，返回前再次核验源文件和权限。结果包含 PDF、缺失字体及转换代次，不暴露 Host 路径或引擎诊断。转换限制由 [Office 提供方](../../document/office-to-pdf/README.zh.md)负责。
...
`agentPreset.openDocument` 是该领域仅存的一元 RPC。它把一个本地创作 preset 的**目录**交给平台打开器——请求只携带 id、绝不携带路径，因此没有任何浏览器载荷能选中任意文件系统目标；部署没有原生打开器时回答 `{ opened: false, path }` 供界面以文本展示，随附 preset 以 `agent-preset-read-only` 拒绝，而网关的 `nativeOpen` 配置可在平台探测（`canOpenNativePath`）失真处钉死该能力。
...
用途受限的 `plugin-admin` 请求在 Plugin Manager 重新核验权限后，可描述或原子修改 Profile 设置。管理员写入必须携带 `expectedRevision`；账户归属的命名空间保持只读。此路径不向普通项目成员授予部署配置权，也不开放 `settings.openDocument` 或整节替换。
