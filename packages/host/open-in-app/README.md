...
`dsh-host-open-in-app` is the desktop-only host half of open-in-app: it resolves which catalog applications this host holds into verified launchers and registers three loopback-protected `ctx.webServer` routes — resolved list, icons, and the launch endpoint that opens a workspace directory. Resolution runs once per process; adapters spawn detached with scrubbed credentials. The consumer is [`dsh-client-ui-open-in-app`](../../client/ui-open-in-app/README.md)'s split button.
