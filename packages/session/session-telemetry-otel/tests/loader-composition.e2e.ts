 ...
 * deployment uses), run mocked-model turns with a real bash round trip, and
 * assert against what the mock OTLP collector actually received on the
 * wire: the feedback-authorized prefix, the deployment-mounted redact rule
 * applied to the exported copy, and the untouched canonical log.
 */
