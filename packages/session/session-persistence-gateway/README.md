...
- `readHeader(id, signal?)` and `readRevision(id, signal?)` use indexed metadata endpoints. `readPage(id, request, signal?)` uses the PostgreSQL `(session_id, seq)` keyset query, returns at most 512 KiB/2,000 events/50 groups per page, and binds its continuation cursor to the source revision. The provider preserves typed page failures and never sends a full detached history through the Gateway page route.
