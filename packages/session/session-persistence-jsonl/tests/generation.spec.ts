...
function verifier(): PrepareJsonlMigrationOptions['verifyCurrentFile'] {
  return (path, compression, expectedId, expectedEventCount, expectedPrefix, _signal, expectedVersion) =>
    verifyJsonlCurrentGeneration(path, compression, expectedId, expectedEventCount, expectedPrefix, expectedVersion)
}
...
function pathVersion(path: string): number {
  const match = /session(?:\.v(\d+))?\.jsonl(?:\.zstd)?$/.exec(basename(path))
  if (match === null) throw new Error(`noncanonical test generation path: ${path}`)
...
describe('JSONL immutable generation publication', () => {
  ...
  it('returns migrated events while publication is still waiting for verification', async () => {
    ...
    const prepared = await prepareJsonlMigration({
      ...request,
      verifyCurrentFile: async (path, compression, expectedId, expectedEventCount, _prefix, _signal, expectedVersion) => {
        entered.resolve(undefined)
        await release.promise
        return verifyJsonlCurrentGeneration(path, compression, expectedId, expectedEventCount, undefined, expectedVersion)
      },
    })
  ...
  it('fails publication without rerunning migration when the source changes', async () => {
    ...
    const prepared = await prepareJsonlMigration({
      ...request,
      verifyCurrentFile: async (path, compression, expectedId, expectedEventCount, _prefix, _signal, expectedVersion) => {
        const verified = await verifyJsonlCurrentGeneration(path, compression, expectedId, expectedEventCount, undefined, expectedVersion)
  ...
  it('verifies exact current identity, completeness, and event count', async () => {
    ...
    await expect(verifyJsonlCurrentGeneration(path, 'none', 'other', 1, undefined, pathVersion(path)))
      .rejects.toThrow('expected "other"')
    await expect(verifyJsonlCurrentGeneration(path, 'none', 'generation-test', 2, undefined, pathVersion(path)))
      .rejects.toThrow('contains 1 events')
    await writeFile(path, line({ ...header(3), isSeeded: false }) + JSON.stringify(event0))
    await expect(verifyJsonlCurrentGeneration(path, 'none', 'generation-test', 1, undefined, pathVersion(path)))
      .rejects.toThrow('torn physical tail')

    await writeFile(path, Buffer.alloc(0))
    await expect(verifyJsonlCurrentGeneration(path, 'none', 'generation-test', 0, undefined, pathVersion(path)))
      .rejects.toThrow('empty or header-less')
    await expect(verifyJsonlCurrentGeneration(path, 'zstd', 'generation-test', 0, undefined, pathVersion(path)))
    ...
    await expect(verifyJsonlCurrentGeneration(path, 'zstd', 'generation-test', 1, undefined, pathVersion(path)))
      .rejects.toThrow('torn physical tail')
  ...
  it('keeps complete Assistant stream checks in current-generation verification', async () => {
    ...
    const verify = async (events: readonly SessionFormatEvent[]) => {
      ...
      return verifyJsonlCurrentGeneration(path, 'none', 'generation-test', events.length, undefined, pathVersion(path))
    }
  ...
  it('checks migration and verification identities exactly', async () => {
    ...
    const mismatched = await prepareJsonlMigration({
      ...mismatch,
      verifyCurrentFile: async (path, compression, expectedId, expectedEventCount, _prefix, _signal, expectedVersion) => ({
        ...await verifyJsonlCurrentGeneration(path, compression, expectedId, expectedEventCount, undefined, expectedVersion),
        digest: 'different',
      }),
    })
