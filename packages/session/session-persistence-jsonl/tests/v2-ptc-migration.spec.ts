...
describe('JSONL V2 PTC publication and restore', () => {
  ...
  it('publishes V4 without changing V2 bytes and preserves restored IDs, roles, content, and replay state', async () => {
    ...
    const successor = join(dirname(predecessor), 'session.v5.jsonl')
    ...
    try {
      expect(reader.header).toEqual({ version: 5, id, createdAt: 1000, isSeeded: false, delegationDepth: 0 })
    ...
    expect(publishedHeader).toEqual({ ...header, version: 5 })
