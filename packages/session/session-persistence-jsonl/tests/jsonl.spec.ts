...
describe('JsonlSessionPersistence: immutable format generations', () => {
  ...
  it('selects v1 from a v0/v1 directory, then v5 from the retained three-generation set', async () => {
    ...
    expect((await readdir(directory)).filter(name => name.startsWith('session')).sort())
      .toEqual(process.platform === 'win32'
        ? ['session.jsonl', 'session.v1.jsonl', 'session.v5.jsonl']
        : ['session.jsonl', 'session.lock', 'session.v1.jsonl', 'session.v5.jsonl'])
    ...
    expect(await readFile(v3Path, 'utf8')).toContain('"version":5')
  })
  ...
  it('fails a stale prepared publication once and re-prepares on the next write open', async () => {
    ...
    expect(await readFile(currentPath, 'utf8')).toContain('"version":5')
  })
  ...
  it('selects the highest opposite-encoding generation for its refusal', async () => {
    ...
    const highest = generationLogPath(root, header.cwd, header.id, 5, 'zstd')
...
describe('JsonlSessionPersistence: nested v3 Assistant streams', () => {
  ...
  it.each([2, 3])('reads v%s rows and appends a v5 turn without changing predecessor bytes', async (version) => {
    ...
    expect(restored.meta.version).toBe(5)
    ...
    expect(currentPath).toBe(join(dirname(sourcePath), 'session.v5.jsonl'))
    const successor = (await readFile(currentPath, 'utf8')).trimEnd().split('\n')
    expect(JSON.parse(successor[0] as string)).toMatchObject({ version: 5 })
