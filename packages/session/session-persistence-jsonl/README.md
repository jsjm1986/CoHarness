...
| `maxPendingEvents` | positive integer (default `100,000`) | Maximum events retained by one live session's write controller; a producer beyond the limit is rejected. |
| `maxPendingBytes` | positive integer (default `64 MiB`) | Maximum UTF-8 JSON bytes retained by one live session's write controller; a producer beyond the limit is rejected. |
| `maxHeaderBytes` | positive integer (default `64 KiB`) | Maximum bytes in one newline-terminated session header. Listing and loading reject larger headers before parsing them. |
| `readStableMaxAttempts` | positive integer (default `8`) | Maximum stat/read retries when a writer changes the artifact during a revision-stable read. |
