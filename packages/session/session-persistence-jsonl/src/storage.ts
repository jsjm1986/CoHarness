...
export class JsonlSessionHandle implements SessionHandle {
  ...
  close(): Promise<void> {
    return this.closing ??= (async () => {
      ...
      // fares: a skipped releaseTrackedHandle would wedge the id in this process
      ...
      this.storage.releaseTrackedHandle(this, this.state.materialized)
...
export class JsonlBackendTracker {
  ...
  /**
   * The adopted write handle for a session, if one owns it.
   * @param id - the session to look up.
   * @returns the active write handle, or `undefined` when unclaimed or still adopting.
   */
  writerOf(id: SessionId): JsonlSessionHandle | undefined {
    return this.writers.get(id) ?? undefined
  }
