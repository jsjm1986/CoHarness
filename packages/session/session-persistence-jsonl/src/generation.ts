 ...
 * @param expectedVersion - format version the file must declare; defaults to the catalog's current version.
...
export async function verifyJsonlCurrentGeneration(
  ...
  return defaultGenerationRuntime.verify(path, compression, expectedId, expectedEventCount, expectedPrefix, expectedVersion)
}
...
async function verifyCurrentGeneration(
  ...
  const generation = decodeCurrentGeneration(snapshot.bytes, compression, expectedVersion)
...
function decodeCurrentGeneration(
  ...
  if (compression === 'none') {
    const headerEnd = bytes.indexOf(0x0A)
    if (headerEnd === -1) throw new Error('empty or header-less session log')
    const scanner = new SessionLogScanner(bytes.subarray(0, headerEnd + 1), 'strict', expectedVersion)
  ...
  try {
    ...
    const scanner = new SessionLogScanner(header.value, 'strict', expectedVersion)
...
export function createJsonlGenerationRuntime(
  ...
  return {
    ...
    verify: (path, compression, expectedId, expectedEventCount, expectedPrefix, expectedVersion) => verifyCurrentGeneration(
      path, compression, expectedId, expectedEventCount, internals.fs, expectedPrefix, expectedVersion,
    ),
  }
}
