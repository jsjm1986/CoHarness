...
import { inspectSessionFormatVersion, snapshotSessionFormatArtifact, snapshotSessionFormatHeader } from './legacy-json.ts'
import type { SessionFormatArtifact, SessionFormatChain, SessionFormatChainOptions, SessionFormatHeader, SessionFormatMigration, SessionFormatMigrationContext, SessionFormatMigrationStage, SessionFormatMigrationStream, SessionFormatEvent } from './legacy-types.ts'
...
class CompiledSessionFormatChain implements SessionFormatChain {
  ...
  createStream(
    ...
    let edgeCut = inheritedEventCount
    ...
    for (const migration of plan) {
      ...
      const stage = migration.createStage({ sourceHeader: header, targetHeader, sourceInheritedEventCount: edgeCut })
      stages.push(stage)
      /* Each stage states its own output cut for the next edge; a stage that
       * reshapes the inherited region leaves it undefined and its downstream
       * derives the boundary from the stream instead. */
      edgeCut = stage.headerInheritedEventCount
      header = targetHeader
    }
    ...
    return {
      ...
      emitEvent,
      finish: () => {
        let targetInheritedEventCount = inheritedEventCount ?? 0
        for (let index = 0; index < stages.length; index += 1) {
