...
export interface SessionFormatCatalog {
  ...
  /**
   * Create one single-pass physical-row restore that decodes the artifact at
   * its own stored version without migrating to current. Verification of a
   * staged or version-pinned generation needs the file's declared generation,
   * not the build's current one.
   */
  createStoredRestore(headerValue: unknown, options: SessionFormatRestoreOptions): SessionFormatRestore
