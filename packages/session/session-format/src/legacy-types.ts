...
export interface SessionFormatMigrationStage {
  /**
   * Target inherited cut in this stage's own output coordinates, declared when
   * it is knowable before body decoding. Stages that reshape the inherited
   * region leave it undefined and state the boundary in-band instead.
   */
...
export interface SessionFormatMigration {
  ...
  readonly createStage?: (input: {
    ...
    /** Inherited cut in this edge's input coordinates, or undefined when a prior stage only states it in-band. */
