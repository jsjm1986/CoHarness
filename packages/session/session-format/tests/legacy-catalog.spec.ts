...
import type { SessionFormatEvent } from '../src/legacy-types.ts'
...
describe('Session format catalog', () => {
  ...
  it('migrates empty prompts, remaps replacements, and carries inherited cuts', () => {
    const source = {
      header: { version: 2, id: 'remap', createdAt: 1, agentPreset: 'code', isSeeded: true },
...
it('streams a v2 request-header migration with mapped surface references', () => {
  ...
  const stream = sessionFormatCatalog.createStream({ version: 2, id: 'stream-v2', createdAt: 1, isSeeded: true }, 1, {
  ...
  expect(output.map(item => item.type)).toEqual(['step/start', 'system/message', 'session/end-seed', 'system/message', 'request/header'])
})
