...
export function sessionFormatVersionRefusal(
  id: string,
  version: number,
  supportedVersion = SESSION_FORMAT_VERSION,
): string {
  return version > supportedVersion
    ? `session "${id}" uses log format v${version}, but this harness reads only v${supportedVersion}: the log was written by a newer harness — upgrade the harness to open it`
    : `session "${id}" uses log format v${version}, older than the supported v${supportedVersion}, and this build ships no upgrade path for it`
}
