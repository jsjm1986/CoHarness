...
| `readPage(id, request, signal?): Promise<SessionPersistencePage>` | Read one bounded ascending event range with `maxBytes` (512 KiB), `maxEvents` (2,000), and `maxGroups` (50) limits. A revision-bound cursor continues in the same direction and becomes invalid after an append or repair. Providers classify `too-large`, `aborted`, `timeout`, `dependency`, and `protocol` failures; providers without a seek implementation retain the complete-read compatibility fallback. |
