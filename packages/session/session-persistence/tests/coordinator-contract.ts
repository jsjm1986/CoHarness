...
import { meta, oneTurnLog, appendLog } from './legacy-contract.ts'
...
export interface CoordinatorFixture {
  ...
  /**
   * Message the backend raises for a persisted event whose envelope is structurally
   * invalid (a `replace` surfaceOp with negative bounds). Set when the backend validates
   * envelopes at its own read boundary before the coordinator's legacy-shape checks run;
   * omit when the coordinator's `invalid replace surfaceOp` rejection is the first to fire.
   */
  malformedSurfaceOpMessage?: string
  ...
  describe(`PersistenceCoordinator orchestration: ${name}`, () => {
    ...
    it('loads pre-identity message logs into resumable current sessions', async () => {
      ...
      try {
        ...
        await ctx.sessionPersistence.createStored(meta(id, WORK))
    ...
    it('loads pre-react-loop session logs into resumable current sessions', async () => {
      ...
      try {
        ...
        await ctx.sessionPersistence.createStored(meta(flatId, WORK))
        ...
        await ctx.sessionPersistence.createStored(meta(extendedId, WORK))
    ...
    it('rejects malformed persisted message events before returning them', async () => {
      ...
      try {
        ...
        for (const malformed of malformedLegacy) {
          ...
          await ctx.sessionPersistence.createStored(meta(malformedId, WORK))
        ...
        await ctx.sessionPersistence.createStored(meta(malformedReplacementId, WORK))
        ...
        await expect(ctx.sessionPersistence.inspect(malformedReplacementId))
          .rejects.toThrow(fix.malformedSurfaceOpMessage ?? 'invalid replace surfaceOp')
        ...
        await ctx.sessionPersistence.createStored(meta(pluginId, WORK))
        ...
        for (const type of ['user/message', 'assistant/message'] as const) {
          ...
          await ctx.sessionPersistence.createStored(meta(missingContentId, WORK))
    ...
    it('append snapshots the batch: mutating the caller array/events after the call is ignored', async () => {
      ...
      try {
        ...
        await ctx.sessionPersistence.createStored(m)
    ...
    it('session disposal drains buffered events before retiring ownership', async () => {
      ...
      try {
        ...
        await vi.waitFor(async () => {
          expect((await ctx.sessionPersistence.listHeaders()).map(meta => meta.id)).toContain(SessionId('buffered'))
        })
    ...
    it('a live session claims cursor-0 ownerless state created via the public API and persists its seed', async () => {
      ...
      try {
        ...
        await ctx.sessionPersistence.createStored(meta('lazy-claim', WORK))
    ...
    it('a fresh session reusing a previously-loaded id is rejected (ownerless guard)', async () => {
      ...
      try {
        ...
        await ctx.sessionPersistence.createStored(meta('preview', WORK))
    ...
    it('a live session whose seed matches the loaded prefix claims ownerless state and persists the suffix', async () => {
      ...
      try {
        ...
        await ctx.sessionPersistence.createStored(storedMeta)
    ...
    it('a live session at a DIFFERENT cwd cannot claim cursor-0 ownerless state (cwd scope)', async () => {
      ...
      try {
        ...
        await ctx.sessionPersistence.createStored(meta('wrong-cwd-claim', OTHER))
    ...
    it('a live session cannot claim ownerless state with a different inherited cut', async () => {
      ...
      try {
        ...
        // The constructor pins the claimed cut to the full constructor seed,
        // so the collision is exercised by seed length alone (stored cut is 0).
        const seed = oneTurnLog()
    ...
    it('a live session cannot adopt a stored prefix with a different inherited cut', async () => {
      ...
      const first = await freshCtx(fix)
      try {
        await first.ctx.sessionPersistence.createStored(
          { ...meta(id, WORK), isSeeded: true },
          SessionLogOffset(0),
        )
    ...
    it('a live session at a DIFFERENT cwd cannot claim loaded-prefix ownerless state (cwd scope)', async () => {
      ...
      try {
        ...
        await ctx.sessionPersistence.createStored(meta('wrong-cwd-load', OTHER))
    ...
    it('a no-cwd ownerless state cannot be claimed by a live session WITH a cwd (cwd scope, undefined side)', async () => {
      ...
      try {
        ...
        await ctx.sessionPersistence.createStored(meta('no-cwd-state'))
    ...
    it('append adopts a storage-only session (fresh instance) and continues the seq', async () => {
      ...
      try {
        ...
        await first.ctx.sessionPersistence.createStored(m)
    ...
    it('append of an empty batch is a no-op (stays lazy)', async () => {
      ...
      try {
        ...
        expect((await ctx.sessionPersistence.listHeaders()).map(h => h.id)).not.toContain(m.id)
      } finally {
        await fiber.dispose()
        await fix.cleanup()
      }
    ...
    it('create rejects a duplicate id (in memory and on a persisted log)', async () => {
      ...
      try {
        ...
        await expect(first.ctx.sessionPersistence.createStored(m)).rejects.toThrow(/already exists in this backend/)
        await first.ctx.sessionPersistence.append(m.id, oneTurnLog())
      } finally {
        await first.fiber.dispose()
      }
      ...
      const second = await freshCtx(fix)
      try {
        await expect(second.ctx.sessionPersistence.createStored(meta('dup', WORK)))
    ...
    it('rejects a newer format version on load, naming the upgrade direction', async () => {
      ...
      const { ctx, fiber } = await freshCtx(fix)
      try {
        const m = { version: 99 as unknown as SessionHeader['version'], id: SessionId('v99'), createdAt: 1, cwd: WORK, isSeeded: false }
        await ctx.sessionPersistence.createStored(m)
    ...
    it('rejects an unsupported older format version', async () => {
      const fix = await makeFixture()
      const { ctx, fiber } = await freshCtx(fix)
      try {
        const m = { version: -1 as unknown as SessionHeader['version'], id: SessionId('v-older'), createdAt: 1, cwd: WORK, isSeeded: false }
        await ctx.sessionPersistence.createStored(m)
        await ctx.sessionPersistence.append(m.id, oneTurnLog())
        const failure = await ctx.sessionPersistence.load(m.id).then(() => undefined, (error: unknown) => error as Error)
        expect(failure?.name).toBe('SessionFormatUnsupportedError')
        expect(failure?.message).toMatch(/older than the supported v4.*no upgrade path/)
      } finally {
        await fiber.dispose()
        await fix.cleanup()
      }
    ...
    it('rejects an unknown event type on load unless the event is marked ignorable', async () => {
      ...
      try {
        ...
        await ctx.sessionPersistence.createStored(required)
        ...
        await ctx.sessionPersistence.createStored(skippable)
