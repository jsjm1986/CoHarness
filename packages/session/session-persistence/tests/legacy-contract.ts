  ...
  describe(`SessionPersistence contract: ${name}`, () => {
    it('round-trips a session: create + append → load returns identical meta and byte-identical events', async () => {
      ...
      try {
        ...
        await persistence.createStored(m)
    ...
    it('requires an explicit inherited cut exactly when the header is seeded', async () => {
      ...
      try {
        ...
        await expect(persistence.createStored(seeded))
          .rejects.toThrow('seeded session metadata requires an inherited event count')
        await expect(persistence.createStored(meta('unseeded-nonzero'), SessionLogOffset(1)))
          .rejects.toThrow('unseeded session metadata inherited event count must be 0')

        await persistence.createStored(seeded, SessionLogOffset(0))
    ...
    it('does not materialize a seeded artifact before its complete inherited prefix', async () => {
      ...
      try {
        ...
        await persistence.createStored(seeded, SessionLogOffset(2))
        ...
        expect((await persistence.listHeaders()).map(header => header.id)).not.toContain(seeded.id)
    ...
    it('list() excludes a created-but-never-appended (zero-event) session', async () => {
      const { persistence, dispose } = await make()
      try {
        await persistence.createStored(meta('empty'))
        expect((await persistence.listHeaders()).map(m => m.id)).not.toContain(SessionId('empty'))
    ...
    it('rejects pre-aborted observation reads with the exact cancellation reason', async () => {
      ...
      try {
        ...
        await expect(persistence.listHeaders(controller.signal)).rejects.toBe(reason)
    ...
    it('lists stable lightweight revisions that change after an append', async () => {
      ...
      try {
        ...
        expect((await persistence.listHeaders()).map(x => x.id)).toContain(m.id)
    ...
    it('append rejects non-JSON-serializable event data, naming the event type', async () => {
      ...
      try {
        ...
        for (const [i, bad] of badValues.entries()) {
          ...
          const mi = meta(`s5-${i}`)
          await persistence.createStored(mi)
          const events = [
            {
              type: 'user/message',
              seq: SessionSeq(0),
