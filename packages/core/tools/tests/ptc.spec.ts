...
describe('mode-aware wire contribution', () => {
  it("accepts the canonical 'ptc' spelling with PTC mode behavior", async () => {
...
describe('the sub-dispatch scheduler (native concurrency contract)', () => {
  ...
  it('bounds outstanding concurrent sub-calls and recovers the slot after settlement', async () => {
    const { ctx, runtime } = await setup({ mode: 'ptc', maxPendingSubCalls: 2 })
    const gated = registerGated(ctx, 'safe_read', true)
    runtime.behavior = async (request) => {
      const tools = request.bindings[0]!.functions
