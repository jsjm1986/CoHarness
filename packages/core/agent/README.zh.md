...
带作用域的注册接口：`Agent.ctx` 是 agent 的作用域上下文（`dsh-scope`，键 = 该 agent）。通过它注册工具／段／变量／监听器，只对该 agent 生效，并在 dispose（资源释放）时全部撤销。`agentEvents(ctx, agent)` 是普通 agent 主体操作的融合分发器（一次完成载体 + 注入主体）；其通知 mode 会调用每个监听器，并同时收容同步抛出和返回 Promise 的拒绝。注册表生命周期复用一个稳定路由载体。`assembleContextFor(agent)` 构建按 agent 的组装上下文（同时包含 `agent` + `scope`）。`installModelSelection(agentCtx, selection)` 在提示词组装期间快照可变的提供方／模型／推理强度选择，将路由应用到提示词变量，并将完整目标应用到一个步骤的请求路由；如果没有选定推理强度，则会清除继承的推理强度，使该目标使用适配器／提供方默认值。`CreateAgentOptions.setup(agentCtx, agent)` 和 `ResumeAgentOptions.setup(agentCtx, agent)` 在新建或恢复的 agent 尚未发布时组合其带作用域的世界。子 agent 通过 setup 的显式 `agent` 参数传入，Cordis Context 不再提供隐式 `agent` 属性。Setup 是受信任、仅用于组合的同进程代码：只有创建完成后才能驱动 agent。
...
`AgentLoop` 在发起方边界内运行每个具体驱动器的完整生命周期。并发驱动器彼此隔离：子驱动器的 continuation 携带子 agent，而 `withInitiator()` 返回后，父 continuation 立即重新取得父 agent；drain 跟踪持续到子驱动器的 Promise 结算。创建、持久化加载和未发布 setup 位于子边界之外，因此由父 agent 发起的 setup 会继承父 agent，而 setup 的显式 `agent` 参数标识子 agent。
...
可选的 `dsh-agent-loop` projection 会把这些轮次与步骤边界发布为 host-only 的 `turnBoundary` 状态，供按索引的授权读取使用；Agent 接口仍独立于该 loop 包。
...
- `agent.inbox`：结构化 `Inbox` 接口，承载持久 `agent/inbox/spliced` 事件。`nextTurn` 与 `nextStep` 暴露待处理的 `UserMessage` 值。`append`、`prepend`、`replace`、`remove`、`clear` 与 `splice` 用于变更队列；`replace(messageId, newMessage)` 与 `remove(messageId)` 通过 `MessageId` 跨两份列表定位待处理消息。替换可以改变标识，并先将旧消息作为 discarded 发布，再将新消息作为 inserted 发布。普通删除和 `clear()` 都是持久取消，并发出 `agent/inbox/discarded`。在步骤边界，循环的内部实现通过纯删除 splice 认领下一个候选批次并发出 `agent/inbox/claimed`；待处理检测与认领是循环内部行为，不属于 `Agent.inbox`。`MessageId` 是唯一的入队项标识，在消息待处理期间必须保持唯一。
