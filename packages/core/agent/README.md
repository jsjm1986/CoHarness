...
The scoped-registration surface: `Agent.ctx` is the agent's scope context (`dsh-scope`, key = the agent) — register tools/sections/variables/listeners through it for that agent alone, all unwound on disposal. `agentEvents(ctx, agent)` is the fused dispatcher for ordinary agent-subject operations (carrier + injected subject in one move); its notification mode invokes every listener and contains both synchronous throws and returned-promise rejections. The registry lifecycle pair reuses one stable routing carrier. `assembleContextFor(agent)` builds the per-agent assembly context (`agent` + `scope` together). `installModelSelection(agentCtx, selection)` snapshots a mutable provider/model/reasoning-effort selection during prompt assembly, applies its provider and model to prompt variables, and applies the complete selection to request routing for one step; an absent selected effort clears an inherited effort so adapter/provider defaults apply. `CreateAgentOptions.setup(agentCtx, agent)` and `ResumeAgentOptions.setup(agentCtx, agent)` compose a fresh or resumed agent's scoped world while both objects remain unpublished. The child is an explicit setup argument; the Cordis Context no longer exposes an ambient `agent` property. Setup is trusted, composition-only same-process code: drive the agent only after creation resolves.
...
- `ctx.agents.register(agent: Agent): ReturnType<Context['effect']>` — record an **already-constructed** agent with source `startup`. Returns the exact awaitable Cordis effect disposer: retain it, `await registration` before using the agent, and `await registration()` to dispose. Initialization failure rolls back the entry. Disposed with the calling fiber; yield the exact effect, not a wrapper, when nesting ordered teardown.
- Advanced ordered lifecycle: `enter(agent, owner): () => void` enforces `agent.id === agent.session.id`, performs the authoritative ID collision check, and inserts without announcing; `owner` explicitly records the live creator-agent relation (or `undefined` for a root), independently of durable session lineage. `await announce(agent, source, signal?)` runs serial `agent/created` initialization exactly once; source is required and the optional cancellation signal is passed through. A detach requested during initialization is deferred until dispatch settles, including rejection. Every detach checks the captured entry object, so a stale capability cannot delete a later same-ID replacement. The async factory owns rollback using this split; ordinary plugins use `register()`.
...
`AgentLoop` runs each concrete driver's complete lifetime inside an initiator boundary. Concurrent drivers remain isolated: a child driver's continuations carry the child, while the parent continuation regains the parent as soon as `withInitiator()` returns; drain tracking continues until the child driver's Promise settles. Creation, persistence load, and unpublished setup remain outside the child's boundary, so setup initiated by a parent inherits the parent while its explicit `agent` argument identifies the child.
...
`agent/created` runs after scoped setup and after both registry entries exist. It is awaited serial initialization: listeners must return `undefined` or `Promise<undefined>`, and a throw or rejection fails creation and skips later listeners. The payload carries the startup source and optional initialization cancellation signal. Listeners may inject startup context but must not await `agent.whenIdle()` or their own owner's disposal, since queued work and teardown wait for initialization. `agent/session-start` remains a non-vetoing notification for staged migration; do not register the same initializer on both events.

`agent/disposed` always means the exact agent has left the registry. AgentLoop emits it after its driver is quiescent, while ordered teardown may still be detaching the session and unwinding the scope; custom agents registered directly own any stronger driver-ordering contract themselves.
...
The optional `dsh-agent-loop` projection publishes those turn and step boundaries as the host-only `turnBoundary` state for indexed authority reads; the Agent interface remains independent of that loop package.
...
- `agent.inbox` — the structural `Inbox` interface over durable `agent/inbox/spliced` events. `nextTurn` and `nextStep` expose pending `UserMessage` values. `append`, `prepend`, `replace`, `remove`, `clear`, and `splice` mutate them; `replace(messageId, newMessage)` and `remove(messageId)` locate the pending message across both lists. Replacement may change identity and publishes the old message as discarded followed by the new message as inserted. Ordinary removals and `clear()` are durable cancellations and emit `agent/inbox/discarded`. At a step boundary, the loop's internal implementation claims the next proposed batch through pure deletion splices and emits `agent/inbox/claimed`; pending detection and claiming are loop-internal, not part of `Agent.inbox`. `MessageId` is the only occurrence identity and must remain unique while pending.
...
- **`agent/session-start` cannot gate startup** — it remains a synchronous, veto-less migration notification. Awaited startup initialization belongs in `agent/created`; unpublished composition belongs in factory `setup(agentCtx, agent)`.
