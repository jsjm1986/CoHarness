...
export class ReactLoopAgent implements Agent {
  ...
  private async step(decision: Extract<PreparedStep, { kind: 'enter' }>): Promise<StepEndReason | null> {
    ...
    // Reserve the system surface node before user messages enter the surface.
    // Reconciliation against the prepared route happens in the retry loop; an
    // unchanged prompt produces no duplicate event.
    for (const commit of this.systemPrompt.reserve(renderedPrompt)) {
      this.session.append('system/message', { turn, step, message: commit.message }, commit.intent)
    }
    // The entered batch commits before request preparation: a failed or
    // vetoed admission still leaves the accepted input durable.
