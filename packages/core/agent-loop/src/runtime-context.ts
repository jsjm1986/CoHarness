...
export class SystemPromptProjection {
  ...
  /**
   * Reserve the first system surface node before entered messages publish.
   * Reconciliation stays with {@link project} once the prepared route's
   * capability is known; a session that already holds a system node needs no
   * pre-admission commit.
   * @param rendered - the fully rendered system prompt.
   * @returns the initial append commit, or an empty list when a node exists.
   */
  reserve(rendered: string): SystemPromptCommit[] {
    if (this.systemNodes().length > 0) return []
