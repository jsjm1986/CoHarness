...
When `ctx.sessionPersistence` is mounted, these asynchronous factory paths acquire one write `SessionHandle` before publication. A fresh create registers its header through the handle; a resume opens the existing id. The handle remains owned by the published lifecycle and closes after the Agent and Session have drained, so a second writer is rejected before publication and the id is reusable after teardown. The synchronous in-memory `ctx.agentLoop.create()` path remains a compatibility convenience and does not claim a persistence handle.
...
When `dsh-session-projection` is mounted, AgentLoop also registers the host-only `turnBoundary` projection. It records the open turn and latest step boundaries for authority readers without adding a second event stream; assemblies without the optional registry keep the same loop lifecycle.
