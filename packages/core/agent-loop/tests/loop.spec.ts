...
describe('agent loop', () => {
  ...
  it('re-wakes a follow-up that arrives in the normal turn-closing microtask', async () => {
    ...
    const ctx = await harness(adapter)
    const agent = await ctx.agentLoop.create(SessionId('closing-followup'), { provider: 'mock', model: 'mock' })
    let sent = false
    ctx.on('session/event', (_session, event) => {
  ...
  it('re-wakes steering that arrives in the normal turn-closing microtask', async () => {
    ...
    const ctx = await harness(adapter)
    const agent = await ctx.agentLoop.create(SessionId('closing-steer'), { provider: 'mock', model: 'mock' })
    let sent = false
    ctx.on('session/event', (_session, event) => {
      if (event.type !== 'turn/end' || sent) return
      sent = true
  ...
  it('appends the complete entered-message batch before dispatching message-entered effects', async () => {
    ...
    const agent = await ctx.agentLoop.create(SessionId('entered-message-batch'), { provider: 'mock', model: 'mock' })
