...
async function inboxAgent(rawId: string, limits?: { maxMessages?: number; maxBytes?: number }): Promise<{
  ctx: Context
  session: Session
  agent: Agent
...
describe('ReactLoopInbox', () => {
  ...
  it('clears both pending lists as durable cancellations', async () => {
    const { ctx, session, agent, inbox } = await inboxAgent('clear-inbox')
    const discarded: UserMessage[] = []
    ctx.on('agent/inbox/discarded', ({ message }) => void discarded.push(message))
  ...
  it('enforces count and UTF-8 byte quotas across both targets', async () => {
    const { agent } = await inboxAgent('bounded-inbox', { maxMessages: 2, maxBytes: 300 })
    ...
    agent.inbox.append('next-step', second)
    expect(() => { agent.inbox.append('next-turn', third) }).toThrow('inbox message limit reached')
    expect(agent.inbox.nextTurn).toEqual([first])
    expect(agent.inbox.nextStep).toEqual([second])

    const { agent: tiny } = await inboxAgent('tiny-inbox', { maxBytes: 1 })
    expect(() => { tiny.inbox.append('next-turn', first) }).toThrow('inbox byte limit reached')
  })

  it('keeps identity lookup correct after prepend, remove, and replacement', async () => {
    const { agent, inbox } = await inboxAgent('indexed-inbox')
    const first = createUserMessage({ content: [{ type: 'text', text: 'first' }], source: { kind: 'user' } })
    const second = createUserMessage({ content: [{ type: 'text', text: 'second' }], source: { kind: 'user' } })
    const third = createUserMessage({ content: [{ type: 'text', text: 'third' }], source: { kind: 'user' } })
    agent.inbox.append('next-turn', first)
    agent.inbox.append('next-turn', second)
    agent.inbox.prepend('next-turn', third)
    expect(agent.inbox.remove(first.id)).toBe(true)
    expect(agent.inbox.replace(second.id, first)).toBe(true)
    expect(agent.inbox.nextTurn.map(message => message.id)).toEqual([third.id, first.id])
    expect(agent.inbox.remove(third.id)).toBe(true)
    expect(agent.inbox.remove(first.id)).toBe(true)
    expect(inbox.hasPending).toBe(false)
  })
})
