...
import * as llmIndex from '@deepseek-ai/dsh-llm'
...
describe('loop-owned request freezing', () => {
  it('adopts restored identities and proves frozen requests at dispatch', async () => {
    ...
    const freeze = vi.spyOn(llmIndex, 'deepFreeze')
    ...
    // Restore deep-freezes every adopted event graph at ingest; the loop's
    // WeakSet proof below is what skips re-traversal on the next request.
    expect(Object.isFrozen(userEvent.data.content)).toBe(true)
    expect(Object.isFrozen(assistantEvent.data.message)).toBe(true)
    ...
    expect(Object.isFrozen(userEvent)).toBe(true)
    expect(Object.isFrozen(assistantEvent.data)).toBe(true)
    expect(Object.isFrozen(assistantEvent.data.stream)).toBe(true)
    before.pop()
  ...
  it('retries freezing an identity whose previous traversal failed', async () => {
    ...
    const realFreeze = llmIndex.deepFreeze
    let traversals = 0
    vi.spyOn(llmIndex, 'deepFreeze').mockImplementation((value) => {
