 ...
 * reasoning to stderr when `COHARNESS_HEADLESS_PROGRESS` opts in, flushes its
 * Session, prints the final assistant text to stdout, and exits. With `--json`
 * it projects the run as newline-delimited events instead of the final text.
 *
...
async function run(ctx: Context, config: Config, io: HeadlessIo): Promise<void> {
  ...
  const stopReasoning = projection === undefined && config.progress === true
