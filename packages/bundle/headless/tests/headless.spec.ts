...
async function bench(script: Script): Promise<{
  ...
  return {
    ...
    run: async () => {
      ...
      apply(ctx, { task: 'do the thing', progress: script.progress ?? false })
...
describe('headless runner', () => {
  ...
  it('reports a direct Agent creation failure', async () => {
    ...
    apply(ctx, { task: 't', progress: false })
  ...
  it('fails loud without the launcher-provided exit request', () => {
    const ctx = new Context()
    expect(() => { apply(ctx, { task: 't', progress: false }) }).toThrow('must provide ctx.appExit')
  })

  it('validates config: the task is required', () => {
    expect(() => new Config({} as never)).toThrow()
    expect(new Config({ task: 'x', progress: false })).toEqual({ task: 'x', progress: false })
  })
})
