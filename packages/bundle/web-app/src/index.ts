...
function webSurfacePrompt(webUrl: string): string {
  const updateContract = 'Client-plugin HMR is disabled by default to avoid a production file poll. Client-plugin changes reload without a refresh only when '
    + 'the host is launched with `DSH_CLIENT_HMR=1` and `pnpm run dev:web` is also running from this same checkout to rebuild their bundles; verify both before promising automatic updates. '
