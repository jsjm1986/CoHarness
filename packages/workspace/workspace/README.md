...
- `ctx.workspaceRegistry.create(path, title?)` — canonicalizes `path` through the runtime filesystem, rejects a nonexistent or non-directory path, creates at most one record per canonical path, and prepends a new record to durable workspace order. Repeated calls for that path return the existing workspace without changing its title; different paths may share a display title.
- `ctx.workspaceRegistry.get(id)` / `list()` / `resolveByPath(path)` — cache-served lookups. `list()` is synchronous and follows durable registry order; `resolveByPath` is async because it applies the same provider-owned path identity and rejects a missing path rather than creating it.
...
- `ctx.workspaceRegistry.archiveSession(id)` / `restoreSession(id)` / `archivedSessionIds` — the registry-global archive set, layered over workspace accounting: an archived session disappears from grouping surfaces but keeps its session log and its `sessionIds` slot. `archivedEntries()` also reports each entry's persisted root lineage and retained Workspace placement. The versioned `archiveSnapshot()` and `workspace/archive-changed` event let Gateway adapters reconcile restore operations without allowing stale carriers to resurrect a row. The revision is a non-negative safe integer; a mutation at the maximum rejects instead of repeating a value. State written before the archive fields existed defaults to an empty set and revision zero.
...
- `Workspace.status()` — uncached directory check, `'ok' | 'missing-dir'`; a missing, inaccessible, or redirected directory never mutates the record.

`storageDomain`, `sessionPersistence`, and `fs` are required startup dependencies. An unavailable peer leaves the plugin pending and cannot commit an empty initialized marker. On the first successful start, the registry calls `SessionPersistence.listHeaders()` and uses only header `id`, `cwd`, and `createdAt` to group valid historical directories and persist initial order; it never reads event bodies. The initialized marker is written last, so partial bootstrap writes are reused safely after restart. Later cwd-only sessions remain Ungrouped.
...
Path resolution, Session attachment, header grouping, and status checks all use the same filesystem provider. Missing paths reject with `FS_NOT_FOUND`; remote errors never fall back to a Host directory. Each registry and its storage belong to one fixed execution target.
