...
- `ctx.workspaceRegistry.create(path, title?)`：通过运行节点的文件系统规范化 `path`，拒绝不存在或非目录的路径，每个规范路径最多创建一条记录，并将新记录前置到持久 workspace 顺序。对同一路径重复调用会返回现有 workspace，且不改变其标题；不同路径可以共用显示标题。
- `ctx.workspaceRegistry.get(id)`/`list()`/`resolveByPath(path)`：由缓存提供的查找。`list()` 为同步操作，并遵循持久注册表顺序；`resolveByPath` 为异步操作，因为它采用相同的提供方路径标识，并会拒绝缺失路径，而不是创建路径。
...
- `ctx.workspaceRegistry.archiveSession(id)`／`restoreSession(id)`／`archivedSessionIds`：覆盖在 workspace 记账之上的注册表级全局归档集合：被归档的会话从各分组视图中消失，但其会话日志和 `sessionIds` 席位保持不变。`archivedEntries()` 同时报告每条记录持久化的根会话谱系和保留的 Workspace 位置。带版本的 `archiveSnapshot()` 与 `workspace/archive-changed` 事件让 Gateway 适配器能够对账恢复操作，并阻止旧载体重新显示错误状态。revision 是非负安全整数；达到最大值时，变更会拒绝，而不会重复修订值。归档字段出现前写入的状态默认为空集合、revision 为零。
...
- `Workspace.status()`：未缓存的目录检查，返回 `'ok' | 'missing-dir'`；目录缺失、不可访问或被重定向均不会改动记录。

`storageDomain`、`sessionPersistence` 和 `fs` 是启动必需依赖。任一依赖服务不可用时，插件保持待处理，且不能提交空的已初始化标记。首次成功启动时，注册表调用 `SessionPersistence.listHeaders()`，仅使用头部 `id`、`cwd` 和 `createdAt` 对有效历史目录分组并持久化初始顺序；它绝不读取事件正文。已初始化标记最后写入，因此重启后可安全复用引导初始化期间的部分写入。后续仅能通过 cwd 识别的会话仍属于 Ungrouped。
...
路径解析、会话挂接、头部分组和状态检查共用同一个文件系统提供方。缺失路径以 `FS_NOT_FOUND` 拒绝；远端错误绝不回退读取 Host 目录。每个注册表及其存储属于一个固定执行目标。
