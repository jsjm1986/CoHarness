...
/** Create a Web writable that does not attach per-connection listeners to stdout. */
function processStdoutWritable(): WritableStream<Uint8Array> {
...
/** Create a cancellable Web readable that removes every stdin listener on teardown. */
function processStdinReadable(): ReadableStream<Uint8Array> {
  let cleanup = (): void => {}
...
export function apply(ctx: Context, config: AcpConfig): void {
  ...
  const ownsProcessStreams = config.stream === undefined
  const stream: Stream = config.stream ?? ndJsonStream(
    processStdoutWritable(),
    processStdinReadable(),
  )
  ...
  const quiesce = (): Promise<void> => {
    ...
    quiescing = (async () => {
      ...
      // Cancel the SDK's stdio reader after session responses and diagnostics
      // have settled. A caller-provided stream belongs to that caller, so its
      // connection remains open until the caller closes the transport.
      if (ownsProcessStreams) connection.close()
