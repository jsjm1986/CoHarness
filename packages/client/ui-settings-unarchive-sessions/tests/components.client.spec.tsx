...
function sessionState(
  ...
  return {
    ...
    archivedById: Object.fromEntries(archived.map(session => [session.id, session])),
    current: undefined,
    phase,
    subagentsByParent: {},
    jobsBySession: {},
...
function workspaceState(archivedSessionIds: readonly string[], items: readonly WorkspaceView[] = []): WorkspaceListState {
  return {
    items,
...
function props(options: {
  ...
  return {
    ...
    useWorkspaces: ((select: (state: WorkspaceListState) => unknown) => select(options.workspaces)),
...
function twoRows(): ArchivedSessionsSectionProps {
  return props({
    ...
    workspaces: workspaceState(['older', 'newer'], [workspace('Project', ['older', 'newer'])]),
  })
}
...
describe('ArchivedSessionsSection', () => {
  ...
  it('labels a session outside every Workspace and hides an entry whose session is gone', () => {
    render(<ArchivedSessionsSection {...props({
      ...
      workspaces: workspaceState(['gone', 'loose']),
    })} />)
  ...
  it('reads sessions until the list arrives, then reports an empty archive', () => {
    ...
    render(<ArchivedSessionsSection {...props({ sessions: sessionState([]), workspaces: workspaceState([]) })} />)
    expect(screen.getByText(en.empty)).toBeTruthy()
    expect(screen.queryByRole('listitem')).toBeNull()
  })

  it('lists an archived session whose summary survives only in the archive view', () => {
    render(<ArchivedSessionsSection {...props({
      sessions: sessionState([], 'ready', [summary('cold', 'Cold session', Date.now())]),
      workspaces: workspaceState(['cold'], [workspace('Vault', ['cold'])]),
    })} />)

    expect(screen.getByText('Cold session')).toBeTruthy()
    expect(screen.getByText('Vault · now')).toBeTruthy()
    expect(screen.queryByText(en.unavailable)).toBeNull()
  })

  it('does not report an empty archive while every entry lacks a Session summary', () => {
    render(<ArchivedSessionsSection {...props({
      sessions: sessionState([]),
      workspaces: workspaceState(['gone', 'vanished']),
    })} />)
  ...
  it('unarchives the clicked row and keeps a rejection as a console diagnostic', async () => {
    ...
    render(<ArchivedSessionsSection {...props({
      ...
      workspaces: workspaceState(['older']),
