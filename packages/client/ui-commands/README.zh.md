...
动作只消费仍然有效的触发词，保留已附加的图片。草稿变化或 Session 退出后不会调用动作。动作本身不提交内容，这与选项弹窗及 Host 命令不同。

## 弹层可用性

弹层选项可以禁用并说明原因，指针和键盘选择都会拒绝这些选项。业务 spec 可以订阅已捕获选项的失效通知。该订阅仅在自身弹层打开时存在，也只关闭自身弹层；关闭、替换、成功选择或作用域释放都会清理订阅。重新打开时获取最新选项，不建立另一套目录缓存。
