...
侧边栏外壳插件：负责品牌行、New Session 操作、布局持有的折叠控件、可感知滚动的区域 seat，以及带紧凑操作组的底部 Settings seat。[ui-workspace](../ui-workspace/README.zh.md) 持有渲染到 `sidebar.workspaces` 的 Workspace 与 Session 浏览器；本包既不派生其中的行，也不持有其视图偏好。折叠到布局拥有的 56px 轨道仍属于本地呈现行为。约定：[slot 系统标准](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.zh.md)。

展开的品牌行把 `sidebar.brand.mark` 与 `sidebar.brand.name` 渲染为两个独立的 single slot，收起轨道则渲染同一个 mark slot。没有占位者时，外壳使用 CoHarness 标记，以及带有构建期 7 位 `DSH_CLIENT_COMMIT_HASH` 徽标的 `CoHarness` 标签。部署包可以单独替换任一值，而无须替换 New Session 控件或轨道几何；声明感知的 `slots.inject()` 让这种包无论先于还是后于侧边栏激活都能生效。
...
`SidebarRootComponentProps` 组合布局 owner share、全局 `useSessions` 和 `useWorkspaces` 钩子、已声明的品牌、`sidebar.workspaces`、`sidebar.settings` 与 `sidebar.settings.action` 子 slot，以及注入的 `startSession` 与侧边栏切换回调。这里没有插件 store。
...
页脚承载 `sidebar.settings`：侧边栏渲染固定在底部的布局 slot，并共享其栏状态（`wide`）；ui-settings 在此注册触发行和设置面板。`sidebar.settings.action` 会在展开行中把紧凑操作放到触发器右侧，在轨道中放到触发器下方。页脚控件通过继承的 36px 行高和统一图标锚点保持对齐；当前 scope 使用安静的上下文行，文档保持扁平，Settings 以分隔线开始独立的账户区。旁侧操作各自拥有图标、工具提示和无障碍名称；粗指针与紧凑布局仍保留 44px 触控热区。
