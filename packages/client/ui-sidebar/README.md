...
The foot is the `sidebar.settings` seat: the sidebar renders only the bottom-pinned layout slot and shares its column state (`wide`); ui-settings registers the trigger row and settings panel there. Footer controls use an inherited 36px row rhythm and shared icon anchor; the active scope is a quiet context row, Documents and account actions stay flat, and Settings starts a separated account group. Sign out is muted until hover or focus exposes its destructive affordance, while coarse and compact layouts retain the 44px touch target.
