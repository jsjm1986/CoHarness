...
The dialog is a full-height workbench on desktop and a full-screen compact workbench on phones. Desktop keeps the scope rail and inline actions; compact layouts use a current-scope trigger, a bottom scope sheet, and a More sheet for filters and secondary operations. The current folder is represented by a compact breadcrumb with an up action, while the document list remains the only main scrollport. Upload is the primary compact toolbar action; uploads and desktop drops land in the current folder. A caption states personal vs project visibility and the filtered document count. Copy uses a metadata plan, target-folder selector, and per-file retry controls.
