...
import { workspacePathForResource, workspaceResourceAddress } from '@deepseek-ai/dsh-client-runtime/client'
...
export function ReviewTab({
  ...
  if (coordinates === undefined || coordinates.sessionId !== sessionId) throw new Error(`ui-deliverables: not a review address "${tab.contentId}"`)
  ...
  useEffect(() => {
    if (tab.visible && summary === undefined) void loadChangesSummary(sessionId, seq)
  }, [tab.visible, summary, sessionId, seq, loadChangesSummary])
  useEffect(() => {
  ...
  useEffect(() => {
    if (tab.visible && file !== undefined && diffState === undefined) void loadChangesDiff(sessionId, seq, index)
  }, [tab.visible, file, diffState, sessionId, seq, index, loadChangesDiff])
        ...
        <span className={css.tools}>
          ...
          {file !== undefined && <Tooltip label={t('review.openFile')} side="bottom" delayMs={500}>
            <button type="button" className={css.tool} aria-label={t('review.openFileAria', { name: file.display })} data-review-tool="open-file"
              onClick={() => {
                tab.actions.openResource(workspaceResourceAddress(sessionId, workspacePathForResource(cwd, file.path)))
              }}><IconCodeOutline16 /></button>
          </Tooltip>}
          {file !== undefined && native && <Tooltip label={t(phase === 'error' ? 'diff.openNativeError' : 'diff.openNative')} side="bottom" delayMs={500}>
            <button type="button" className={css.tool} disabled={phase === 'opening'} data-review-tool="open-native"
              ...
              onClick={() => { void openChanged(sessionId, seq, index, file.path) }}><IconRightUpOutline16 /></button>
          </Tooltip>}
        </span>
      </div>
