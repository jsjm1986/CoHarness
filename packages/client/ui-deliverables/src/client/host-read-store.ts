...
export class HostReadStore<T> {
  ...
  constructor(
    private readonly policy: HostReadPolicy<T>,
    private readonly fetchRecord: (url: string, signal: AbortSignal) => Promise<Response>,
  ) {}
  ...
  private async read(url: string, signal: AbortSignal): Promise<void> {
    let next: T
    try {
      next = await this.policy.decode(await this.fetchRecord(url, signal))
    } catch {
