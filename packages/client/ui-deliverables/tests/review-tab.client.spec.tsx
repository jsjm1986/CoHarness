...
const summary: ChangesSummary = {
  ...
  files: [
    ...
    { path: 'out/big.bin', display: 'out/big.bin', added: 0, deleted: 0, oversized: true },
...
describe('review addresses', () => {
  it('round-trips coordinates and titles the tab by the turn', () => {
    ...
    expect(definition.resourceSession?.(ADDRESS)).toBe(SESSION)
    expect(definition.resourceSession?.('invalid')).toBeUndefined()
    for (const bad of [
...
describe('ReviewTab', () => {
  ...
  it('opens the whole file in the sidebar and the native open only with a desktop', () => {
    ...
    expect(tabActions.openResource).toHaveBeenCalledWith(workspaceResourceAddress(SESSION, 'out/big.bin'))
    fireEvent.click(view.getByRole('button', { name: 'Open out/big.bin in default app' }))
    expect(injected.openChanged).toHaveBeenCalledWith('viewed', 5, 1, 'out/big.bin')
