...
// consumes it — the keyed ReadImageRow composing the shared read-family assembly
// with the image card as its collapsed-by-default expanded body. Also pins the
// keyed 'read_image' toolview registration and the text that stays readable when
// the attachment presentation plugin renders nothing.
//
...
// renders through the chat node's slot-backed `renderMessageImages`, and an empty
...
const settled = (over?: Partial<ToolResultNode>): ToolResultNode => ({
  ...
  isError: false,
  meta: imageMeta(), callView: null, resultView: null, subCalls: [], ...over,
} as unknown as ToolResultNode)

/**
 * A renderMessageImages stub standing in for the attachment presentation
 * plugin's gallery. The owner is the real `MessageImagesOwnerProps` minus the
 * loader — `MessageImageSource` is a union of a durable attachment arm and a
 * submission-echo preview arm, so the stub renders both.
 */
const stubRenderMessageImages = (): RenderMessageImages => (
  vi.fn((owner: Omit<MessageImagesOwnerProps, 'loadImage'>) => (
    <div data-images>
...
describe('imageCardModel', () => {
  ...
  it('declines a root call with missing or malformed metadata, and falls back for a nested one', () => {
    ...
    const nested = imageCardModel(settled({ meta: undefined }), undefined, undefined, true)
  ...
  it('declines when no content block matches the image envelope', () => {
    // The derivation matches its own envelope by shape: a content block another
  ...
  it('declines a running call and an error result, and derives a nested call from its own path', () => {
    ...
    const withMeta = imageCardModel(settled({ meta: { path: 'shots/persisted.png' } }), undefined, undefined, true)
...
describe('ReadImageRow keyed toolview', () => {
  ...
  it('classifies read_image into the read family so the row keeps its promises', () => {
    ...
    // (the read variant's FILE_PATH variants cover only read/write/edit), so the
    // openable path the row advertises would never be openable.
    const openFile = vi.fn()
    const view = render(<ReadImageRow {...rowProps(settled(), stubRenderMessageImages())} openFile={openFile} />)
  ...
  it('expands to the image, dispatched through the slot-backed gallery renderer', () => {
    const renderMessageImages = stubRenderMessageImages()
    const view = render(<ReadImageRow {...rowProps(settled(), renderMessageImages)} />)
    expect(view.container.querySelector('[data-images]')).toBeNull()
    toggleRow(view)
    expect(view.container.querySelector('[data-images]')).not.toBeNull()
    expect(renderMessageImages).toHaveBeenLastCalledWith({
      images: [{ attachment: sampleImage }],
      align: 'start',
    })
  ...
  it('never prints the raw attachment object under the picture', () => {
    ...
    const view = render(<ReadImageRow {...rowProps(settled(), stubRenderMessageImages())} />)
    toggleRow(view)
  ...
  it('keeps the result text readable when the gallery renders nothing', () => {
    // The attachment presentation plugin renders nothing in a deployment without
    // it. That must not leave a blank card.
    const emptyGallery = vi.fn(() => null) as unknown as RenderMessageImages
    const view = render(<ReadImageRow {...rowProps(settled(), emptyGallery)} />)
    toggleRow(view)
    expect(emptyGallery).toHaveBeenCalled()
    expect(view.container.querySelector('[data-images]')).toBeNull()
    expect(view.container.textContent).toContain('image/png image, 1496x260 px')
  })

  it('degrades to the text body when no gallery renderer is supplied', () => {
  ...
  it('a running call renders the summary row alone', () => {
    const renderMessageImages = stubRenderMessageImages()
    const view = render(<ReadImageRow {...rowProps(running(), renderMessageImages)} />)
    expect(view.container.querySelector('[data-images]')).toBeNull()
  ...
  it('a refusal renders its error without an image card', () => {
    ...
    } as never), renderMessageImages)} />)
    expect(view.container.querySelector('[data-images]')).toBeNull()
    expect(renderMessageImages).not.toHaveBeenCalled()
    expect(view.container.textContent).toContain('does not declare image input')
  })

  it('a nested call renders the gallery labeled by its own file_path', () => {
    // Nested ptc-dispatch results persist no presentationMeta, so the row
    // supplies `nested` and the label comes from the call's file_path argument.
    const renderMessageImages = stubRenderMessageImages()
    const view = render(<ReadImageRow {...rowProps(settled({ meta: undefined }), renderMessageImages, true)} />)
    toggleRow(view)
    expect(view.container.querySelector('[data-images]')).not.toBeNull()
    expect(view.container.textContent).toContain('shots/card.png')
  })
  ...
  it('registers under the read_image key of the keyed toolview slot', () => {
    ...
    expect(registered).toEqual([{ name: 'tool.call.toolview', key: 'read_image', locale: 'conversation' }])
