...
Hot reload for script-loaded client plugins. The Web bundle keeps this row disabled by default so ordinary launches do not run a file poll or SSE route. Enable it with `DSH_CLIENT_HMR=1` together with a rebuild watcher (`pnpm run dev:web`) when developing client bundles.
