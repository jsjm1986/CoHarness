...
function Harness({ cell = CELL, wrapLines = false, onToggleWrapLines }: {
  cell?: TrajectoryCellProps
  wrapLines?: boolean
  onToggleWrapLines?: (wrapped: boolean) => void
}) {
  return <TrajectoryTable
    turns={[{ turn: 1, groups: [{ title: 'Step 1', cells: [cell] }] }]}
    t={t}
    wrapLines={wrapLines} onToggleWrapLines={onToggleWrapLines}
...
describe('PTC trajectory inspector', () => {
  ...
  it('samples the last wrapping choice on open without changing another open inspector', () => {
    ...
    const props = {
      wrapLines: preference,
      onToggleWrapLines: (wrapped: boolean) => { preference = wrapped },
    }
    ...
    second.rerender(<Harness {...props} wrapLines={preference} />)
    ...
    expect(a.getByRole('button', { name: 'Wrap lines' }).getAttribute('aria-pressed')).toBe('true')
    first.rerender(<Harness {...props} wrapLines={preference} />)
