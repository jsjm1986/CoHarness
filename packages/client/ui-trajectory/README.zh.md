...
compact 视口会保留记录的持久选择，同时把事件投影为使用较大正文角色的手机可读流和紧凑时间线条。打开事件检查器时，它会变为带不透明阅读层与共享遮罩的顶部圆角 Sheet，只保留一个摘要滚动位置，并应用安全区内边距和小型拖拽提示，使短屏仍能看到状态和值，同时不改变底层选择状态。

`run_code` 工具记录会打开程序检查器：Code 标签页显示带语言标记的行号源码、切换原始 JSON 参数的控制钮，以及源码、JSON 和捕获输出的复制控件。概述区以紧凑预览形式保留同一组面板，并隐藏仅限检查器的行（如计时来源）。每个打开的检查器在选中记录或激活 Code 标签页时采样持久化的折行偏好（`dsh.trajectory.wrap`），因此切换会写入下一份默认值，而不会重排已打开的检查器。实际耗时开关以同样方式在浏览器范围持久化（`dsh.trajectory.duration`）。

处于舞台的会话正在运行时，共享 Session 窗口会在后台补齐更早页面，记录表不显示更早历史行；下一次舞台进入从有界尾页开始时才恢复该入口。
