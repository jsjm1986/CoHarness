...
- The same sidebar footer exposes **Sign out** only after the Gateway account context is ready. It submits the native `POST /logout` form so Gateway owns token revocation, audit logging, cookie clearing, and the redirect to `/login`. The expanded action is a compact secondary row; its destructive color appears on hover or focus, while the rail and touch layouts preserve their established target sizes.
