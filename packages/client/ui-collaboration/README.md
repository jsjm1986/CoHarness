...
- The same sidebar footer exposes **Sign out** only after the Gateway account context is ready. It submits the native `POST /logout` form so Gateway owns token revocation, audit logging, cookie clearing, and the redirect to `/login`.
