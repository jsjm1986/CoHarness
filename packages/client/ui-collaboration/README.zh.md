...
- 同一个侧栏 footer 会在 Gateway 账户上下文 ready 后显示「退出登录」。它提交原生 `POST /logout` 表单，由 Gateway 负责令牌撤销、审计记录、Cookie 清理以及跳转到 `/login`。展开栏中的操作使用紧凑的次要行，悬停或聚焦时才显示危险色；轨道与触控布局仍保留既有的触控尺寸。
