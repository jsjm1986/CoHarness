...
import type { SettingsMutationScope } from '@deepseek-ai/dsh-client-ui-settings/client'
...
export class SubagentModelSelectionCardController {
   ...
   * @param api - authenticated Host model catalog API.
   */
  constructor(
    private readonly scope: SettingsMutationScope<SubagentModelSelectionSettings>,
  ...
  private toggleModel(key: string): void {
    if (this.disposed || !this.enabled() || this.saving
      || this.scope.getSnapshot().status !== 'ready' || !this.scope.getSnapshot().writable) return
  ...
  private async save(): Promise<void> {
    ...
    if (generation !== this.saveGeneration) return
    const landed = this.scope.getSnapshot().write.status === 'idle'
      && this.currentEnabled() === desiredEnabled && sameRoutes(this.currentRoutes(), desired)
    this.saving = false
    this.failed = !landed
  ...
  private async loadCatalog(): Promise<void> {
    ...
    try {
      ...
      if (result.ok) {
        this.catalogGroups = result.value.groups
        this.catalogPartial = result.value.failures.length > 0
        this.catalogStatus = 'ready'
      } else {
        this.catalogStatus = 'error'
      }
    } catch (_catalogTransportFailure) {
      if (generation !== this.catalogGeneration) return
      this.catalogStatus = 'error'
    }
    this.publish()
