...
分区以选择器开关开头——写入的是命名空间第二个字段 `modeSelectionEnabled`。开关关闭期间，新会话 chip 不再出现，未指名的会话按部署默认值组装；已保存的 `default` 被搁置而非改写，并在选择器重新开启时恢复。两类写入落定后均由宿主名册给出有效默认值，若当前新会话界面已持有一个空白会话，分区会经 chip 自身的 select 通道把它同步到那个确切的会话——已开始与历史会话不受影响。隐藏选择器会同时禁用默认值选择与「创造模式」入口——它们都会宣称一个宿主拒绝生效的效果——而查看、复制、位置与删除保持可用；写入进行中或该命名空间拒绝本浏览器的写入时（只读 provider，或由管理者持有该字段的项目作用域），开关本身也保持禁用。
