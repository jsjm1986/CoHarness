...
function mountBrowser(navigation?: { readonly url?: string }) {
  ...
  return {
    view, remount: renderBody, store, lifetime, injected,
    replaceOccurrence() {
      act(() => {
        lifetime.abort()
        ...
        view.rerender(<BrowserBody {...props} />)
      })
    },
  }
}
...
describe('BrowserBody', () => {
  it('does not reuse an old account URL or sandbox override when a tab id is restored under a new occurrence', async () => {
    const mounted = mountBrowser({ url: 'https://private.example/account-one' })
    await waitFor(() => { expect(mounted.view.container.querySelector('iframe')?.getAttribute('src')).toBe('https://private.example/account-one') })
    fireEvent.click(mounted.view.getByRole('button', { name: zh['sandbox.disable'] }))
    mounted.replaceOccurrence()
    await waitFor(() => { expect(mounted.view.container.querySelector('iframe')).toBeNull() })
    expect(mounted.view.getByRole('textbox')).toHaveProperty('value', '')
    expect(mounted.view.getByRole('button', { name: zh['sandbox.disable'] })).toBeDefined()
    expect(mounted.store.getSnapshot().byTab[TAB]).toBeUndefined()
  })
