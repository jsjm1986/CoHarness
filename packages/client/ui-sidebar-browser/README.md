...
Browse HTTP(S) pages, including loopback services, inside independent right-Sidebar tabs. The current carrier is an iframe with application-managed history in the Web application. The package never injects Electron or Node access into visited content.
...
The shipped Web composition mounts this package. Open **Browser** from the right-Sidebar guide, enter an HTTP(S) URL, or select an HTTP(S) link in Assistant Markdown. A host name without a scheme becomes HTTPS. Public and loopback targets use the same default sandbox. Each guide action or message-link activation creates another Browser tab.

### When to choose it

Choose Browser for a Web page that should remain beside the current Session. Choose [Workspace file preview](../ui-workbench/README.md) for local files, and use the explicit external-browser action when a site refuses iframe embedding or needs browser capabilities this package withholds.
...
Client plugins can open a tab through `ctx.sidebarRight.openSessionTab(sessionId, 'browser', { params: { url } })`. The optional URL passes the same validation as address-bar input before navigation.

Loopback and localhost refer to the visitor’s machine, not the Gateway or an SSH execution target.
...
- [Right Sidebar](../ui-sidebar-right/README.md) — tab composition, navigation, and lifecycle.
- [Workspace file preview](../ui-workbench/README.md) — authorized workspace resources and current file rendering.
- [Scoped sidebar decision](../../../.agents/notes/implemented/architecture/2026-09-23-account-scoped-auxiliary-sidebar.md) — account ownership and resource lifetimes.
...
- In Web, a popup that escapes the sandbox retains its opener and can use that chain to navigate the top-level application. There is no native browser carrier in this package.
...
- Browser history survives body remounts and ordinary page reloads under a verified principal/runtime/Session key. Identity loss clears live tabs without assigning their stored history to a new account. Closing a tab removes its history. Stored URLs are validated again before restoration; sandbox overrides are never persisted.
- Local files are rejected and remain owned by Workspace file preview.
