...
describe('WorkspaceBrowser.module.css list', () => {
  ...
  it('counts the themed scrollbar inside the shell trailing inset', () => {
    ...
    expect(root?.get('--dsh-session-list-scrollbar-width')).toBe('var(--dsh-scrollbar-width)')
  ...
  it('reveals row actions and grows session rows on coarse and compact', () => {
    ...
    expect(css).toContain('var(--dsw-radius-lg)')
    expect(rowsCss).toContain('background: transparent')
    expect(rowsCss).toContain('min-height: 48px')
    expect(rowsCss).toContain('min-height: 44px')
