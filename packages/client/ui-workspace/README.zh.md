...
compact 抽屉会把每个 Workspace 分组呈现为带边框的卡片，并把 Session 行缩进为卡片内的触控目标；行树、复用规则、状态优先级、排序行为和操作回调与桌面浏览器保持一致。这只是移动呈现变化，不会创建第二份 Workspace 或 Session 投影。
...
真实 Workspace 行始终显示其“新建会话”加号，因此创建不会依赖悬停。刚预留的空白草稿可以携带仅客户端使用的 Workspace 提示，在 Host 成员关系到达前也会显示在该分组中，不会进入 wire 或搜索索引。
...
选择 Session 时先加载历史，再切换当前视图。加载失败会保留原视图和草稿，并在浏览区域显示错误；再次选择该行即可重试。被取代的请求不能显示过期错误或选中目标。
