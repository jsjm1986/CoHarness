...
function TabSlot({
  ...
  const lifetime = useRef({ signal, generation: 0 })
  if (lifetime.current.signal !== signal) lifetime.current = { signal, generation: lifetime.current.generation + 1 }
  ...
  return (
    <Fragment key={lifetime.current.generation}>
      {renderSlot(seat, {}, { entryKey: definition?.id ?? tab.kind, fallback, hookContext })}
    </Fragment>
  )
}
