  ...
  return {
    ...
    adopt(sessionId, store) {
      ...
      const sync = (): void => {
        ...
        else controller.tabDomain.forget(sessionId)
      }
...
export class SidebarRightController implements ISidebarRight {
  ...
  /**
   * Open a product page in its explicit adopted Session without using the active pane.
   * @param sessionId - verified owner of the page.
   * @param kind - registered tab kind.
   * @param options - typed navigation and placement.
   */
  openSessionTab<K extends string>(sessionId: SessionId, kind: K, options: SidebarRightOpenTabOptions<K> = {}): void {
    if (!this.adopted.has(sessionId)) throw new Error('Sidebar Session is not adopted')
   ...
   */
  openSessionResource(sessionId: SessionId, address: string, options: SidebarRightOpenResourceOptions = {}): void {
    const actions = this.actionsFor(sessionId)
    if (actions === undefined) throw new Error('Sidebar Session is not available')
    this.placeResource(sessionId, actions, address, options)
  }
  ...
  private placeResource(
    ...
    this.validateResource?.(sessionId, address)
   ...
   * @param sessionId - exact owner, or the mounted owner when omitted.
   * @param expanded - requested visibility.
   */
  setExpandedIn(sessionId: SessionId | undefined, expanded: boolean): void {
    const target = sessionId ?? this.require().sessionId
