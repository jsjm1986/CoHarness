...
export type PinResource = (address: string, signal: AbortSignal, sessionId: SessionId) => void
...
export class TabDomain {
  ...
  sync(sessionId: SessionId, layout: LayoutState): void {
    ...
    for (const tab of Object.values(layout.tabs)) {
      ...
      this.pin(occurrence.navigation.getSnapshot().address, occurrence.signal, sessionId)
  ...
  /** Release records whose Session layout is no longer authorized or adopted.
   * @param sessionId - exact Session whose records are invalidated.
   */
  ...
  /** Release every tab occurrence on plugin teardown. */
  dispose(): void {
