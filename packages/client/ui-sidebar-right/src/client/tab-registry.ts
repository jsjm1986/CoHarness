...
export interface SidebarRightTabDefinition {
  ...
  /** Resolve the owning Session of a custom resource address before restore or open.
   * @param address - address claimed by this registered type.
   * @returns exact Session, or undefined for an invalid address.
   */
  readonly resourceSession?: (address: string) => SessionId | undefined
  /**
...
export class SidebarRightTabRegistry {
  ...
  private refresh(): void {
    ...
    for (const listener of [...this.listeners]) {
      try { listener() } catch (error) { console.error('[ui-sidebar-right] tab registry subscriber failed:', error) }
    }
  }
}
