...
The root-scoped `rightbar` entry renders the explicit target Session through the existing SessionProvider. The selected auxiliary target can differ from the active Workbench pane. Desktop header and compact topbar controls share the same Session store; neither owns a second layout.
...
A tab type registers in two stages, and the shipped guide type goes through exactly the same public path a type from another package does (the workspace file tab is one consumer). Both stages sit inside the type's own `ctx.effect`, so the registration lives exactly as long as the plugin that made it.
