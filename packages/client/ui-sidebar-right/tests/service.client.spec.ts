...
describe('SidebarRightController — a tab\'s own actions', () => {
  it('rejects explicit product navigation after its Session adoption is released', () => {
    const { controller, adopt, instance, titles } = harness()
    const address = 'dsh-resource://file/session/s-test/a.txt'
    expect(() =>{  controller.openSessionResource(SESSION, address) }).toThrow('Session is not available')
    const release = adopt(SESSION, instance)
    controller.openSessionResource(SESSION, address)
  ...
  it('adoption syncs the Tab domain on each commit of that store: the seeded guide is pinned, a closed tab aborted', () => {
    ...
    expect(pin).toHaveBeenCalledWith('sidebar://guide', expect.any(AbortSignal), SESSION)
    ...
    expect(pin).toHaveBeenCalledWith(A_TXT, occurrence.signal, SESSION)
    ...
    expect(pin).toHaveBeenCalledWith(B_TXT, held.signal, SESSION)
