...
async function mountSeat(viewportWidth = 1440, canShow = true, entryCount = 0) {
  ...
  let reference = runtime.sessions.retain(SESSION, { source: 'controllerOperation' })
  runtime.sessions.open(SESSION)
  ...
  const selectSession = (id: SessionId): void => {
    const next = runtime.sessions.retain(id, { source: 'controllerOperation' })
    reference.release()
    reference = next
  ...
  return {
    ...
    open, selectSession, frame, pin, bodies, titles, hooks, view, description,
  }
}
...
describe('RightbarSeat presentation', () => {
  it('remounts every tab body when another verified account restores the same record', async () => {
    const h = await mountSeat()
    act(() => {
      h.description.set({ executionAuthorityRequired: true })
      h.runtime.ctx.projectUiPolicy.setVerifiedAccountId(1)
    })
    const id = h.open('private.txt').id
    const body = () => element(h.view.container, `[data-tab-body="${id}"]`)
    const first = body().dataset.instance
    const signal = h.bodies.get(id)!.tab.signal
    const saved = h.instance.getSnapshot().bySession[SESSION]!
    writeSidebarLayout(JSON.stringify(['account:2', ['personal'], SESSION]), saved)
    act(() => { h.runtime.ctx.projectUiPolicy.setVerifiedAccountId(2) })
    expect(signal.aborted).toBe(true)
    expect(body().dataset.instance).not.toBe(first)
    expect(h.bodies.get(id)!.tab.signal.aborted).toBe(false)
  })

  it('hides with no selected Session and restores its sidebar state on return', async () => {
    const h = await mountSeat()
    h.open('retained.txt')
    const retained = h.layout()
    act(() => { h.runtime.sessions.clear() })
    ...
    act(() => { h.runtime.sessions.open(SESSION) })
