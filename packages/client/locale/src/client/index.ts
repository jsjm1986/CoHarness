...
export class LocaleRuntime {
  ...
  /** Fail a definition whose complete fallback path does not reach English. */
  private assertFallbackChain(start: LocaleId): void {
    ...
    while (current !== undefined) {
      ...
      /* v8 ignore next -- addLanguage validates every fallback before insertion. */
      if (next === undefined) {
  ...
  /** Resolve and cache a language's complete fallback chain. */
  private fallbackChain(start: LocaleId): readonly LocaleId[] {
    ...
    /* v8 ignore next -- every active catalog chain terminates at built-in English. */
   ...
   */
  register<N extends keyof LocaleNamespaceMap & string>(ns: N, dicts: Record<BuiltInLocaleId, LocaleDictOf<N>>): () => void
  /**
 ...
 * registration wins before a language-wide fallback. `window` is the browser
 * test, not `navigator`: Node exposes a global navigator that must not decide
 * the locale for non-browser runs.
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.effect(() => host.subscribe(sync), 'locale: settings row state')
