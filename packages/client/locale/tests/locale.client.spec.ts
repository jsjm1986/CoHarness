...
describe('LocaleRuntime', () => {
  ...
  it('does not change a read-only Host scope', () => {
    const host = stubSettingsScope<LocaleSettings>()
    host.publish({ status: 'ready', writable: false, writableReason: 'deployment' })
    const { svc } = make(host)
    svc.setLocale('en')
    expect(svc.getLocale().active).toBe('zh')
  ...
  it('reconciles an optimistic pick when the Host rejects its write', async () => {
    const host = stubSettingsScope<LocaleSettings>()
    host.publish({
      status: 'ready', writable: true, value: { preference: 'zh' }, revision: 1,
    })
    host.set.mockImplementation(() => {
      host.publish({ write: { status: 'error', code: 'settings-rejected', message: 'read-only' } })
  ...
  it('rejects a fallback cycle after an intermediate language is unloaded', () => {
    const { svc } = make()
