...
import {
  IconBrowseOutline16, IconChevronRightOutline14, IconFolderClose16, IconUserOutline16,
...
function candidateIcon(icon: string | undefined): ReactNode {
  switch (icon) {
    ...
    case 'session': return <IconUserOutline16 />
              ...
              : (
                <Fragment key={group.source}>
                  ...
                  {group.status === 'pending' && group.items.length === 0
                    ? <div role="status" className={css.loading} data-source={group.source}>{t('loading')}</div>
