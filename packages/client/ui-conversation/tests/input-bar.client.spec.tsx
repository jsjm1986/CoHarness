...
import type {
  ComposerAttachment, ComposerAttachmentsOwnerProps, ComposerDocument,
...
describe('image draft rail', () => {
  ...
  it('routes non-images to document intake without applying image limits', () => {
...
describe('document draft rail', () => {
  ...
  it('includes ready document ids in the submitted message', () => {
    ...
    expect((view.getByRole('button', { name: '发送消息' }) as HTMLButtonElement).disabled).toBe(false)
    fireEvent.keyDown(textarea, { key: 'Enter' })
    expect(sink).toHaveBeenCalledWith('请阅读', [], ['document-1'], 'queue', expect.any(AbortSignal))
  })
})
...
describe('running and lock semantics', () => {
  it.each(['queue', 'steer'] as const)('running pointer submission uses the %s preference', (mode) => {
    const { button, sink } = bench({ running: true, busyEnter: mode, draft: 'next message' })
    expect(button.getAttribute('aria-label')).toBe(mode === 'queue' ? '排队发送' : '插话发送')
    fireEvent.click(button)
    expect(sink).toHaveBeenCalledWith('next message', [], [], mode, expect.any(AbortSignal))
  })

  it('a live preference change updates the mounted button before the next click', () => {
    const { button, sink, busyEnter } = bench({ running: true, draft: 'same draft' })
    expect(button.getAttribute('aria-label')).toBe('排队发送')
    act(() => { busyEnter.set('steer') })
    expect(button.getAttribute('aria-label')).toBe('插话发送')
    fireEvent.click(button)
    expect(sink).toHaveBeenCalledWith('same draft', [], [], 'steer', expect.any(AbortSignal))
  })

  it('a command keeps the plain Send label and follows command adjudication', async () => {
    const { button, shell, sink } = bench({ running: true, busyEnter: 'steer', draft: '/goal next' })
    expect(button.getAttribute('aria-label')).toBe('发送消息')
    const submit = vi.fn(() => Promise.resolve({ kind: 'success' as const }))
    act(() => {
      shell.beginCommand({ token: '/goal ', submit }, { start: 0, end: 6, draftRev: shell.snapshot.draftRev })
    })
    expect(button.getAttribute('aria-label')).toBe('发送消息')
    fireEvent.click(button)
    await vi.waitFor(() => { expect(submit).toHaveBeenCalledWith('next', SCTX, []) })
    expect(sink).not.toHaveBeenCalled()
  })

  it('an upload-pending draft keeps plain Send disabled under the Steer preference', () => {
    const { button, sink } = bench({ running: true, busyEnter: 'steer', draft: 'with a file',
      documents: [documentDraft({ status: 'uploading', progress: 0.5 })] })
    expect(button.getAttribute('aria-label')).toBe('发送消息')
  ...
  it('closes the Stop tooltip when settlement turns the same seat into disabled Send', () => {
    vi.useFakeTimers()
    try {
  ...
  it('running switches the primary between Stop and the configured Steer delivery', async () => {
  ...
  it('running continuable child pointer Send follows Steer while Stop stays independent', () => {
    const { button, interruptButton, sink, stop } = bench({ running: true, busyEnter: 'steer', draft: 'child follow-up',
      subagent: { address: { parentSessionId: 'parent' as SessionId, childSessionId: SID, mode: 'continuable' }, parentAvailable: true } })
    expect(button.getAttribute('aria-label')).toBe('插话发送')
    fireEvent.click(button)
    expect(sink).toHaveBeenCalledWith('child follow-up', [], [], 'steer', expect.any(AbortSignal))
    fireEvent.click(interruptButton!)
  ...
  it('idle primary sends and disables on empty draft', () => {
    const { button, sink } = bench({ draft: 'go', busyEnter: 'steer' })
    fireEvent.click(button)
