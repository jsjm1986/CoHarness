...
export class SessionInputShell implements SessionInput {
  ...
  submit(mode: InputSubmitMode = 'queue'): void {
    if (this.snapshot.draft.trim() === '' && (this.imageIds.length > 0 || this.documentIds.length > 0)) {
  ...
  private sinkSerialized(attempt: SubmitAttempt, draft: string, mode: InputSubmitMode): void {
    ...
    if (occurrences.length === 0) {
      this.settleSubmit(
        attempt,
        this.deps.defaultSink(draft.trim(), imageIds, documentIds, mode, attempt.signal),
        imageIds,
        documentIds,
      )
      return
    ...
    void Promise.all(occurrences.map(async (o) => {
      ...
      (parts) => {
        ...
        this.settleSubmit(
          attempt,
          this.deps.defaultSink(out.trim(), imageIds, documentIds, mode, attempt.signal),
          imageIds,
          documentIds,
        )
      },
  ...
  private settleSubmit(
    ...
    pending.then(
      (outcome) => {
        ...
        if (outcome.kind === 'success' && documentIds.length > 0) {
          const submitted = new Set(documentIds)
          this.documentIds = this.documentIds.filter(id => !submitted.has(id))
        }
        this.run(this.core.dispatch({
          type: 'submit-settled',
          attempt,
