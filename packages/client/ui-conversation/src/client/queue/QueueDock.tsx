...
import type { PendingSubmission, SessionId } from '@deepseek-ai/dsh-client-runtime/client'
import {
  IconCheckOutline16, IconChevronDownOutline14, IconChevronUpOutline14, IconCloseOutline16,
  IconEditOutline16, IconQueueOutline14, IconSendOutline14, IconTrashOutline16, Tooltip,
} from '@deepseek-ai/dsh-client-ui-primitives'
import type { MessageImageLoader } from '../contract/slots.ts'
...
export interface QueueDockInjected {
  ...
  /** Resolve one durable queued image through the session authorization scope. */
  loadImage: MessageImageLoader
}

const EMPTY_PENDING_SUBMISSIONS: readonly PendingSubmission[] = []

/** Durable image references carried by one queued row. */
function queueImageRefs(content: QueueRow['content']): ImageAttachmentRef[] {
  return content.flatMap((block) => {
    if (block.type !== 'image') return []
    // Queue frames are wire data; retain a defensive guard for older fixtures
    // and foreign peers that carry an image marker without its reference.
...
/** One durable queued image thumbnail; a failed read keeps an empty placeholder. */
function QueueThumb({ attachment, loadImage, label }: {
  ...
  useEffect(() => {
    ...
    void loadImage(attachment).then(
      ...
      () => { /* Keep the placeholder; the transcript surface owns diagnostics. */ },
    )
    return () => { alive = false }
    ...
    <div className={css.dock} data-queue-dock="">
      <div className={css.panel}>
        ...
        <ul id={listId} className={css.list} hidden={!listVisible}>
                ...
                {editing?.id === row.id
                  ...
                  : (
                    <>
                      ...
                      {row.preview !== '' && <span className={css.preview}>{row.preview}</span>}
                    </>
                  )}
          ...
          {listVisible && pendingQueue.map((submission: PendingSubmission) => (
      ...
      inject: (sessionId: SessionId): QueueDockInjected => {
        ...
        return {
          ...
          loadImage: Object.assign(
            (attachment: ImageAttachmentRef) => conversation.resolveImage(sessionId, attachment),
            { peek: (attachment: ImageAttachmentRef) => conversation.peekImage(sessionId, attachment) },
          ),
        }
      },
    }, QueueDock))
