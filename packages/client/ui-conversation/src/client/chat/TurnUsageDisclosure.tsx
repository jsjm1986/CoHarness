...
function compact(value: number, t: ComposerBarProps['t']): string {
  return t('message.turnUsage.count', { count: formatTokens(value, t) })
}

function exact(value: number, t: ComposerBarProps['t']): string {
  return t('message.turnUsage.count', { count: formatExactTokens(value, t) })
}

/** Compact exact per-turn usage with an opt-in bucket breakdown. */
export function TurnUsageDisclosure({ usage, t }: TurnUsageDisclosureProps) {
  ...
  const total = compact(usage.totalTokens, t)
