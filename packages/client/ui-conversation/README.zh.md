...
常驻会话壳会跨无会话与会话状态切换而保留。没有当前会话时，它会锁定消息操作，并让整张虚线编辑器卡片成为根作用域 `conversation.hero.workspace` Workspace picker 的入口；textarea 保持只读且支持键盘操作。Hero 前方的标记是独立的根作用域 `conversation.hero.brand.mark` slot，未被占用时回退到 CoHarness 标记。选择 Workspace 会打开该空间最新的可见历史会话；没有历史时才连接或复用由 Host 拥有的空白会话，而且不会替换会话壳。根组件始终拥有同一个滚动容器与 Hero／编辑器子树；首个会话到达时，彼此独立的严格会话页头和主体 outlet 只填入各自区域，因此 Workspace picker、滚动主体、编辑器 seat 与 textarea 都保留原有 React 和 DOM identity。Hero 中存在未发送草稿时，切换 Workspace 会先要求确认；确认会清理浏览器拥有的草稿资源，取消则保持当前会话不变。活跃阶段，会话标题栏作为普通列 chrome，仅显示当前会话标题和视图标签；fork 谱系仍保留为会话数据，不投影到标题栏。其下滚动容器（`data-conversation-scroll`）承载流动排版的各视图与 sticky 编辑器栈（统计 dock＋输入区 dock＋输入栏）。该滚动容器无条件预留自己的滚动条槽，选用编辑器 overlay 的视图也仍把它保留为滚动容器，因此无论对话记录是否滚动、无论展示哪个视图标签，输入卡片都保持同一个横向位置（[决策](../../../.agents/notes/implemented/bug-fix/2026-08-04-composer-tab-gutter-reservation.zh.md)）。textarea 上的滚轮会链式处理：限高草稿先在本地滚动，到达边缘后再转交给该宿主。只有 Safari 会在原生编辑缩短草稿并留下陈旧软换行溢出时执行绘制前恢复；草稿增长、程序化更新与其他浏览器都不会为这项恢复读取布局（[决策](../../../.agents/notes/implemented/bug-fix/2026-08-13-safari-textarea-soft-wrap-reflow.zh.md)）。

Hero 本身就是“新建对话”入口——它的编辑器即草稿——因此工作区行只保留 Workspace 选择器（使用作用域图标而非文件夹，避免与工作台的文件控件并置时被误读为文件操作）。选择 Workspace 仍然优先打开历史；明确的新建会话操作保留在侧边栏入口和每个真实 Workspace 行上，走既有的空白会话复用路径，因此选择历史会话后不会失去开始新聊天的入口。
...
在 compact 视口下，AppFrame 顶栏拥有手机级 Session 标题；常驻 Session header 保留 action、utility seat 和视图 Tab，但不再重复完整的 breadcrumb 标题。Chat 流会在最后一行下方预留实际 composer 高度；compact 统计 dock 只显示轮次／步骤摘要与统计入口，完整耗时和 token 分组在手机 Sheet 中展开，从而不挤占阅读行。ContextMeter 明细也使用同一套 Sheet 几何。输入触发器、命令和模型选择菜单使用共享的手机 Sheet 几何，同时保留既有焦点和提交路径。
...
Think 行默认保持折叠，并在不展开思维链的情况下暴露实时推理（reasoning）吞吐：当推理块是流式输出尾部时，摘要从结算后的首个可见行切换到最新的非空行，其单行滚动区会随每个 delta 追到行内末端。展开该行会移除移动摘要，让完整推理（包括前导排版空白）进入普通页面流；结算后恢复左对齐的稳定首个可见行（[决策](../../../.agents/notes/implemented/feature/2026-08-02-web-thinking-tail-scroll.zh.md)）。
...
composer 统计 pill 的 token 账目来自经标准套件 `useProjection` 读取的通用 token-meter 投影 `tokenUsage`：计费输入为未缓存输入、缓存读取与缓存写入之和；缓存命中率以缓存读取除以该总量。仪表盘 pill 显示全日志轮次、步骤和解码吞吐，点击后展开 LLM／工具用时与 TTFT 明细；数据库 pill 显示准确的累计 token 总量和缓存命中率，点击后展开输入、缓存和输出分桶。所有数字都来自全日志的 `sessionStats` 投影（Host 端从步边界、首 token chunk、紧凑 `assistant/attempt` 记录、工具配对与已组装消息折算），因此分页与压缩都无法改变；未组合该单元的装配回退为对可见节点做窗口折算，其字段与投影一一对应。缺少 timing 边界或 usage 采样的步骤会退出延迟数字而不是使其失真；压缩使已加载窗口不再包含 assistant 节点时，持久计数、token 与上下文分组仍保持可见。轮次、步骤、耗时、缓存与 token 标签使用同一命名空间。只要准确记账可用，已结算轮次的 assistant footer 会出现用量 pill，显示总量和缓存命中率，并点击展开提供方／模型、输入、缓存、输出与推理明细；footer 会在用量 pill 旁放置独立的本轮用时与速度 pill；有记录时，点击它可查看准确的用时、首 token 和解码速度明细。未组合 token-meter 的部署会省略 token 分组；compact 下 pill 收缩为图标触控目标，打开的 dialog 仍保留完整文字。上下文占用率渲染为 composer 尾部的 ContextMeter：模型座位之后的一枚 14px 占用圆环，由 `contextPressure` 供数，仅当分子与路由容量都已知时才渲染；点击弹出的面板把「已用百分比」标题与 `~已用 / 容量` 数字，与来自 `contextBreakdown` 投影、带 `~` 前缀的启发式组成明细行（系统提示词、工具、对话消息）及分色分段进度条并列；compact 下该明细使用共享手机 Sheet 遮罩和几何。圆环与标题读取 `projectedTokens`——把提供方样本沿此后表层的增减推进到当下——因此压缩会立刻反映出来，而不必再等一整轮；组成明细行仍是纯启发式，因此加起来依然不等于标题数字（[原理](../../llm/token-meter/README.zh.md)）。占用率是刻意为之的近似值：分子与容量是两个相互独立的「后写覆盖」投影字段，并非同一次请求的原子观测。
