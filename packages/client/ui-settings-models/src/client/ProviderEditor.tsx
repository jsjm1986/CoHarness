...
export function ProviderEditor(props: ProviderEditorProps): ReactNode {
  const { namespace, schema, settingsPath, api, discoverModels, t } = props
  ...
  const applyOnce = async (): Promise<string | undefined> => {
    ...
    // A provider names the conventional reference when this page stores a key.
    // Native pi-ai profiles preserve an inherited reference when one exists.
    const recordsKeyRef = keyValue.length > 0
      && stringAt(draft, 'apiKeyEnv') === undefined
      && (layout === 'deepseek'
        || (layout === 'pi-ai' && stringAt(fallback, 'apiKeyEnv') === undefined))
        ...
        {props.credentialOnly === true ? null : <details className={styles['customized']}>
          ...
          <div className={styles['customizedBody']}>
            ...
            {family === 'deepseek'
              ...
              : <ModelListEditor {...catalogProps} probe={probe} probeBlocked={keyFailure} discoverModels={discoverModels} />}
          </div>
        </details>}
      </>
    )
