...
项目空间的镜像仍暴露项目当前有效值，但会以 `writable: false` 和 `writableReason: project` 标记只读；设置提供方只读时使用 `provider`。scope 在首次视图仍处于 loading 或当前权限只读时拒绝 mutation，因此这些状态不会产生任何 mutation RPC。每次接受的写入会发布 `saving`，成功应答折入共享镜像；最新写入被拒绝或失败后先恢复再记录 `error`。功能行渲染这些状态，不会持久化被阻止的选择。
