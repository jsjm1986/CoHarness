...
export function formatScheduleFrequency(record: ScheduleRecord, t: TranslateNS<typeof NS>): string {
...
export function formatScheduleLocalTime(scheduledAt: string, locale?: string): string {
  return new Intl.DateTimeFormat(locale, { dateStyle: 'medium', timeStyle: 'short' }).format(Date.parse(scheduledAt))
}

/** Human relative target using the largest natural clock unit. */
export function formatScheduleRelative(scheduledAt: string, now: number, t: TranslateNS<typeof NS>): string {
...
export function ScheduleCatalogAction({ useSession, useProjection, t }: ScheduleCatalogActionProps) {
    ...
    ? createPortal((
      ...
      >
        {rows.map((record) => {
          ...
          return (
            <li key={record.id} className={overdue ? `${css.row} ${css.rowOverdue}` : css.row}>
  ...
  return <div ref={rootRef} className={css.root} onKeyDown={onKeyDown}>{trigger}{catalog}</div>
}
