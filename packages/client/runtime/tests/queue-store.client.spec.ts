...
describe('queue snapshot intake', () => {
  it('restores a cold inbox projection, keeps it across subscription baselines, and releases its listener', async () => {
    ...
    session.projections.seed({ asOfSeq: 4, values: { queuedInbox: frame.items } })
    ...
    session.projections.seed({ asOfSeq: 3, values: { queuedInbox: [] } })
    expect(session.getSnapshot().queue).toHaveLength(1)
    session.dispose()
    session.projections.seed({ asOfSeq: 5, values: { queuedInbox: [] } })
