...
describe('Workspace resource lifetime', () => {
  ...
  it('aborts in-flight reads on disconnect and clears retained content on revoked access', async () => {
    const registry = new WorkspaceResourceRegistry()
    const pending = deferred<WorkspaceResourceValue>()
    const stat = vi.fn((_address, _signal: AbortSignal) => pending.promise)
    registry.register(base, { stat }, 10)
    ...
    const release = registry.pin(request)
    await vi.waitFor(() => { expect(stat).toHaveBeenCalledTimes(1) })
    registry.disconnect(base, new WorkspaceResourceError('collaboration-forbidden', 'denied'))
    expect(stat.mock.calls[0]?.[1].aborted).toBe(true)
    pending.resolve(value())
    await Promise.resolve()
    expect(registry.source(request).get()).toMatchObject({ status: 'failed', error: { code: 'collaboration-forbidden' } })
    expect(registry.source(request).get().value).toBeUndefined()
    release()
  })

  it('defers outage-pinned resources until the stream reconnects', async () => {
    const registry = new WorkspaceResourceRegistry()
    const stat = vi.fn(async () => value())
    registry.register(base, { stat }, 10)
    registry.disconnect(base)
    ...
    expect(registry.source(request).get().status).toBe('none')
    registry.connected(base)
    await vi.waitFor(() => { expect(registry.source(request).get()).toMatchObject({ status: 'live', value: { version: 'v1' } }) })
    release()
  })

  it('marks a deleted file changed and republishes authoritative metadata on reload', async () => {
    const registry = new WorkspaceResourceRegistry()
    const stat = vi.fn(async () => value())
    registry.register(base, { stat }, 10)
    const request = resource()
    const source = registry.source(request)
    await source.reload()
    registry.handleChange(base, { sessionId, path: request.path })
    expect(source.get()).toMatchObject({ value: { changed: true } })
    await source.reload()
    expect(source.get()).toMatchObject({ status: 'live', value: { version: 'v1', changed: false } })
    registry.handleChange(base, { sessionId, path: request.path, version: 'v1' })
    expect(source.get()).toMatchObject({ value: { version: 'v1', changed: false } })
  })

  it('reports an unregistered target as unavailable without opening reads', async () => {
    const registry = new WorkspaceResourceRegistry()
    const source = registry.source(resource())
    expect(source.get()).toMatchObject({ status: 'failed', error: { code: 'access-revoked' } })
