...
- `readSession(sessionId)` returns one complete detached raw log after the same core replay validation used by resume; it never enters the session into the live store. Body-free records expose only `SessionHeader.isSeeded`; reads that return event bodies (`readSession`, `readSurface`, `readEvent`) and retained `SessionObservation` values also carry the exact `inheritedEventCount`, so callers distinguish inherited and owned events without inferring a cut from the log.
