...
持久化 FTS 行位于专用派生数据库中。连接本地 TEMP 表保存实时行，这些行会遮蔽同一会话的持久化基库，并在实时所有者消失后使其重新可见。两张表都在数字列 `seed_length` 中保留精确的继承切点；重建出的 header 只暴露 `isSeeded`，而该切点参与实时指纹与持久化源修订的计算。卸载持久化会隐藏持久行，但不会丢弃缓存；重新挂载会对账缓存。关闭或重新打开数据库会删除全部实时覆盖层，但保留持久行。
...
**运行时不变式：** 不发布伴生入口。系统会在每次串行化查询边界校验对账、游标世代与派生索引归属。
