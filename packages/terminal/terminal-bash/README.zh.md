...
`shellDialect` 选择 shell 栈（默认 `bash`，或 `pwsh`）：它决定默认的 `shellPath`／`shellArgs`（bash 为 `--noprofile --norc -i`；pwsh 经共享的 `dsh-pwsh-local` 解析器得到 `-NoLogo -NoProfile`）与启动契约。bash 方言通过环境安装提示符（`PS1` 加 OSC `133;D;` 终结的 `PROMPT_COMMAND`）。pwsh 无法从环境安装提示符，因此后端通过会话写入 `prompt` 函数，并只接受后端的 `stdin_read` 就绪证据；回显的引导源码不能发布 shell，后续发送共享一个绝对启动 deadline。其环境去掉 bash 专属标记并加 `NO_COLOR`。首发送还会带上共享的 `dsh-pwsh-local` 编码前缀，在一切运行之前把 `[Console]::OutputEncoding` 与 `$OutputEncoding` 钉为 UTF-8。零滚动的 `@xterm/headless` 实例消费原始 PTY 数据，并通过同一个句柄串行发送终端协议应答；调用方输入前会排空应答，并在并发协议活动后重新检查前台状态。两种方言发出相同的 BEL 终结 OSC 标记，因此就绪机制与消费方与方言无关。
