...
|---|---|---|
| `SessionStart` | `agent/created` (serial) | additionalContext → `agent.inject()` before creation returns and the first request (cannot deny creation) |
...
| `PostToolUse` | `tools/post-execute` (waterfall) | `deny` → `block` with feedback; additionalContext-only → delegate via `next()` then prepend a separately sourced context to the downstream decision; PTC mode defers sub-call contexts until the outer `run_code` result |
...
`SessionStart` is awaited during initialization. Creation cancellation or bridge disposal aborts its processes and prevents late context injection. Hook failures are logged without denying creation. `SubagentStart` and `SubagentStop` remain detached. Every run chain is tracked; bridge disposal aborts running hook processes and drains continuations before resolving (`createDetachedRuns` in `dsh-hook-protocol`).
...
- **`SessionStart` is partial:** JSON `additionalContext` is consumed, but plain stdout context, `initialUserMessage`, `sessionTitle`, `watchPaths`, `reloadSkills`, and `CLAUDE_ENV_FILE` are unsupported. The payload omits current optional fields such as `model`, `agent_type`, and `session_title`.
