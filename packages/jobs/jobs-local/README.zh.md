...
终止记录在超过 `terminalRetentionMs`（默认 24 小时）、`maxTerminalJobs`（1,000）或 `maxTerminalOutputBytes`（64 MiB）时回收。活动记录和拥有等待者的记录会保留；这些限制只在终止结果发布后生效。`terminalRetentionMs` 限制为 Node 的最大计时器延迟（`2,147,483,647` 毫秒），因此无效计时器值不会把周期清理变成高频循环。
