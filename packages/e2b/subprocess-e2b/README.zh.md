...
- **异步远程启动**：同步 seam 会立即返回一个句柄，同时由 `Sandbox.commands.run(..., { background: true })` 在远程启动进程。subprocess 句柄不公开进程 ID；包装层会发布私有进程组 ID，由适配器验证后才放行 stdin 和常规观察。自有启动信号会在分配前中止环境和私有状态准备；分配开始后，取消会等待可清理的临时 SDK 句柄。
...
- **不支持需要 OS PID 的消费方**：subprocess 句柄不公开进程 ID；包括 ACP（Agent Client Protocol）子进程后端在内，要求该 ID 的消费方无法原样使用本提供方。
