...
describe.skipIf(process.platform === 'win32')('SSH helper runtime', () => {
  ...
  it('rejects stale read versions and bounds directory enumeration through the helper wire', async () => {
    const test = await helper()
    try {
      await writeFile(`${test.root}/versioned`, 'before')
      await writeFile(`${test.root}/other`, 'other')
      const target = await test.client.request('fs.resolve', { path: 'versioned' }, targetSchema)
      const info = await test.client.request('fs.stat', { target }, infoSchema)
      const read = { target, offset: 0, length: 6, expectedVersion: info.version }
      expect(Buffer.from(await test.client.request('fs.readRange', read, z.string()), 'base64').toString()).toBe('before')
      const id = await test.client.request('fs.stream', { target, expectedVersion: info.version }, z.string())
      await writeFile(`${test.root}/versioned`, 'after')
      await expect(test.client.request('fs.readRange', read, z.string())).rejects.toMatchObject({ code: 'FS_STALE_VERSION' })
      await expect(test.client.request('fs.next', { id }, z.unknown())).rejects.toMatchObject({ code: 'FS_STALE_VERSION' })
      await expect(test.client.request('fs.next', { id }, z.unknown())).rejects.toThrow('Unknown SSH text stream')
      const directory = await test.client.request('fs.resolve', { path: '.' }, targetSchema)
      expect(await test.client.request('fs.list', { target: directory, maxEntries: 1 }, entriesSchema)).toHaveLength(1)
      expect(await test.client.request('fs.list', { target: directory, maxEntries: 0 }, entriesSchema)).toEqual([])
      await expect(test.client.request('fs.list', { target: directory, maxEntries: -1 }, entriesSchema)).rejects.toThrow()
      await expect(test.client.request('fs.stream', { target, expectedVersion: 1 }, z.string())).rejects.toThrow()
    } finally { await test.close() }
  })
