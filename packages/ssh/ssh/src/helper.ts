...
export async function runSshHelper(transport: HelperTransport): Promise<void> {
  ...
  const peer = new SshRpcPeer(transport.input, transport.output, MAX_FRAME_BYTES, 128, async (method, raw, requestSignal) => {
    ...
    if (method === 'fs.list') {
      const input = z.object({ target: targetSchema, maxEntries: z.number().int().nonnegative().optional() }).strict().parse(raw)
      return ctx.fs.listDir(asTarget(input.target), signal, input.maxEntries)
    }
    if (method === 'fs.stat' || method === 'fs.readText' || method === 'fs.stream') {
      const input = z.object({ target: targetSchema, ...(method === 'fs.stream' ? { expectedVersion: z.string().optional() } : {}) }).strict().parse(raw)
      ...
      if (method === 'fs.stream') {
        ...
        const stream = await ctx.fs.streamText(
          target, AbortSignal.any([signal, controller.signal]), input.expectedVersion as FsVersion | undefined,
        )
    ...
    if (method === 'fs.readBytes' || method === 'fs.readRange') {
      const input = z.object({
        ...
        ...(method === 'fs.readRange' ? { expectedVersion: z.string().optional() } : {}),
      }).strict().parse(raw)
      const target = asTarget(input.target)
      const bytes = method === 'fs.readBytes'
        ...
        : await ctx.fs.readByteRange(target, {
          ...
          ...(input.expectedVersion === undefined ? {} : { expectedVersion: input.expectedVersion as FsVersion }),
