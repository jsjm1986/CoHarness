...
Byte windows and text streams accept `expectedVersion`; the remote provider rejects stale versions during reads. Directory listing passes `maxEntries` to remote enumeration instead of scanning everything and truncating on the Host.
