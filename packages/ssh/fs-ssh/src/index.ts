...
export class SshFileSystem extends FileSystem {
  ...
  override async streamText(target: FsTarget, signal?: AbortSignal, expectedVersion?: FsVersion): Promise<AsyncIterable<string>> {
    const id = await this.call('fs.stream', { target, ...(expectedVersion === undefined ? {} : { expectedVersion }) }, textStreamIdSchema, signal)
  ...
  override async readByteRange(
    target: FsTarget, range: { offset: number; length: number; expectedVersion?: FsVersion }, signal?: AbortSignal,
  ): Promise<Uint8Array> {
  ...
  override async listDir(target: FsTarget, signal?: AbortSignal, maxEntries?: number): Promise<FsDirEntry[]> {
    return await this.call('fs.list', { target, ...(maxEntries === undefined ? {} : { maxEntries }) }, entriesSchema, signal) as FsDirEntry[]
  }
