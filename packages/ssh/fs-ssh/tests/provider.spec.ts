...
describe('SSH filesystem provider', () => {
  ...
  it('carries source versions and finite listing limits to the helper', async () => {
    const { fs, dispatch } = await setup()
    const version = FsVersion('observed')
    dispatch.mockResolvedValueOnce('YQ==').mockResolvedValueOnce([]).mockResolvedValueOnce(streamId)
      .mockResolvedValueOnce({ done: true, value: 'a' })
    expect(await fs.readByteRange(target, { offset: 0, length: 1, expectedVersion: version })).toEqual(Buffer.from('a'))
    expect(await fs.listDir(target, undefined, 1)).toEqual([])
    const content: string[] = []
    for await (const chunk of await fs.streamText(target, undefined, version)) content.push(chunk)
    expect(content).toEqual(['a'])
    expect(dispatch.mock.calls.slice(0, 3)).toEqual([
      ['fs.readRange', { target, offset: 0, length: 1, expectedVersion: version }, undefined],
      ['fs.list', { target, maxEntries: 1 }, undefined],
      ['fs.stream', { target, expectedVersion: version }, undefined],
    ])
  })
