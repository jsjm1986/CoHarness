...
describe('DeepSeekFileStore', () => {
  ...
  it('removes reclaimed remote ids from the local index as each delete succeeds', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'dsh-file-store-'))
    const index = new DeepSeekUploadIndex(join(dir, 'index.json'))
    ...
    await index.commit({
      ...
      attachmentId: VERSION.attachment.attachmentId,
      variantId: VERSION.variantId,
      fileId: DeepSeekFileId('file-api-old'),
      bytes: VERSION.bytes,
      createdAt: NOW,
      expiresAt: NOW + POLICY.expiresAfterSeconds * 1_000,
    }, NOW, 0)
    const fetchImpl = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      if (init?.method === 'DELETE') return new Response(JSON.stringify({
        id: requestUrl(input).split('/').at(-1), object: 'file', deleted: true,
      }), { status: 200 })
      return new Response(JSON.stringify({
        object: 'list',
        data: [{
    ...
    await expect(store.reclaimOldestOwned(CONNECTION, 1)).resolves.toBe(1)
    await expect(index.get(scope, VERSION.variantId, NOW, 0)).resolves.toBeUndefined()
  })
