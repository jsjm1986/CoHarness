...
describe('Cordis provider composition', () => {
  ...
  it.each([
    ...
    if (inHistory && model === MODEL) await ctx.settings.update(settingsNamespace(Messages.name), { models: [{ id: model, systemPromptUpdate: 'in-history' }] })
  ...
  it.each([false, true])('continues and resumes Chat Completions sessions through Messages, in-history=%s', async (inHistory) => {
    ...
    await ctx.settings.update(settingsNamespace('llm-deepseek'), { protocol: 'chat-completions', baseURL: http.url, models: [{ id: MODEL, systemPromptUpdate: 'in-history' }] })
    ...
    await ctx.settings.update(settingsNamespace(Messages.name), { protocol: 'messages', models: [{ id: MODEL, ...inHistory ? { systemPromptUpdate: 'in-history' } : {} }] })
  ...
  it('loads one provider from YAML, rotates settings and credentials, then removes disposed registrations', async () => {
    ...
    await ctx.settings.update(settingsNamespace(Messages.name), { baseURL: second.url, maxTokens: 51, retryPolicy: { mode: 'always' } })
    ...
    await ctx.settings.update(settingsNamespace(Messages.name), { models: [{ id: 'duplicate' }, { id: 'duplicate' }], baseURL: http.url })
    await chunks(ctx.llm.stream(options()))
    expect(second.requests).toHaveLength(2)
    expect(http.requests).toHaveLength(1)
    await ctx.settings.update(settingsNamespace(Messages.name), { models: [{ id: MODEL }], baseURL: http.url })
    await chunks(ctx.llm.stream(options()))
