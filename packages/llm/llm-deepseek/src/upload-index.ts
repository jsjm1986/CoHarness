...
export class DeepSeekUploadIndex {
  ...
  /**
   * Remove every mapping for exact remote generations in one namespace.
   *
   * The Files quota reclaimer can delete several provider files in one pass;
   * deleting by exact ids keeps a concurrently installed replacement for the
   * same image variant intact.
   * @param scope - endpoint/API-key namespace.
   * @param fileIds - remote generations that were successfully deleted.
   */
  async removeFileIds(
    scope: DeepSeekFileScopeType,
    fileIds: readonly DeepSeekFileIdType[],
  ): Promise<void> {
    ...
    await withFileLock(this.path, async () => {
      const index = await this.load()
      const records = index.records.filter(record => !(record.scope === scope && ids.has(record.fileId)))
      if (records.length !== index.records.length) await this.save({ formatVersion: 3, records })
    })
  }
