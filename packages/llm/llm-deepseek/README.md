...
The plugin registers the single provider route `deepseek-official` together with its resolved `retryPolicy`; omission resolves to normal mode with five retries. A request selects it with `provider: deepseek-official`; its `model` is passed through as the wire `model` string, so changing DeepSeek models does not require lifecycle-time registration. Omitting `models` advertises V41 Flash (`deepseek-flash`), V4 Flash, V4 Pro, and the image-capable V4 Flash Vision Exp, each with a 1,000,000-token context window; an explicit list replaces those defaults, while `models: []` advertises none. Catalog entries are exposed through `ctx.llm.listModels('deepseek-official')` for clients such as ACP editors and the Web selector, but remain advisory: unlisted model ids still pass through unchanged as text-only routes. An omitted entry name defaults to its id, and omitted `inputModalities` means `text` only.

An image-capable catalog entry declares `inputModalities: [text, image]` and may set `imagePixelBudget` or `imageMaxBytes`. The ordinary default is 640,000 total pixels and 1MiB encoded bytes; low detail defaults to 512 by 512 total pixels. The V41 vision-token estimator scales images below 544×544 total pixels up, projects a 14px patch grid at a 3:1 downsample ratio, and solves the largest aspect-preserving grid under 1,024 tokens; actual provider usage remains authoritative. The attachment store scales by `min(1, sqrt(pixelBudget / (width * height)))` and rounds inward to keep the pixel count at or below the hard cap. Request encoders run lazily and share concurrent generation of one `variantId`; a caller can cancel its own wait without interrupting other waiters. The adapter normally uploads the exact derived request bytes through `POST /files` and sends `{type: "file", file_id}` blocks. A failed or timed-out file-id resolution rebuilds the whole chat request with those same request versions as base64 data URLs; one request never mixes file ids and inline images. Every retained image is preceded by stable text naming the complete attachment id and actual request dimensions.

`maxRequestFilesBytes` and `maxImagesPerRequest` bound retained request versions at 128MiB and 600 images by default. The adapter removes the oldest over-budget prefix before attachment reads, then checks exact derived lengths without restoring omitted images. Removed images become the fixed model-visible placeholder `[image omitted to keep the request within its image limit; older images are omitted first. If this image is still needed, read its file again when a path is available; otherwise ask the user to attach it again.]`. Inline fallback has an independent base64 budget: `maxInlineRequestImageBytes` defaults to 20MiB and `inlineImageOffloadByteQuantum` to 10MiB. Prepared request versions are reused byte-for-byte; fallback does not decode or compress an image again.

Uploaded ids are indexed below `DSH_HOME` by endpoint/API-key scope and request `variantId`. The variant covers the normalized attachment id, transform version, route pixel and byte budgets, and encoder parameters, so Files API and inline fallback refer to the same deterministic bytes. Uploads request a seven-day lifetime by default and refresh ids with no more than one hour remaining. Stale or invalid ids are invalidated and retried once; file resolution has a per-image one-minute deadline by default, independent of the five-minute stream idle deadline. Concurrent resolution shares one Files upload with waiter-local cancellation, and quota recovery deletes the configured number of oldest harness-owned files before one retry. `DeepSeekFilesClient.delete`, `DeepSeekFileStore.release`, and `releaseAll` expose explicit remote-space reclamation.
...
- Token accounting: `cacheReadTokens` ← `prompt_cache_hit_tokens` / `prompt_tokens_details.cached_tokens`; DeepSeek reports no cache-write metric. `totalTokens` is retained when aggregate prompt and completion counts are safe and consistent.
