...
entry config 与可编辑的 `llm-pi-ai` settings namespace 属于个人 Provider 层，不得声明 `org-*` 路由或 `DSH_` 凭据引用；这两个命名空间专供 Gateway 管理的组织 Provider 使用。Gateway 校验每份完整组织 profile 后通过 `ctx.modelProviderConfig` 注入，治理插件则通过只读凭据层解析其中的 `DSH_` 引用。管理界面可以复用同一套 Models 编辑器与 profile 字段，但数据经组织 facade 持久化，不会让该 profile 进入用户可编辑的 settings 文档。
...
- pi-ai 将推理 token 折叠到输出 usage 中；没有可映射的独立推理计数。其精确 `totalTokens` 值会保留。
...
- **`headers` 可能承载一条脱敏器看不见的凭据**：profile 解析会拒绝 Fetch 无法表示的名称和值，但该字典仍是纯字符串，因此设在其中的 `Authorization` 或 `api-key` 会被脱敏后的 `describe()` 原样返回，并被任何配置 UI 渲染出来。请把凭据存为 `apiKeyEnv` 引用；把该字典整体改为只写与其余[协议边界工作](../llm/README.zh.md#known-limitations-and-deferred-work)一并暂缓。
- **路由的 catalog 不会自我刷新**：catalog 就是 `settings.yaml` 所写的内容，因此模型列表的新鲜度只到最近一次编辑为止。端点询问只是一次性问题，候选项必须由用户采纳；只有当有人写入模型时，路由才会增加它。
