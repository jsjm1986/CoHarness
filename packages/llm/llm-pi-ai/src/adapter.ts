...
export class PiAiAdapter extends LlmAdapter {
  ...
  private current(): PiAiSnapshot {
    ...
    if (
      this.snapshot?.profiles === profiles
      && this.endpointCache.generation === this.snapshot.endpointCacheGeneration
    ) {
      return this.snapshot
    }
    // A generation newer than the captured one means the profile owner already
    // invalidated the cache for this configuration change; clearing again
    // would drop candidates discovery learned after that invalidation.
    if (this.endpointCache.generation === this.snapshot?.endpointCacheGeneration) {
    ...
    this.snapshot = { profiles, models, endpointCacheGeneration: this.endpointCache.generation }
    return this.snapshot
  }
