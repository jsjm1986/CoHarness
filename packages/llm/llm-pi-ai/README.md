...
`baseURL` is a prefix for every model on the route, so private proxies such as `https://proxy.example.com:8443` remain supported; a catalog route that omits it keeps each catalog model's own endpoint. OpenAI-compatible requests try the entered prefix first and its one trailing `/v1` toggle when the response is a clear path mismatch (404/405 or an explicit non-SSE success), then remember the working prefix in process memory. The cache is shared with endpoint interrogation, cleared when the profile snapshot changes, and never writes a normalized URL back to settings. The Anthropic Messages SDK always receives a normalized base and appends exactly `/v1/messages`, whether the input included `/v1`. Naming `api` on a catalog route repoints the whole route at that protocol, which is how a deployment moves a provider between, say, Responses and Chat Completions.
...
The entry config and the editable `llm-pi-ai` settings namespace are the personal Provider layer. They cannot declare `org-*` routes or `DSH_` credential references; both namespaces are reserved for Gateway-managed organization Providers. The Gateway validates each complete organization profile and injects it through `ctx.modelProviderConfig`, while the governance plugin resolves its `DSH_` reference through a read-only credential layer. An administration surface may reuse the same Models editor and profile fields, but it persists through the organization facade rather than making the profile editable in a user's settings document.
...
The protocol request is delegated to the shared `@deepseek-ai/dsh-llm/discovery` helper, which Gateway uses for the same operation. It reads three documented listings. `openai-completions` and `openai-responses` send bearer authentication to `GET {baseURL}/models`; `anthropic-messages` sends `x-api-key` plus `anthropic-version: 2023-06-01` to `GET {baseURL}/v1/models`. OpenAI listing discovery tries the entered prefix and then one `/v1` toggle after a 404, 405, or explicit `text/html` response, and shares its process-local cache with model requests. All three return a `data` array that this package can parse without guessing. Azure is excluded despite its OpenAI lineage — it authenticates with an `api-key` header and requires an `api-version` query — and Codex uses OAuth; every other protocol answers `DISCOVERY_UNSUPPORTED` so the surface falls back to hand-entry instead of an authentication failure being reported as a provider with no models. The `baseURL` is treated as a prefix rather than a URL to resolve against, so deployment path segments are preserved; an Anthropic base ending in `/v1` is normalized for both listing and message requests.
...
### Tagged reasoning in OpenAI-compatible text

For `openai-completions`, the adapter also recognizes a strict reasoning prefix when a gateway puts thinking in ordinary `delta.content`. It waits for the first non-whitespace token to be exactly `<thinking>`, `<analysis>`, or `<think>`, requires a non-empty body and matching closing tag, then emits the body as a `reasoning` block and the suffix as `text`. Recognition spans stream chunks and uses cumulative `text_end` or terminal content when deltas are missing. If an ordinary text block's cumulative content differs from its deltas, that complete content remains authoritative at `block-end`. Native pi-ai reasoning fields stay on their native path; tags later in ordinary prose, empty or unclosed tags, and code or XML examples stay text.
...
- pi-ai folds reasoning tokens into output usage; there is no separate reasoning count to map. Its exact `totalTokens` value is preserved.
...
- **`headers` can carry a credential the redactor never sees** — profile resolution rejects names and values Fetch cannot represent, but the dict remains plain strings, so `Authorization` or `api-key` set there is returned verbatim by a redacted `describe()` and rendered by any configuration UI. Store credentials as `apiKeyEnv` references; making the dict write-only is deferred with the rest of the [wire-boundary work](../llm/README.md#known-limitations-and-deferred-work).
- **A route's catalog never refreshes itself** — the catalog is whatever `settings.yaml` says, so a model list is only as current as its last edit. Endpoint interrogation is a one-shot question whose candidates a person must adopt; a route gains a model only when someone writes one.
