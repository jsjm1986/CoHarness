...
`baseURL` sets the endpoint of every model on the route, so private proxies such as `https://proxy.example.com:8443` remain supported; a catalog route that omits it keeps each catalog model's own endpoint. The Anthropic Messages SDK appends `/v1/messages`, so a trailing `/v1` is removed before dispatch and both a relay root and its versioned base reach the same endpoint. Naming `api` on a catalog route repoints the whole route at that protocol, which is how a deployment moves a provider between, say, Responses and Chat Completions.
...
- **Provider HTTP status is partial** — pi-ai's response callback exposes the status and headers before a body is consumed, so the adapter retains the status when a terminal-event parser error follows an explicit non-SSE `Content-Type`. Other pi-ai error events still do not expose a stable HTTP status across providers and carry only stable harness error codes.
