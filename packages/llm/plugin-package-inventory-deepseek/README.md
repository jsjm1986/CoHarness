...
The standalone plugin defaults to enabled. The shipped dsh profiles override it to disabled unless `COHARNESS_SEND_PLUGIN_METADATA=1` is set explicitly, so a normal profile sends no package inventory.
...
Collection does not wait for Loader activation. Callers that require newly mounted includes to appear must await `ctx.loader.await()` before preparing the request; `loader.create()` alone does not wait for the included plugins to activate.
