 ...
 * marker yields `signal`; a numeric marker yields `exitCode`, while the explicit
 * `[exit code: null]` marker retains an unknown exit. Absent both means a clean exit 0.
 *
...
export function parseExitStatus(text: string): ParsedExitStatus {
  ...
  const exit = /\n\[exit code: (\d+|null)\]$/.exec(text)
  if (exit?.[1] !== undefined) return { body: text.slice(0, exit.index), exitCode: exit[1] === 'null' ? null : Number(exit[1]) }
