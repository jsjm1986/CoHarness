 ...
 * without a file extension is identified from its file signature; the
 * attachment service's full decode remains authoritative.
 *
...
/**
 * Identify a supported image container from its leading bytes.
 * @param data - bytes read from the candidate image.
 * @returns the detected supported media type, or undefined for an unknown signature.
 */
...
/** Refuse a media type outside the deployment's accepted image policy. */
function assertDeploymentAccepts(
...
export function applyReadImageTool(ctx: Context): void {
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      // Extension-bearing paths declare their format before any filesystem
      // I/O. An extension-less path waits for its bytes so normalized
      // content-addressed attachment paths can be read directly.
