...
describe('tool-str-replace-editor', () => {
  it('registers the standalone schema and configurable description', async () => {
    ...
    expect(ctx.tools.get('str_replace_editor')?.presentCall?.({
      ...
      insert_line: null,
      new_str: 'x',
    })).toMatchObject({ locations: [{ path: '/workspace/a.txt' }] })
    expect(ctx.tools.get('str_replace_editor')?.presentCall?.({
      command: 'create',
  ...
  it('accepts null placeholders for unused editor parameters', async () => {
    const { ctx, root, owner } = await setup()
    const sample = join(root, 'null-placeholders.txt')
