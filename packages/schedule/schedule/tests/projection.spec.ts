...
function afterRecord(id: string, prompt = id): ScheduleRecord {
  return {
    id: ScheduleId(id), kind: 'after', prompt, afterSeconds: 30,
...
function atRecord(id: string): ScheduleRecord {
  return {
    id: ScheduleId(id), kind: 'at', prompt: id,
...
function everyRecord(id: string): ScheduleRecord {
  return {
    id: ScheduleId(id), kind: 'every', prompt: id, everySeconds: 300,
...
function created(schedule: ScheduleRecord, seq: number): SessionEvent {
  return change({ version: 1, operation: 'create', schedule }, seq)
}
