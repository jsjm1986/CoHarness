...
export class LocalTerminalHandle implements SubprocessTerminalHandle {
  ...
  constructor(
    ...
    this.rootIdentity = this.observe().tree(this.pid).find(member => member.pid === this.pid)
  ...
  private observe(): ProcessSnapshot {
    const snapshot = this.inspector.snapshot?.()
    if (snapshot !== undefined) return snapshot
    return {
      tree: rootPid => this.inspector.processTree(rootPid),
      session: sessionId => this.inspector.processSession(sessionId),
      alive: identity => this.inspector.isAlive(identity),
    }
  }
  ...
  private async waitForMembers(members: ProcessIdentity[]): Promise<ProcessIdentity[]> {
    ...
    let survivors = this.survivors(members, this.observe())
    while (survivors.length > 0 && Date.now() < until) {
      await delay(Math.min(25, Math.max(1, until - Date.now())))
      survivors = this.survivors(members, this.observe())
    }
    return survivors
  }
  ...
  private forceStopDescendants(): void {
    ...
    try {
      members = this.descendants(this.observe())
  ...
  private async stopDescendants(): Promise<ProcessIdentity[]> {
    const captured = this.descendants(this.observe())
    this.signalMembers(captured, 'SIGTERM')
    const capturedSurvivors = await this.waitForMembers(captured)
    const members = this.unionMembers(capturedSurvivors, this.descendants(this.observe()))
    this.signalMembers(members, 'SIGKILL')
