...
`compositionInventory()` exposes the same roster as flattened, read-only plugin rows for the Host plugin inventory. It answers from the newest live standing mount when one exists, otherwise parses the composition file without mounting it; unresolved `!!js` disabled expressions stay `conditional` rather than being guessed. The browser-safe `./display` subpath resolves built-in preset ids through the active locale and leaves user-authored names unchanged.
