...
class PresetTree extends Include {
  constructor(ctx: Context, config: Include.Config) {
    super(ctx, config)
    // A directly mounted Include records itself under the nearest Loader
    // entry. Standing preset mounts are not Loader rows, so reclaim that slot
    // before inventory readers walk the host tree.
...
export interface PresetMount {
  ...
  readonly fiber: Fiber
  /** Entry tree used to resolve the mounted preset's active package rows. */
  readonly tree?: EntryTree
 ...
 * last read.
 *
 * The record set is module state and spans Cordis runtimes in one process. A
 * reader for one root must not report a same-id mount owned by another root.
 * @param within - optional runtime root used to restrict the result.
...
export async function inactiveRows(tree: EntryTree): Promise<string[]> {
  ...
  for (const entry of tree.entries()) {
    ...
    /* v8 ignore next 4 -- the loader logs an entry whose module failed to import
       without one, so a settled tree never holds an enabled fiber-less entry;
       the branch exists only because `Entry.fiber` is declared optional. */
    if (fiber === undefined) {
...
export async function mountPreset(agentCtx: Context, preset: AgentPreset): Promise<void> {
  ...
  try {
    ...
    mounts.add({ presetId: preset.id, fiber, key: scopeOf(agentCtx), tree })
  } catch (error) {
    try {
