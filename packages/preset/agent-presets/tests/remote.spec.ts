...
describe('the roster a client reads', () => {
  ...
  it('answers an empty roster with nothing authorable', async () => {
    const ctx = await harness({ default: 'standard', roots: [], includeUserRoot: false })
...
describe('reading one composition', () => {
  ...
  it('raises an unrelated read failure through the RemoteError boundary', async () => {
    const ctx = await harness()
    ...
    expect(failure.message).toContain('disk failed')
...
describe('authoring over Remote', () => {
  ...
  it('raises an unrelated authoring failure through the RemoteError boundary', async () => {
    const ctx = await harness()
    ...
    const failure = await remoteFailure(ctx.agentPresets.remoteExportCopy('standard', 'mine'))
    expect(failure.code).toBe('gateway/internal')
    expect(failure.message).toContain('copy failed')
    expect(failure.cause).toBe(thrown)
  })
})
...
describe('switching one session\'s composition', () => {
  ...
  it('refuses once the conversation has started', async () => {
    ...
    // One content-bearing message is enough: the history from here on was
    // produced under `standard`'s tools, and a swap would strand those tool
    // calls. An empty turn alone does not end blankness — only real content does.
    agent.session.append('user/message', {
      id: MessageId('m-locked'),
      role: 'user',
  ...
  it('raises an unrelated switch failure through the RemoteError boundary', async () => {
    const ctx = await harness()
    ...
    const failure = await remoteFailure(ctx.agentPresets.select(agent, 'minimal'))
    expect(failure.code).toBe('gateway/internal')
    expect(failure.message).toContain('mount failed')
    expect(failure.cause).toBe(thrown)
  })
})
