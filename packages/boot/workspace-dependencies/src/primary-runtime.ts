/** Offline installation and absolute paths for CoHarness's bundled script dependencies. */
...
/** Versions recorded by the CoHarness build, independent of user-installed packages. */
 ...
 * @param source - Payload carried by the current CoHarness installation.
...
export async function installPrimaryRuntime(source: string, root: string): Promise<WorkspaceDependencies> {
  if (!isAbsolute(source) || !isAbsolute(root)) throw new Error('primary runtime: source and root must be absolute')
  const nested = (parent: string, child: string): boolean => {
    const path = relative(resolve(parent), resolve(child))
    return path === '' || path !== '..' && !path.startsWith(`..${sep}`) && !isAbsolute(path)
  }
  if (nested(source, root) || nested(root, source)) throw new Error('primary runtime: source and installation must not overlap')
  await mkdir(dirname(root), { recursive: true })
  const canonicalSource = await realpath(source)
  const canonicalRoot = join(await realpath(dirname(root)), basename(root))
  if (nested(canonicalSource, canonicalRoot) || nested(canonicalRoot, canonicalSource)) {
    throw new Error('primary runtime: source and installation must not overlap')
  }
  return withFileLock(canonicalRoot, () => installPayload(canonicalSource, root))
}

async function installPayload(source: string, root: string): Promise<WorkspaceDependencies> {
