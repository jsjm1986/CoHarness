...
| `watchUserPatches(ctx, options)` | 通过精确路径 watcher（`watchConfig`）注册指名的 patch 文件；每次新增、变更或移除都会通过调用方的 `compose` 闭包（应用自有层围绕当前用户层）重组完整 patch 列表且不回滚，条目级失败以一条带标签的警告错误上报，并返回异步 disposer |
...
`Loader.await()` 等待导入与生命周期任务结算，不因这些任务失败而拒绝。结算后，`assertEntriesLoaded` 拒绝已启用却没有 fiber 的配置项并列出未解析插件；`assertEntriesActivated` 包含失败 fiber 的原始堆栈及等待中配置项尚未解析的服务。审计通过一个进程级检查点标记确切的拒绝原因，使 `installFailLoud` 合并 Loader 的重复通知，而无关的未处理拒绝仍然致命。`boot()` 释放部分构造的上下文，并用 bin 名称包装启动失败。
...
内置 Loader 根据 Node 内部暴露的 module-job 方法（`getModuleJobForImport` 或 `getOrCreateModuleJob`）识别 ESM resolver。这样 Node 24.0–24.11.1 仍使用 v1 参数顺序，更新版本使用 v2；仅按主版本号判断是不够的。
...
每次 profile 启动都由 `watchUserPatches` 持续应用 `cordis.patch.yml` 的变更（一次性 surface 经由有界关闭释放监视器）。即使该文件或其直接父目录不存在，监视器仍会监视确切路径，串行处理突发变更，并按调用方的层次顺序重新组合用户 patch（组合包层在下、overlay 在上）。Loader 更新前的读取、解析与组合失败不会改变运行中的树。Loader 更新是非事务的：激活失败可能留下部分更新的树，不恢复先前条目。重载诊断列出无效的 disabled 表达式、导入失败、失败的 fiber 和未满足的服务；监视器记录错误与 patch 路径，并继续接收变更。上下文释放时关闭监视器，并等待进行中的刷新结束。
