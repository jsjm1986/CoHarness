...
**A read-only layer owns its references exclusively.** A registered layer decides which references it owns from current runtime state. An owned reference resolves and describes only through that layer: an unavailable or unconfigured result never falls through to a personal value, and `set`/`unset` reject until the registration is disposed. Overlapping claims fail loud.
...
The shadowing rule on `set`/`unset` is deliberate fail-loud: when a provider-internal read-only source or a registered read-only layer owns the reference, a write would appear to succeed while resolution keeps returning the shadowing value. The seam rejects instead, and `describe().writable` lets a UI render the reference read-only up front.
...
The Gateway model-governance plugin registers an organization layer for credential references in the active managed Provider snapshot. It resolves each value through the authenticated loopback runtime API with source `organization`; missing authorization, an unconfigured organization key, or a Gateway failure cannot expose a same-named personal value.
