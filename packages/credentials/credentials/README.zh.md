...
**只读层独占自己声明的引用。** 已注册的层根据当前运行时状态决定自己拥有哪些引用。被拥有的引用只通过该层解析和描述：不可用或未配置的结果绝不回退到个人值，`set`/`unset` 在注册释放前都会拒绝。多个层重复声明同一引用会明确报错。
...
`set`/`unset` 的遮蔽规则有意采用明确报错的方式：当提供方内部只读来源或已注册只读层拥有该引用时，写入会表面成功而解析仍返回遮蔽值。seam 选择直接拒绝，并通过 `describe().writable` 让界面提前把该引用渲染为只读。
...
Gateway 模型治理插件会为当前受管 Provider 快照中的凭据引用注册组织层。该层通过已鉴权的回环运行时 API 按次解析值，并将来源标记为 `organization`；授权缺失、组织密钥未配置或 Gateway 请求失败都不会暴露同名个人值。
