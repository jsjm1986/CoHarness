...
Config-only leaves (`mcp-memory`, `web-cordis`, `web-schedule`) keep no `tests/` tree; their wiring is exercised by the consuming app's suites (`apps/cli`, `apps/web`).
