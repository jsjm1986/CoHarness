...
/**
 * Both windows contain equal event and compact-record counts. The CoHarness
 * fold also rebuilds the location, turn-navigation, and timeline projections
 * on every window replace, so the small window carries more fixed cost and a
 * record-proportional fold plus joined text measures about 5×; replaying every
 * delta measures about 11× as the delta count grows 20×.
 */
const EXPECTED_DELTA_SCALING = 5
