...
// shipped tree with the tools row patched to mode: ptc (the run_code-only
...
const FIXTURE = fileURLToPath(new URL('./snapshots/ptc-round/session.jsonl', import.meta.url))
const UI_EXPECTED = fileURLToPath(new URL('./snapshots/ptc-round/ui.expected.md', import.meta.url))
...
describe('web e2e: PTC round renders nested sub-calls', () => {
  ...
  it.skipIf(MODE === 'record')('the durable log carries run_code with full-content sub-dispatches', () => {
    ...
    expect(starts.length).toBe(dispatches.length)
    ...
    for (const dispatch of dispatches) {
      ...
      expect(data.subCallId.startsWith(`${data.parentCallId}:ptc:`)).toBe(true)
