...
// telemetry row against a local dead endpoint (no record leaves
// the process), so the golden pins the shipped default sentence
// `Session sharing is feedback-gated…`; the per-status sentences are pinned
// by the package and OTel unit tests.
...
// Discard port: loopback listener never binds, so the telemetry row discloses
...
describe('web e2e: /feedback command acknowledgement', () => {
  ...
  it.skipIf(MODE === 'record')('records feedback and renders the acknowledgement with session id and sharing status', async () => {
    ...
    const snapshot = (await captureStableAria(page, '[data-variant="others"]', scaffold.workspaceCwd))
      // The ack row prints the anonymous user id, minted per run.
      .replace(/Anonymous user: [0-9a-f-]+/gi, 'Anonymous user: {{uuid}}')
