...
const SNAPSHOT_DIR = fileURLToPath(new URL('./snapshots/auto-review-denial', import.meta.url))
const FIXTURE = fileURLToPath(new URL('./snapshots/auto-review-denial/session.jsonl', import.meta.url))
const UI_EXPECTED = fileURLToPath(new URL('./snapshots/auto-review-denial/ui.expected.md', import.meta.url))
const MODE = webSnapshotMode()
const SEED_ID = 'auto-review-denial-web-e2e'

// Message clocks carry a calendar-day prefix only when the seeded time falls
// on another day than the run; the golden must depend on neither the runner
// timezone nor the wall clock at seed time.
function normalizeUi(aria: string): string {
  return aria
...
describe.skipIf(MODE === 'record')('web e2e: cold Auto-review denial', () => {
  ...
  beforeAll(async () => {
    ...
    // The settled turn folds its tool rows behind the turn-process disclosure.
    await expandTurnProcesses(page)
    await page.locator('[data-tool="mystery"]').waitFor({ state: 'visible', timeout: 15_000 })
  }, 120_000)
  ...
  it('shows only the Auto identity and normalized denial reason', async () => {
    ...
    const collapsed = normalizeUi(await captureStableAria(page, '[class*="centerCol"]', scaffold.workspaceCwd))
    ...
    const expanded = normalizeUi(await captureStableAria(page, '[class*="centerCol"]', scaffold.workspaceCwd))
    ...
    const trajectory = normalizeUi(await captureStableAria(page, '[class*="centerCol"]', scaffold.workspaceCwd))
