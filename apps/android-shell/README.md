...
The root `pnpm run check:android` command builds the ordinary debug and instrumentation APKs, runs Android lint and unit tests, and requires Java 21 plus SDK 36. The optional `android-audit` manual CI suite runs `test:android:protocol` against a private HTTP endpoint and executes the downloaded APKs on a named emulator through `check:android:bridge`. The native tests retain `NATIVE_PUSH_ENABLED=true` and exercise the actual MainActivity-to-WebView event path; diagnostic APKs cannot supply this evidence. Validation APKs use a private loopback endpoint, with cleartext allowed only for loopback in the debug variant.

Android is an optional consumer. Pull requests, the full audit and default product release requirements exclude native Android validation; the selection report retains affected Android rules as manual-only scope. Hosted Web and Gateway protocol checks keep their normal requirements. Compilation, bridge instrumentation and real push delivery remain distinct proofs; this optional suite does not validate delivery through a push service.
