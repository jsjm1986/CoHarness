...
`config/examples/` 交付记忆 MCP 服务器的可选覆盖层。运行时 Cordis 工具与会话内 Schedule 位于 [`examples/`](../../examples/)（`web-cordis`、`web-schedule`）；GitHub 评审 webhook 覆盖层随 webhook 车道待定。它们绝不属于默认 profile；设置与安全说明由[用户指南](../../docs/user/guide/index.zh.md)和[开发实战指南](../../docs/user/develop/practice/index.zh.md)负责。
