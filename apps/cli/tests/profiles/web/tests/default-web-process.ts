...
export async function withDefaultWeb(test: TestContext, inspect: (app: DefaultWeb) => Promise<void>): Promise<void> {
  ...
  try {
    ...
    await writeFile(patch, JSON.stringify([{ id: 'userdoc-local', config: { uploadRoot: join(root, 'documents') } }, { insert: [{
      id: 'runtime-roster-observer',
      name: pathToFileURL(join(root, 'fixtures/runtime-roster-observer.js')).href,
      config: { negativeControl: pathToFileURL(join(repoRoot, 'apps/web/tests/fixtures/plugins/fixture-experimental-isolation/index.js')).href },
    }] }]) + '\n')
