...
const GENERIC_SKIPS: readonly GenericSkip[] = [
  ...
  // The Chinese user guide names the shipped `cordis` preset id, not a vendor package.
  ...
  // Typert and Gateway retain the unscoped `cordis/*` event namespace as the
  // canonical wire id while accepting the rescoped package namespace too.
  { file: 'docs/subsystems/typert.md', upstream: ['cordis'] },
  { file: 'docs/subsystems/typert.zh.md', upstream: ['cordis'] },
  { file: 'packages/api/gateway/src/client/index.ts', upstream: ['cordis'] },
  { file: 'packages/api/gateway/tests/gateway.client.spec.ts', upstream: ['cordis'] },
  { file: 'packages/host/apiproxy/tests/api-proxy-config.spec.ts', upstream: ['cordis'] },
