...
interface DemoPolicy {
  readonly kind: 'dsh-direct' | 'dsh-wrapper' | 'example'
  readonly wrapper?: string
}

/** Public product launcher, runnable example applications, and the build-only WebWorker packer. */
...
const EXECUTABLE_SOURCE_ALLOWLIST = new Map<string, string>([
  ...
  ['packages/examples/acp-demo/src/bin.ts', 'runnable example application'],
  ['packages/examples/jsonrpc-demo/src/bin.ts', 'runnable example application'],
  ['packages/examples/jsonrpc-demo/src/packaged-bin.ts', 'runnable example application'],
...
/** Root demos are application wrappers and therefore must visibly select their launcher. */
...
function referencesExampleEntry(source: string): boolean {
  return /packages\/examples\/[^/\s'"`]+\/(?:src|lib)\/[^\s'"`]+/.test(source)
}
...
function rootDemoViolations(root: string): string[] {
  ...
  for (const [name, commandValue] of Object.entries(manifest.scripts ?? {}).sort(([left], [right]) => left.localeCompare(right))) {
    ...
    if (policy.kind === 'example' && wrapper === undefined) {
      if (!referencesExampleEntry(command)) failures.push(`package.json scripts.${name}: example demo must launch a packages/examples entry`)
      continue
    }
    ...
    if (policy.kind === 'example') {
      if (!referencesExampleEntry(source)) failures.push(`${wrapper}: example demo wrapper must launch a packages/examples entry`)
      continue
    }
