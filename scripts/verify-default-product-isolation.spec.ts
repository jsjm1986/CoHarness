...
const preset = 'apps/cli/config/agent-presets/standard/agent.cordis.yml'
...
describe('default product isolation', () => {
  it('rejects unknown workspace packages', () => {
    const root = fixture()
    const file = 'packages/core/core/package.json'
    manifest(root, file, { dependencies: { '@deepseek-ai/dsh-unknown': '0.0.1' } })
  ...
  it.each([false, true])('rejects a declared config tree without composition files when its directory exists: %s', (existing) => {
    const root = fixture()
    manifest(root, 'apps/cli/package.json', { dsh: { configTrees: [{ path: './other-config' }] } })
    if (existing) write(root, 'apps/cli/other-config/README.i18n.yaml', 'en: test\n')

    expect(verifyDefaultProductIsolation(root).failures.join('\n')).toContain('apps/cli/other-config')
  })
