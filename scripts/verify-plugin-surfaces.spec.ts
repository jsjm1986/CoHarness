...
import {
  clientGraphTransportViolations,
...
describe('verify-plugin-surfaces', () => {
  it('rejects missing live delivery and production artifact polling', () => {
    const row = {
      name: '@deepseek-ai/dsh-client-hmr',
      config: { watchArtifacts: { __jsExpr: "process.env.DSH_CLIENT_HMR === '1'" } },
    }
    expect(clientGraphTransportViolations(row)).toEqual([])
    expect(clientGraphTransportViolations(undefined)).toHaveLength(1)
    expect(clientGraphTransportViolations({ ...row, disabled: true })).toHaveLength(1)
    expect(clientGraphTransportViolations({ ...row, disabled: { __jsExpr: "process.env.DSH_CLIENT_HMR !== '1'" } })).toHaveLength(1)
    expect(clientGraphTransportViolations({ ...row, config: { watchArtifacts: true } })).toHaveLength(1)
    expect(clientGraphTransportViolations({ name: row.name })).toHaveLength(1)
  })
  ...
  it('accepts live graph delivery, opt-in artifact polling and all Bundle patch targets', () => {
    const facts = collectPluginSurfaceFacts(ROOT)
