...
function renderFixture(headerLine: string, events: readonly SessionEvent[]): string {
  return [
    ...
    ...packChunkRuns(events).map((stored) => {
      ...
      // envelopes are regenerated by parseSessionLog on replay, while the
      // physical Python snapshots remain outside this helper's scope.
      delete record.seq
      delete record.time
      delete record.seq0
      delete record.time0
...
export function canonicalSessionFixture(content: string, label = '<session-fixture>'): string | undefined {
  ...
  try {
    // parseSessionLog accepts both physical rows and the projected fixture
    // form used by the ACP/Web snapshot lanes, synthesizing omitted envelopes.
  ...
  try {
    decoded = parseSessionLog(canonical)
  } catch (error) {
    const detail = error instanceof Error ? error.message : String(error)
    throw new Error(`${label}: canonical rewrite: ${detail}`, { cause: error })
  }
  try {
