...
const current = { key: 'session/current', selectedVersions: Array<number>(11).fill(SESSION_FORMAT_VERSION) }
...
describe('recorded-session corpus policy', () => {
  it('accepts a current majority and complete bounded migration coverage', () => {
    expect(assertSnapshotCorpusPolicy([
      ...
      ...adjacent,
    ])).toEqual({ currentRoles: 11, retainedRoles: 5 + adjacent.length, retainedScenarios: 3 + adjacent.length })
  })
  ...
  it('bounds historical roles and requires a current majority', () => {
    expect(() => assertSnapshotCorpusPolicy([
      ...
      ...adjacent,
    ])).toThrow(`Session corpus retains ${11 + adjacent.length} historical roles; maximum is 13`)
    expect(() => assertSnapshotCorpusPolicy([
