...
describe('npm install layout verifier', () => {
  ...
  it('checks source peer ranges before synthetic cloning rewrites them', () => {
    const index: RegistryIndex = new Map([
      ['@deepseek-ai/dsh', new Map([['1.0.0', {
        name: '@deepseek-ai/dsh',
        version: '1.0.0',
        peerDependencies: { '@deepseek-ai/dsh-child': '^0.9.0' },
      }]])],
      ['@deepseek-ai/dsh-child', new Map([['1.0.0', {
        name: '@deepseek-ai/dsh-child',
        version: '1.0.0',
      }]])],
    ])

    expect(() => assertSourceDshPeerRanges(index, '1.0.0')).toThrow(
      'invalid source DSH peer metadata',
    )
  })
