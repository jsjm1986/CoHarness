...
describe('application entrypoints', () => {
  ...
  it('rejects an example demo wrapper that launches a non-example package entry', () => {
    const root = fixture()
    ...
    expect(applicationEntrypointViolations(root)).toEqual([
      'scripts/demo-ptc.mjs: example demo wrapper must launch a packages/examples entry',
    ])
  })
