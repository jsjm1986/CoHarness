...
describe('gate graph validation', () => {
  ...
  it.each(['ci-primary', 'ci-static', 'check-all', 'hygiene'] as const)(
    'keeps the plugin surface audit in %s',
    (mode) => {
      const ids = withPnpmEntrypoint(() => gatesForMode(mode).map(subject => subject.id))

      expect(ids).toContain('plugin-surfaces')
    },
  )
  ...
  it('runs the changed packages and the incremental gate in the scoped coverage lane', () => {
    ...
    expect(gates[0]).toMatchObject({
      ...
      args: ['/private/pnpm.cjs', 'exec', 'vitest', 'run', '--coverage', '--coverage.reporter=json',
        ...
        '--maxWorkers=1', '--testTimeout=30000', '--expect.poll.timeout=30000', '--hookTimeout=30000'],
