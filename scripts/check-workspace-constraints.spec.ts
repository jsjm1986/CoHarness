...
describe('experimental workspace constraints', () => {
  ...
  it('lists exactly the local private experimental set', () => {
...
it('publishes CLI runtime declarations and shipped presets, and rejects a payload that omits them', () => {
  ...
  expect(checkWorkspaceManifest({ dir: 'apps/cli', manifest: { ...manifest, files: ['lib/*.js'] } }))
    .toEqual([expect.stringContaining('@deepseek-ai/dsh: package.json files must be ["lib/*.js","lib/types/*.d.ts","config"]')])
})

it('rejects a shared Web payload that omits the stylesheet chunks', () => {
