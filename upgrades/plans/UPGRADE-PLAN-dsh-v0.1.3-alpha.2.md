# CoHarness 对齐 deepseek-harness dsh-v0.1.3-alpha.2：选择性同步审计
...
## alpha.2 implementation decisions

- The current CoHarness worktree is the source baseline; the uncommitted multi-session Workbench is retained.
- `open-in-app` packages remain available for desktop-host deployments but are not mounted by the cloud Web bundle.
- Ordinary `SubprocessHandle.pid` is provider-private; terminal handles keep their pid.
- CoHarness message-feedback sidecar and authorization remain authoritative; upstream sidecar removal is not adopted.
- Host-native and platform-specific evidence remains pending for Windows, macOS x64, real API, cloud Gateway, and desktop Open in App.
