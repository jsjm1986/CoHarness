...
Under **Model catalog**, choose **Fetch available models** to query the base URL and credential currently shown in the form. A saved managed organization or project route reuses its encrypted key when the key field is blank; a personal route likewise keeps its stored key. Selecting candidates updates the draft; the provider is not stored until you save. Catalog providers use their installed catalog without a network request.

Discovery follows the selected protocol. `openai-completions` and `openai-responses` send bearer authentication to `GET {baseURL}/models`; `anthropic-messages` sends `x-api-key` and `anthropic-version: 2023-06-01` to `GET {baseURL}/v1/models`, then tries `{baseURL}/models` when a relay clearly omits the versioned directory. For Anthropic Messages, a trailing `/v1` is normalized for message requests, so the relay root and versioned base are accepted. For an OpenAI-compatible relay, either the root prefix or its `/v1` prefix is accepted: the host tries the entered prefix first and its one alternate only after a clear path response, then keeps the successful choice in process memory. Enter the API prefix, not a full operation path; a website response is used only as the signal to try the alternate. Non-success JSON from a relay contributes only a short redacted code/type/message diagnostic; for example, `INSUFFICIENT_BALANCE: Insufficient account balance` identifies an exhausted relay account instead of mislabeling it as a bad key.
...
- **Fetching available models returns 401** — Check the key and protocol. OpenAI-compatible discovery uses bearer-authenticated `GET {baseURL}/models`; Anthropic Messages uses `x-api-key` with `GET {baseURL}/v1/models`. A saved managed route is probed with its stored key when the form leaves the key blank.
- **Fetching available models returns HTML or `Stream ended without finish_reason`** — The host already tries the one root/`/v1` alternate for OpenAI-compatible routes when the response clearly identifies a path mismatch; Anthropic Messages also tries `/models` when `/v1/models` is absent. If both candidates fail, check that `baseURL` is the API prefix rather than a website or full operation path; for Anthropic Messages, set `api: anthropic-messages` and use the relay base so message requests reach `/v1/messages`.
