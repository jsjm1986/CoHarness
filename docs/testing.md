...
- **Web browser snapshot** (`pnpm run test:web`; impact-selected Linux PR gate): Chromium boots the real HTTP Web composition and drives `apps/web/tests/**/*.e2e.ts` and `*.snapshot.ts`. The scenarios assert browser state, protocol output, and committed snapshots; failure diagnostics may save screenshots under the ignored artifacts directory, while visual-audit screenshots are operator evidence rather than committed goldens. CI runs read-only `DSH_SNAPSHOT=replay` only when changed files can reach the Web bundle or its dependencies; documentation-only and non-browser scoped changes use the cheaper consumer path. Record/refresh stay explicit local operations and every diff is reviewed ([web e2e lane](../.agents/notes/implemented/testing/2026-07-24-web-gui-browser-e2e-lane.md), [CI gate decision](../.agents/notes/implemented/testing/2026-07-30-web-browser-snapshot-ci-gate.md)). `test:web` [builds first](../.agents/notes/implemented/bug-fix/2026-07-28-themed-scrollbars-and-reserved-gutter.md) for plugin CSS.
