...
- **Web browser snapshot** (`pnpm run test:web`; impact-selected Linux PR gate): Chromium boots the real HTTP Web composition and drives `apps/web/tests/**/*.e2e.ts` and `*.snapshot.ts` against browser state, protocol output, and committed goldens. CI runs read-only `DSH_SNAPSHOT=replay` when changed files can reach the Web bundle or its dependencies; other changes take the cheaper consumer path. Record/refresh stay local and every diff is reviewed ([web e2e lane](../.agents/notes/implemented/testing/2026-07-24-web-gui-browser-e2e-lane.md), [CI gate decision](../.agents/notes/implemented/testing/2026-07-30-web-browser-snapshot-ci-gate.md)). `test:web` [builds first](../.agents/notes/implemented/bug-fix/2026-07-28-themed-scrollbars-and-reserved-gutter.md) for plugin CSS.
