...
- **Web browser snapshot** (`pnpm run test:web`; the always-present `web verification` PR check): Chromium compares replayed browser output with `apps/web/tests/snapshots/`; CI forces read-only `DSH_SNAPSHOT=replay` and record/refresh stay local, and every golden diff is reviewed ([web e2e lane](../.agents/notes/implemented/testing/2026-07-24-web-gui-browser-e2e-lane.md), [CI gate decision](../.agents/notes/implemented/testing/2026-07-30-web-browser-snapshot-ci-gate.md), [tiering decision](../.agents/notes/implemented/testing/2026-09-11-web-verification-tiering.md)). `scripts/ci-pr-scope.ts` reads `scripts/web-test-policy.json` and picks a tier per PR — `skip`/`scoped` record why no browser work runs, `focused` replays the changed business groups plus a stable smoke set, `full` replays the whole inventory; unclassified paths and unmapped browser packages fall back to full. `test:web` [builds first](../.agents/notes/implemented/bug-fix/2026-07-28-themed-scrollbars-and-reserved-gutter.md) for plugin CSS.
