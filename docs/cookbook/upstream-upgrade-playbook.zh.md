...
每个行为或协议领域使用一行，而不是为每个上游提交或包建立一行。每行记录上游提交、CoHarness owner、本地等价实现、wire 影响、权限或数据出域影响、迁移影响、定向测试、外部证据和处置决定。
...
每次升级使用一套持久记录：plan 记录意图和取舍，manifest 记录确切提交与实现状态，alignment matrix 记录各领域证据。记录统一放在仓库的升级目录，并从 PR 链接过去。下一次升级从上一次发布提交开始，并引用之前的记录，不能覆盖历史记录。
