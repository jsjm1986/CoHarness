...
执行 Inspect Provider 显式声明的只读查询。platform、provider 和 method 必须来自 cordis_inspect_list，input 必须符合该方法的 schema。编写插件代码前用本 Tool 读取精确 Service 方法、Event mode、Builtin 签名、Tool schema、主题 token，或实时 Slot 树及 props。Host 查询在本地执行；Client 查询等待首个有效页面响应，在页面回答或 Tool 被取消前保持 pending。本 Tool 不能调用业务 Service 方法或修改运行时。查询 Service.listService 和 Event.listEvents 时，先不传 input 浏览紧凑签名目录，再查询精确 service 或 event 获取结构化约定和引用类型。查询 Slots.listSubTree 时，先不传 root 浏览紧凑树，再查询精确 root 获取完整注册约定和 props。
...
```json
{
  ...
  "properties": {
    "pluginId": {
      "type": "string",
      "description": "Existing session-owned Plugin ID or an explicit @pluginId reference; omit it to list every current Plugin."
    },
    "packageId": {
      "type": "string",
...
```json
{
  ...
  "properties": {
    "query": {
      "type": "string",
      "description": "Optional case-insensitive substring of the document name or id. Omit to list all."
    },
    "directory": {
      "type": "string",
      "description": "Optional root-relative folder, using `/` separators. Omit for the whole personal workspace."
    },
    "offset": {
      "type": "integer",
      "description": "Zero-based result offset for a capped listing. Defaults to 0."
    },
    "limit": {
      "type": "integer",
...
```json
{
  ...
  "properties": {
    "doc_id": {
      "type": "string",
      "description": "Document id returned by userdoc_list; it is relative to the personal document root."
    },
    "offset": {
      "type": "integer",
