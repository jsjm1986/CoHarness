...
The source is the published tag `dsh-v0.1.3-alpha.2`, whose writer constant is 2. The [tag record](../releases/dsh-v0.1.3-alpha.2.md) describes the adjacent release changes. This complete inventory includes unchanged roots as well as that tag’s changed roots.
...
The [V2 codec](../../../packages/session/session-persistence-jsonl/src/format.ts) owns one-event-per-row framing, range-encoded source references, and inherited cuts derived from `session/end-seed` markers. The [V2-to-V3 specification](../../../packages/session/session-format/README.md) owns system-message insertion, PTC event names, and canonical envelope conversion.
