...
来源为 `dsh-v0.1.3-alpha.1` 祖先历史中首次 V2 写入器变更之前的最后一个 V1 源码树，关联于 PR #3349（分支 `worktree/session-format-03-v0-v1-migration`）。其源码变更日期为 2026-09-02，标题为 `fix(session): close live assistant framing races`。这标识中间源码树，而非 PR 最终合并的源码树。[预发布归档](../releases/README.zh.md)没有 V1 tag，本记录不会为此虚构产品发布。
...
[V0/V1 codec](../../../packages/session/session-persistence-jsonl/src/format.ts)负责共享物理布局，包括打包的 chunk 与数值继承切点。[V1 到 V2 迁移参考](../../../packages/session/session-format/README.zh.md)负责 chunk 消耗、内嵌流与序号重映射；这些转换不能从类型摘要中推断。
