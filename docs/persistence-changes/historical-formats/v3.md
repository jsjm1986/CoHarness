...
Inspect the V3 persistence types captured at `dsh-v0.1.6-alpha.2`. The surface owns a dedicated `system/message` event, PTC dispatch events carry their production names, and the event envelope and session header declarations are revised. This snapshot retains V3 declarations before the V4 migration changes.
...
The source is the published tag `dsh-v0.1.6-alpha.2`, whose writer constant is 3. This complete inventory includes unchanged roots as well as that tag’s changed roots.
...
- `SessionHeader.version` resolves to the literal 3. The physical header is unchanged from V2: `JsonlHeaderLine` retains its V2 digest.
- `system/message` enters the surface: system prompt text moves out of `request/header.data.header.system` into its own event. The `SessionEventEnvelope` digest changes with the canonical envelope revision.
- `tool/code-dispatch` and `tool/code-dispatch-start` are renamed to `tool/ptc-dispatch` and `tool/ptc-dispatch-start`.
- `image/offload`, `deliverables/presented`, `subagent/catalog`, and `workspace/changes` enter the first-party event set.

The [V3 codec](../../../packages/session/session-persistence-jsonl/src/format.ts) owns one-event-per-row framing, range-encoded source references, and inherited cuts derived from `session/end-seed` markers. The [V3-to-V4 specification](../../../packages/session/session-format/README.md) owns the migration into the current writer.
