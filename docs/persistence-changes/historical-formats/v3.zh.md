...
查阅 `dsh-v0.1.6-alpha.2` 对应的 V3 持久化类型。Surface 拥有独立的 `system/message` 事件，PTC 派发事件携带正式名称，事件信封与 Session header 声明已修订。本快照保留 V4 迁移变更之前的 V3 声明。
...
来源为已发布 tag `dsh-v0.1.6-alpha.2`，其写入器常量为 3。本完整目录同时包含未变化的根类型和该 tag 变化的根类型。
...
- `SessionHeader.version` 解析为字面量 3。物理 header 与 V2 相同：`JsonlHeaderLine` 保持 V2 摘要。
- `system/message` 进入 surface：系统提示词文本从 `request/header.data.header.system` 移入独立事件。`SessionEventEnvelope` 摘要随规范信封修订而变化。
- `tool/code-dispatch` 与 `tool/code-dispatch-start` 更名为 `tool/ptc-dispatch` 与 `tool/ptc-dispatch-start`。
- `image/offload`、`deliverables/presented`、`subagent/catalog` 与 `workspace/changes` 进入第一方事件集合。

[V3 codec](../../../packages/session/session-persistence-jsonl/src/format.ts)负责每行一个事件的编码、范围编码的来源引用，以及从 `session/end-seed` 标记推导的继承切点。[V3 到 V4 规范](../../../packages/session/session-format/README.zh.md)负责迁入当前写入器的迁移。
...
[完整机器目录](v3.schema.json)包含 61 个根类型和 469 种可达类型。摘要包含所有引用字段；源码名称和路径描述所选的历史源码树。
...
[仅源码提取器](../../../scripts/persistence-schema.ts)运行于 TypeScript 6.0.3。重建会省略冗余的 `object` 交叉成员并丢弃规范化后不可达的类型定义；它保留已有的 `any` / `unknown`，且不会用不透明类型替代未解析的导入。

历史 surface 事件声明允许可选的 `surfaceOp`。这是已留存类型的证据，不代表可以省略 codec 或迁移校验器要求的字段。不透明值、外部插件事件、压缩正文编码与运行时关系不在类型目录覆盖范围内。

```sh
pnpm run verify-persistence-formats
```

该检查仅使用当前检出来验证当前写入器之前的所有格式、schema 完整性、双语声明与生成表格。它不重放历史应用，也不证明迁移安全性。维护规则见[格式索引](README.zh.md)，当前声明见[当前目录](../../persistence-catalog.zh.md)。
