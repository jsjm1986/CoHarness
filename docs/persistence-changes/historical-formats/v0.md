...
The source is the published tag `dsh-v0.1.2-rc.1`, whose writer constant is 0. This is the last V0 tag in the [captured prerelease archive](../releases/README.md); the [tag record](../releases/dsh-v0.1.2-rc.1.md) preserves its release context. Earlier V0 tags have additional type differences and are not replaced by this snapshot.
...
The [V0/V1 codec](../../../packages/session/session-persistence-jsonl/src/format.ts) owns packed Assistant rows and range-encoded source references. These physical body encodings are not additional roots in this declared-type inventory. The [adjacent migration reference](../../../packages/session/session-format/README.md) owns accepted legacy normalizations and refusal rules.
