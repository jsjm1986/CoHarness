---
description: "V6 后继之前 PR 218 接受的 CoHarness V5 声明。"
...
本完整目录保留 [PR #218](https://github.com/jsjm1986/CoHarness/pull/218) 中接受的声明，不包含其后的 V6 标头修正。

## 来源证据

PR #218 将写入器推进到 V5 并留下已接受的[变更记录](../2026-09-22-v5-execution.zh.md)：JSONL 读取器终于承认 V4 写入器本就输出的可选 `draft` 标头，Session 事件新增经过验证的 Gateway 执行身份。同一格式窗口还以同版本方式接受了[声明式交付与 workspace 变更事件](../2026-09-23-deliverables-events.zh.md)。V6 后续承认可选 `sshTarget` 标头，不改写本声明。
...
[完整机器目录](v5.schema.json)包含 63 个根类型和 508 种可达类型。摘要包含所有引用字段；源码名称和路径描述所选的历史源码树。
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `data` | 必需 | [`packages/context/execution-authority/src/types.ts#ExecutionEvent`](#persistence-type-packagescontextexecution-authoritysrctypestsexecutionevent) |
| `ignorable` | 可选 | `true` |
| `seq` | 必需 | `number` |
| `time` | 必需 | `number` |
| `type` | 必需 | `"gateway/execution"` |
...
来源：`packages/context/execution-authority/src/types.ts`

| 属性 | 存在性 | 类型 |
|---|---|---|
| `kind` | 必需 | `"accepted"` |
| `state` | 必需 | [`packages/context/execution-authority/src/types.ts#ExecutionState`](#persistence-type-packagescontextexecution-authoritysrctypestsexecutionstate) |
...
[`event:gateway/execution.data[0].state.actors[0]`](#persistence-type-eventgatewayexecutiondata0stateactors0) 的数组。
...
来源：`packages/attachment/attachment/src/brand.ts` · `packages/attachment/userdoc/src/brand.ts` · `packages/compaction/compaction/src/brand.ts` · `packages/context/execution-authority/src/types.ts` · `packages/core/session/src/types.ts` · `packages/experimental/agent-team/src/types.ts` · `packages/feedback/message-feedback/src/types.ts` · `packages/goal/goal/src/types.ts` · `packages/host/apiproxy/src/api/rpc.ts` · `packages/interaction/commands/src/brand.ts` · `packages/interaction/user-approval/src/types.ts` · `packages/llm/llm-retry/src/brand.ts` · `packages/llm/llm/src/brand.ts` · `packages/schedule/schedule/src/types.ts` · `packages/session/session-title/src/index.ts` · `packages/webhook/webhook/src/brand.ts` · `packages/workflow/workflow/src/types.ts`
...
该 schema 从已提交检查点生成，记录其声明类型，不声称 V5 读取器接受所有写入器输出。迁移测试另行覆盖含 `sshTarget` 和省略该字段的标头。
