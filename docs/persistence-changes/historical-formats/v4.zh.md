---
description: "V5 后继之前 PR 218 接受的 CoHarness V4 声明。"
...
本完整目录保留 [PR #218](https://github.com/jsjm1986/CoHarness/pull/218) 中接受的声明，不包含后续执行权威字段或 V5 标头修正。

## 来源证据

PR #218 引入 V4 写入器及已接受的[变更记录](../2026-09-21-v4.zh.md)。逻辑 Session 标头声明可选 `draft`，物理 JSONL 声明却遗漏它。V4 codec 委托 V3 编码器，后者在收到布尔 `draft` 时会输出它；JSONL 读取器较窄的白名单拒绝该已输出字段。V5 修复这一不一致，不改写本历史声明。
...
[完整机器目录](v4.schema.json)包含 60 个根类型和 488 种可达类型。摘要包含所有引用字段；源码名称和路径描述所选的历史源码树。
...
来源：`packages/core/session/src/types.ts` · `packages/interaction/commands/src/types.ts` · `packages/llm/llm/src/message.ts` · `packages/session/session-title/src/index.ts`
...
来源：`packages/host/apiproxy/src/api/sessions.ts`

| 属性 | 存在性 | 类型 |
|---|---|---|
| `clientTimeZone` | 可选 | `string` |
| `documents` | 可选 | [`event:agent/inbox/spliced.data.inserted[0].source[10].documents`](#persistence-type-eventagentinboxspliceddatainserted0source10documents) |
| `kind` | 必需 | `"user"` |
...
来源：`packages/attachment/userdoc/src/types.ts`
...
|---|---|---|
| `kind` | 必需 | `"project"` |
| `mode` | 必需 | [`event:agent/inbox/spliced.data.inserted[0].source[10].participant.scope[1].mode`](#persistence-type-eventagentinboxspliceddatainserted0source10participantscope1mode) |
| `projectId` | 必需 | `number` |
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `data` | 必需 | [`event:feedback/message-delete.data`](#persistence-type-eventfeedbackmessage-deletedata) |
| `ignorable` | 可选 | `true` |
| `seq` | 必需 | `number` |
| `time` | 必需 | `number` |
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `data` | 必需 | [`event:feedback/message-put.data`](#persistence-type-eventfeedbackmessage-putdata) |
| `ignorable` | 可选 | `true` |
| `seq` | 必需 | `number` |
| `time` | 必需 | `number` |
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `data` | 必需 | [`packages/host/apiproxy/src/api/sessions.ts#ModelSelection`](#persistence-type-packageshostapiproxysrcapisessionstsmodelselection) |
| `ignorable` | 可选 | `true` |
| `seq` | 必需 | `number` |
| `time` | 必需 | `number` |
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `origin` | 可选 | [`event:permission/preset.data.origin`](#persistence-type-eventpermissionpresetdataorigin) |
| `preset` | 必需 | `string` |
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `data` | 必需 | [`packages/session/session-title/src/index.ts#SessionTitleEventData`](#persistence-type-packagessessionsession-titlesrcindextssessiontitleeventdata) |
| `ignorable` | 可选 | `true` |
| `seq` | 必需 | `number` |
| `time` | 必需 | `number` |
...
|---|---|---|
| `kind` | 必需 | `"provider"` |
| `model` | 可选 | [`packages/session/session-title/src/index.ts#SessionTitleModelIdentity`](#persistence-type-packagessessionsession-titlesrcindextssessiontitlemodelidentity) |
| `provider` | 必需 | `string` |
...
[`packages/session/session-title/src/index.ts#SessionTitleModelIdentity`](#persistence-type-packagessessionsession-titlesrcindextssessiontitlemodelidentity) 的数组。
...
[`packages/core/session/src/types.ts#TodoItem`](#persistence-type-packagescoresessionsrctypeststodoitem) 的数组。
...
来源：`packages/core/session/src/json.ts`
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `data` | 必需 | [`packages/context/userdoc-context/src/index.ts#UserDocAttachedEventData`](#persistence-type-packagescontextuserdoc-contextsrcindextsuserdocattachedeventdata) |
| `ignorable` | 可选 | `true` |
| `seq` | 必需 | `number` |
| `time` | 必需 | `number` |
| `type` | 必需 | `"userdoc/attached"` |
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `ref` | 必需 | [`packages/attachment/userdoc/src/types.ts#UserDocRef`](#persistence-type-packagesattachmentuserdocsrctypestsuserdocref) |
| `representation` | 必需 | [`packages/attachment/userdoc/src/types.ts#UserDocPromptRepresentation`](#persistence-type-packagesattachmentuserdocsrctypestsuserdocpromptrepresentation) |
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `imageIndexes` | 必需 | [`SessionEventEnvelope[38].sourceEventSeqs`](#persistence-type-sessioneventenvelope38sourceeventseqs) |
| `seq` | 必需 | `number` |
...
来源：`packages/context/userdoc-context/src/index.ts`
...
|---|---|---|
| `content` | 必需 | [`event:agent/inbox/spliced.data.inserted[0].content`](#persistence-type-eventagentinboxspliceddatainserted0content) |
| `delivery` | 必需 | [`event:team/message/queued.data.message.delivery`](#persistence-type-eventteammessagequeueddatamessagedelivery) |
| `id` | 必需 | `string` |
| `senderId` | 必需 | `string` |
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `category` | 可选 | [`event:feedback/record.data.category`](#persistence-type-eventfeedbackrecorddatacategory) |
| `text` | 可选 | `string` |
...
| `role` | 必需 | [`event:agent/inbox/spliced.data.inserted[0].source[10].participant.role`](#persistence-type-eventagentinboxspliceddatainserted0source10participantrole) |
| `scope` | 必需 | [`event:agent/inbox/spliced.data.inserted[0].source[10].participant.scope`](#persistence-type-eventagentinboxspliceddatainserted0source10participantscope) |
| `userId` | 必需 | `number` |
| `username` | 必需 | `string` |
...
来源：`packages/session/session-title/src/index.ts` · `packages/subagent/tool-subagent/src/model-selection.ts`
...
来源：`packages/attachment/attachment/src/brand.ts` · `packages/attachment/userdoc/src/brand.ts` · `packages/compaction/compaction/src/brand.ts` · `packages/core/session/src/types.ts` · `packages/experimental/agent-team/src/types.ts` · `packages/feedback/message-feedback/src/types.ts` · `packages/goal/goal/src/types.ts` · `packages/host/apiproxy/src/api/rpc.ts` · `packages/interaction/commands/src/brand.ts` · `packages/interaction/user-approval/src/types.ts` · `packages/llm/llm-retry/src/brand.ts` · `packages/llm/llm/src/brand.ts` · `packages/schedule/schedule/src/types.ts` · `packages/session/session-title/src/index.ts` · `packages/workflow/workflow/src/types.ts`
...
该 schema 从已提交检查点生成，记录其声明类型，不声称 V4 读取器接受所有写入器输出。迁移测试另行覆盖含 `draft` 和省略该字段的标头。
