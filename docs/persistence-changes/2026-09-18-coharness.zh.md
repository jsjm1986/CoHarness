...
[English](2026-09-18-coharness.md) | 中文

## 概述

在 dsh-v0.1.6-alpha.2 对齐期间确立完整的 CoHarness 持久化类型清单，作为分叉的跟踪基线。
...
基线仅记录当前 schema，不改变任何读写行为。已提交的 Session 代次仍由现有格式版本迁移目录治理。分叉的事件词表自此快照起跟踪自身谱系；兼容性声明适用于本地代次，而非上游发布历史。

<a id="verification"></a>
## 验证

verify-persistence-changes、verify-persistence-catalog、verify-persistence-formats 与 verify-persistence-releases 在重新生成的产物上通过。
