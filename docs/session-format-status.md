...
The bilingual pairing check verifies record equality across languages. No local check queries GitHub or proves that the record is up to date; publication verification remains part of the release update.

Use “current format” and “next adjacent version” for general behavior. Keep explicit numbers for fixed migration inputs and outputs, wire schemas, historical evidence, and tests of those particular versions. Use N for the verified latest released format and N+1 for its successor when describing a version transition.
