...
Webhook 子系统会把已通过身份验证的外部交付转换为可选的普通根 Session。提供方适配器拥有身份验证与通用 JSON 接收；受信任的程序化规则拥有条件与外部调用；`ctx.webhookRuntime` 拥有回调生命周期以及基于 Workspace 的 Session 创建。[执行身份决策](../../.agents/notes/implemented/architecture/2026-09-23-webhook-execution-identity.zh.md)区分此进程内运行时与受管 Gateway 接收。
...
`invoke(ruleId, delivery, signal?)` 等待一个已注册规则，返回已接纳提示词的 Session id；不采取动作时返回 `null`。它传播失败，并结合调用方取消与注册卸载。返回值证明提示词已接收，不证明 Agent 轮次完成。
...
受管部署在创建 Session 时通过当前执行授权服务为输入登记身份，并在应用权限或接收提示词之前核验权限预设。缺少身份、资格撤销、授权服务替换和取消均拒绝接收并回滚。独立本机部署保留本机操作者权限。
...
独立部署可把该路由挂载在隔离的第二个 WebServer 上，因此暴露 webhook 入口不会暴露浏览器 API。交付的 profile 不启用端点。此适配器不提供 Gateway 端点管理、跨节点持久去重或执行账号绑定。
