...
A managed deployment can bind one session to a registered SSH target through `SessionHeader.sshTarget`: the API proxy mounts the providers inside a subject-scoped standing realm before the preset composition loads, persists the binding on the header, and re-authorizes every joining caller. See the [session-binding decision](../../.agents/notes/implemented/architecture/2026-09-24-session-bound-ssh-execution.md) for the realm mechanics and authorization model.

See the [decision record](../../.agents/notes/implemented/architecture/2026-09-23-ssh-execution-providers.md) for the alternatives and verification obligations.
