...
`subagentCatalog` 投影通过 Session 观察与客户端快照按父级事件顺序暴露 `SubagentCatalogEntry[]`。每个条目包含子级 id、创建时间、模式与随模式而定的标签；fork 继承的 catalog 事实会被排除。[subagent 包](../../packages/subagent/subagent/README.zh.md)持有 catalog 的创建与持久化语义。
...
Types: [AdmittedPromptContentPart](attachment.zh.md) · [Agent](core.zh.md)
