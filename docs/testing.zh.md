...
本文说明测试层级与证据要求。命令见 [AGENTS.md](../AGENTS.md)，设计动机见链接的 Agent Notes。
...
- **单元测试**（`pnpm run test`）：Vitest 运行位于所属代码旁的包／示例 `tests/**` 与 `scripts/**/*.spec.ts`。每个注册表需要 HMR（热模块替换）安全测试：释放贡献内容的 fiber 并断言清理完成。优先覆盖边界情况、失败、顺序、竞态与永久回归；见 `packages/core/agent-loop/tests/contract-regressions.spec.ts`。
- **覆盖率门禁**（`pnpm run test:coverage`）：门禁级运行，对 `packages/*/*/src` 按文件 100% 覆盖。未覆盖的行可能是应删除的死代码。覆盖率是必要条件，但不能证明交付行为。`packages/shell/pwsh-local/src` 需要真实的 `pwsh`：无 `pwsh` 的主机上执行器套件自动跳过且 `vitest.config.ts` 豁免该文件；CI 自带 pwsh 并按完整标准执行门禁。
