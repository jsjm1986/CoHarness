...
- **Gateway 插件**（`pnpm run test:plugins`）：插件测试套件为必需 CI 检查；插件变更触发 Gateway 集成检查（[详情](../.agents/notes/implemented/testing/2026-09-17-required-gateway-plugin-tests.zh.md)）。

- **单元测试**（`pnpm run test`）：vitest 运行包和示例的 `tests/**` 测试与 `scripts/**/*.spec.ts`；测试文件与其所覆盖的代码放在一起。每个注册表都有一个 HMR（热模块替换）安全测试（对向该注册表贡献内容的 fiber 执行 dispose（资源释放），并断言清理完成）。优先覆盖边界情况、错误路径、顺序、竞态，以及针对约定回归的永久测试（`packages/core/agent-loop/tests/contract-regressions.spec.ts`）。
...
分叉（fork）worker 在共享自托管运行器上并发运行 spec 与覆盖率分区；被隔离的只有进程——端口、可预测路径、外部命名空间与被继承的子进程都不在隔离范围内。每个获取的资源都必须经清理释放；一个只有单独运行才能通过的 spec 是缺陷。[dsh-ci-test-reliability](../.agents/skills/dsh-ci-test-reliability/SKILL.md) 负责资源、同步、超时、平台与清理规则及[故障诊断](../.agents/skills/dsh-ci-test-reliability/references/ci-flake-diagnosis.md)。
...
- 一个守卫只有在回归真的能让它失败时才有效。对于没有 `inject` 的插件（bundle/组合插件），Loader 冒烟测试在默认导出替换必需的具名导出时仍然绿着——添加 `expect('default' in mod).toBe(false)` 加 `unwrapExports` 往返断言，先证明回归变红再回退。
...
每项非平凡的模型可见、协议可见或人类可见变更，都必须在同一 PR 中，经由可运行示例所属套件添加或更新无密钥场景。包测试、e2e 断言、mock 与仅测试组合、PR 理由都不能取代组装后的 transcript；必要时应扩展 harness。ACP 自动化场景使用 `examples/<name>/tests/snapshots/`，即基于 [`dsh-acp-snapshot`](../packages/test-support/acp-snapshot/README.zh.md) 套件工厂的场景表（`examples/acp-agent` 为主套件，`pwsh-tool-turn` 启动真实 `pwsh`）；`examples/headless-agent` 拥有内部规范事件 JSONL 快照与回放 fixture。已完成的交互式终端旅程使用 `apps/cli/tests/snapshots/` 下由 JSONL 驱动的场景；瞬态呈现使用包内语义矩阵，并在输入、Loader 或清理变化时添加 PTY 用例。浏览器渲染的 Web GUI 旅程使用上述 Web 应用快照套件。两个 SDK 各自独立地投影 agent loop、会话生命周期与 `SessionEventMap`，因此改动其中任何一项都要同时更新两者：`examples/jsonrpc-agent/tests/snapshots/` 拥有 TypeScript 客户端；`scripts/snapshots/python-sdk-single-exe/` 拥有 Python 客户端，且只有必需的 `python-runtime` CI 作业会运行它。新的能力 seam、生命周期变体或 transcript 呈现接口在计划阶段就要列出每个覆盖层级，并在实现前验证 harness 能够表达它们。

## 验证范围与证据

[诚实性检查](../scripts/verify-test-honesty.ts)要求覆盖率 ignore 和无条件跳过提供理由。CI 拒绝陈旧 lint 抑制，本地 `--fix` 保留诊断警告。[消费方关系](../scripts/ci-consumer-relations.json)解释额外的 fork 检查。[发布证据](../scripts/release/README.zh.md)绑定候选、环境和已测试产物。
