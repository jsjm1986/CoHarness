...
- **Web 浏览器快照**（`pnpm run test:web`；按影响范围选择的 Linux PR（Pull Request）门禁）：Chromium 启动真实 HTTP Web 组合，并驱动 `apps/web/tests/**/*.e2e.ts` 与 `*.snapshot.ts`。场景会断言浏览器状态、协议输出和已提交快照；失败诊断可能把截图保存到被忽略的 artifacts 目录，视觉审查截图则是操作员证据，不是已提交的 golden。只有变更文件可能影响 Web bundle 或其依赖时，CI 才以只读 `DSH_SNAPSHOT=replay` 运行该 lane；纯文档和不触及浏览器的限定范围变更使用成本更低的 consumer 路径。record/refresh 仍是本地显式操作，每处 diff 都须评审（[Web e2e 车道](../.agents/notes/implemented/testing/2026-07-24-web-gui-browser-e2e-lane.zh.md)、[CI 门禁决策](../.agents/notes/implemented/testing/2026-07-30-web-browser-snapshot-ci-gate.zh.md)）。`test:web` 会[先构建](../.agents/notes/implemented/bug-fix/2026-07-28-themed-scrollbars-and-reserved-gutter.zh.md)以交付插件 CSS。
