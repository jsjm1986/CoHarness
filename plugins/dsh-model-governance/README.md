...
Personal runtimes combine authorized organization routes with user-declared personal BYOK routes. Project runtimes set `userDeclaredAllowed: false`; their model permissions come only from project assignments, so every member of one project runtime sees the same organization model set.
