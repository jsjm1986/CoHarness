...
Policy and usage records contain no API key or prompt/response content. Organization credentials stay encrypted in the Gateway database and cross the authenticated loopback runtime API only when an adapter resolves their reference. Credential source is a non-secret layer id used only to distinguish company and personal cost. Shared-project usage may carry the verified participant user and project ids found in the durable request messages; the Gateway checks the project scope and uses these ids only for activity reporting, not billing ownership. If actor verification fails, the outbox retries the same event without activity fields so project billing is still recorded as unattributed. UUID-named outbox files are committed by same-directory rename and removed only after a successful intake response; intake deduplication makes retries safe.
