...
Policy and usage records contain no API key or prompt/response content. Organization credentials stay encrypted in the Gateway database and cross the authenticated loopback runtime API only when an adapter resolves their reference. Credential source distinguishes company and personal cost. Managed model requests use the actual initiating Agent’s execution authority, or immutable witnesses supplied by an already authorized Auto review. The outbox records one primary actor and `executionInputIds`; a request with several participants still creates one bill. Cold auxiliary requests without a live Agent remain explicitly unattributed. Missing managed authority or incomplete Auto attribution refuses adapter dispatch.

The PostgreSQL intake validates witness membership in the named Session and runtime and the primary actor’s historical participation. Revocation or account deletion cannot erase costs incurred by that proven input; these historical facts confer no execution authority. Project records retain project billing ownership and one activity actor, while personal rows retain only the owner as their billing user. Invalid proof stays in the outbox and never retries after stripping attribution. Legacy records without proof retain the existing current-writer check and actorless retry. UUID-named files are removed only after intake success; event deduplication prevents duplicate charges.
...
A forbidden route terminates the stream with `MODEL_FORBIDDEN` before provider dispatch. An initiating Agent whose identity disagrees with an explicit `sessionId` terminates with `MODEL_ATTRIBUTION_CONFLICT`. Missing or refused execution attribution terminates with `MODEL_EXECUTION_IDENTITY_FAILED`. The plugin adds no prompt content.
