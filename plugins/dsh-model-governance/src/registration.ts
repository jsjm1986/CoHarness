...
import type { ModelRegistrationRecord } from './outbox.ts'
...
type ProviderMap = Record<string, ProviderSnapshot>
...
function modelSnapshot(value: unknown): { id: string; snapshot: ModelSnapshot } | undefined {
...
function profileSnapshot(value: unknown): ProviderSnapshot {
  ...
  for (const raw of rawModels) {
    ...
    if (model !== undefined) models[model.id] = model.snapshot
  }
  const withoutModels: JsonObject = {}
  for (const [key, entry] of Object.entries(profile)) {
    ...
    if (key === 'models' || key === 'apiKeyEnv' || key === 'headers' || key === 'credential') continue
...
export function registrationSnapshot(namespace: string, value: unknown): ProviderMap | undefined {
  if (!TRACKED_NAMESPACES.has(String(namespace))) return undefined
...
function event(
  occurredAt: number,
  provider: string,
  action: ModelRegistrationRecord['action'],
  model?: string,
  identity?: string,
...
export function baselineRegistrations(
  namespace: string,
  userLayer: unknown,
  occurredAt = Date.now(),
): ModelRegistrationRecord[] {
  ...
  const result: ModelRegistrationRecord[] = []
  for (const [provider, profile] of Object.entries(snapshot)) {
    result.push(event(occurredAt, provider, 'provider-created', undefined,
      `${namespace}\0provider\0${provider}\0${profile.fingerprint}`))
    for (const [model, details] of Object.entries(profile.models)) {
      result.push(event(occurredAt, provider, 'model-created', model,
        `${namespace}\0model\0${provider}\0${model}\0${details.fingerprint}`))
    }
  }
  return result
}
...
export function diffRegistrations(
  ...
  for (const provider of providers) {
    ...
    if (oldProvider === undefined && newProvider !== undefined) {
      result.push(event(occurredAt, provider, 'provider-created', undefined,
        `${namespace}\0provider\0${provider}\0${newProvider.fingerprint}`))
    ...
    if (oldProvider !== undefined && newProvider === undefined) {
      ...
      for (const model of Object.keys(oldProvider.models)) result.push(event(occurredAt, provider, 'model-deleted', model))
      continue
    }
    if (oldProvider === undefined || newProvider === undefined) continue
    if (oldProvider.fingerprint !== newProvider.fingerprint) result.push(event(occurredAt, provider, 'provider-modified'))
    ...
    for (const model of models) {
      ...
      if (beforeModel === undefined && afterModel !== undefined) result.push(event(occurredAt, provider, 'model-created', model))
      else if (beforeModel !== undefined && afterModel === undefined) result.push(event(occurredAt, provider, 'model-deleted', model))
