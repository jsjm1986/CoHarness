...
function nodeAt(root: Record<string, unknown>, path: readonly string[]): unknown {
 ...
 */
export function userDeclaredProviders(llm: LlmRuntime, settings: SettingsProvider): Set<string> {
  const userLayers = new Map<string, unknown>(settings.describe().map(descriptor => [descriptor.ns, descriptor.user]))
  ...
  for (const entry of llm.listConfigurableProviders()) {
    ...
    if (present) declared.add(entry.provider)
  }
  return declared
}
...
export class UserDeclaredRoutes {
  private providers: ReadonlySet<string> = new Set()
   ...
   * @param settings - the settings service holding the namespace descriptors.
   */
  refresh(llm: LlmRuntime, settings: SettingsProvider): void {
