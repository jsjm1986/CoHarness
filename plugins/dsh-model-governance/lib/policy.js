...
function validateEmbeddedProfile(value, field, provider, protocol, baseURL, credentialRef) {
...
function providerProfiles(value) {
    ...
    return value.map((entry, index) => {
        ...
        if (!ORGANIZATION_PROVIDER_PATTERN.test(provider)) {
            throw new Error(`model-governance: providers[${String(index)}].provider must match ${String(ORGANIZATION_PROVIDER_PATTERN)}`);
        }
        ...
        const profile = validateEmbeddedProfile(row.profile, `providers/${String(index)}.profile`, provider, protocol, baseURL, credentialRef);
