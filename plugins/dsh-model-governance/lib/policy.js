...
function optionalPositiveTimer(value, field) {
    if (value === undefined)
        return undefined;
...
function modalities(value, field) {
    if (value === undefined)
        return undefined;
    if (!Array.isArray(value) || value.length === 0 || value.some(entry => typeof entry !== 'string' || !MODALITIES.has(entry))) {
        throw new Error(`model-governance: ${field} must contain text or image`);
    }
    return [...value];
}
function reasoningEfforts(value, field) {
...
function compat(value, field, protocol) {
    if (value === undefined)
        return undefined;
    const source = record(value, field);
    ...
    if (source.thinkingFormat !== undefined && !THINKING_FORMATS.has(source.thinkingFormat)) {
...
function stringMap(value, field) {
    ...
    for (const [key, entry] of Object.entries(source)) {
        if (typeof entry !== 'string')
            throw new Error(`model-governance: ${field}.${key} must be a string`);
        result[key] = entry;
    }
    return result;
}
function objectCopy(value, field) {
    if (value === undefined)
        return undefined;
...
function validateOrganizationModelOverrides(profile, field) {
...
function validateEmbeddedProfile(value, field, provider, protocol, baseURL, credentialRef, credentialPattern) {
    ...
    if (profile.displayName !== undefined)
        requiredString(profile.displayName, `${field}.displayName`);
    ...
    if (profile.reasoning !== undefined) {
        ...
        if (!REASONING_LEVELS.has(reasoning))
            throw new Error(`model-governance: ${field}.reasoning is invalid`);
    }
    ...
    if (profile.cacheRetention !== undefined) {
        ...
        if (!CACHE_RETENTIONS.has(retention))
            throw new Error(`model-governance: ${field}.cacheRetention is invalid`);
    }
    if (profile.transport !== undefined) {
        const transport = requiredString(profile.transport, `${field}.transport`);
        if (!TRANSPORTS.has(transport))
            throw new Error(`model-governance: ${field}.transport is invalid`);
    }
    ...
    if (profile.retryPolicy !== undefined) {
        ...
        resolveRetryPolicy(retry, `model-governance: ${field}.retryPolicy`);
    }
    return profile;
}
function modelProfile(value, provider, modelIndex, protocol) {
...
function providerProfiles(value) {
    if (!Array.isArray(value))
        throw new Error('model-governance: providers must be an array');
    const seen = new Set();
    return value.map((entry, index) => {
        ...
        const scope = declaredScope === undefined
        ...
        if (!providerPattern.test(provider)) {
            throw new Error(`model-governance: providers[${String(index)}].provider must match ${String(providerPattern)}`);
        }
        ...
        if (seen.has(provider))
            throw new Error(`model-governance: duplicate Provider ${provider}`);
        seen.add(provider);
        if (row.driver !== 'pi-ai')
            throw new Error(`model-governance: providers[${String(index)}].driver must be pi-ai`);
        if (typeof row.protocol !== 'string' || !PROTOCOLS.has(row.protocol)) {
        ...
        const models = row.models.map((model, modelIndex) => {
            ...
            if (modelIds.has(normalized.id))
                throw new Error(`model-governance: duplicate model ${provider}/${normalized.id}`);
        ...
        const profile = validateEmbeddedProfile(row.profile, `providers/${String(index)}.profile`, provider, protocol, baseURL, credentialRef, credentialPattern);
        ...
        const retryPolicy = row.retryPolicy === undefined ? undefined : (() => {
            ...
            resolveRetryPolicy(source, path);
        ...
        return {
            ...
            protocol,
            baseURL,
            ...scope === 'project' ? { scope: 'project', projectId: projectId } : {},
            ...
            ...cacheRetention === undefined ? {} : { cacheRetention: cacheRetention },
            ...
            ...retryPolicy === undefined ? {} : { retryPolicy: retryPolicy },
            models,
        };
    });
}
