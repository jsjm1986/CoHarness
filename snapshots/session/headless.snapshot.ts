...
async function verifyProviderCwdResume(
  ...
  const env = snapshotChildEnvironment('replay', {
...
describe('headless recorded-session snapshots', () => {
  ...
  for (const scenario of scenarios) {
    ...
    scenarioTest(`${mode}s ${scenario.name} through dsh --profile headless`, async ({ expect }) => {
      ...
      try {
        result = await runLoaderSmoke({
          ...
          expectedExitCode: turnReasonFromSession(primaryFixture)?.kind === 'completed'
            ...
            ? 0
            : 1,
          env: snapshotChildEnvironment(mode, {
      ...
      for (const [index, actual] of actualSnapshots.entries()) {
        expect.soft(records(actual), `${scenario.name}: session ${index}`).toEqual(records(expectedSnapshots[index] as string))
      }
