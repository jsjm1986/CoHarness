...
Admin 的**归档**频道从 Gateway 归档索引列出组织级根对话。它支持按状态、标题／正文／Session ID、用户和项目筛选，打开分页的完整事件阅读器，导出 JSON，并通过确认弹窗批量恢复、移入回收站或永久清理。个人正文仍由所属运行时保存并按需读取；项目正文使用 PostgreSQL。每次查看、导出和变更都会写入审计，但审计行不复制消息正文；运行时归档快照携带 revision，Admin 离线变更会在运行时恢复后对账。
