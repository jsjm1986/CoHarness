...
The document catalog migration is numbered `012_document_catalog.sql` because the base branch owns migration 011. `017_document_trash_lifecycle.sql` adds recoverable `trash` and terminal `purged` states, retention timestamps, organization-scoped actor references, and the purge index; legacy `deleted` rows enter the 30-day recovery window during that migration. `018_document_catalog_tenant_fks.sql` keeps every catalog user and project reference qualified by organization and adds indexes for active modified-time and name searches. `019_conversation_archive_file_cleanup.sql` records local archive-file cleanup tasks with leases and retry timing, so a database purge remains durable when the filesystem is temporarily unavailable.
...
Migration `023_user_conversation_display_preferences.sql` adds nullable account-owned transcript width and font-size columns. NULL preserves the existing defaults, so the migration does not rewrite conversation events or change existing preference choices.

Migration [030](migrations/030_execution_identity.sql) stores runtime-scoped Session lineage, immutable authenticated input records, and monotonic execution actor sets. Input consumption marks only the first admission; it cannot change the input's creator, content hash, or inherited actors. Initial-inheritance hashes and receiver-message relay records make retries preserve later attribution. These records survive runtime restarts and retain historical usage attribution after account access is revoked. Include them in the same database backup as Session metadata and accounting; personal JSONL bodies remain outside PostgreSQL. They contain identifiers and hashes, not message bodies or runtime credentials.
...
The importer is transactional and repeatable for one organization. It preserves password hashes, users, projects, origin/owner metadata, mounts, stopped personal instance assignments, memberships, pending and completed project invitations, model policy, prices, quotas, usage, alerts, and audit rows. Imported invitation UUIDs are derived from the organization and legacy SQLite invitation id, so a repeated import updates the same row. Migration 3 makes each existing project creator an `rw` member when needed; administrator-origin legacy projects remain administrator-origin unless the SQLite row explicitly carries user ownership. Gateway startup serializes first-free port allocation with a node-scoped PostgreSQL advisory lock and creates missing runtime rows for active mounted projects from `HGW_INSTANCE_PORT_BASE`; removed operational instance rows release their node-local ports, while schema SQL never embeds deployment port numbers. Login sessions, lockout attempts, runtime/intake tokens, and existing JSONL/Zstd conversations are deliberately not migrated; users sign in again, credentials are reissued, personal transcripts stay in their current session directories, and collaborative PostgreSQL history begins with project-scope conversations created after cutover.

SQLite imports accept schema v7 and v8 without modifying the source. A v7 source imports every user with Auto review eligibility disabled; a v8 source preserves its explicit eligibility values and rejects a missing or invalid field. PostgreSQL migration [031](migrations/031_auto_review_eligibility.sql) adds the default-false user grant and includes grant changes in the existing access-invalidation trigger.

Migrations [032](migrations/032_desktop_qualification.sql)–[042](migrations/042_webhook_repository_filter.sql) cover the managed-execution surfaces: desktop qualification policies and per-session confirmations (032/033), per-user terminal qualification policies and revocable terminal-session grants (034/035), webhook delivery receipts with intake windows (036), body-replay aliases binding every seen provider delivery id to one receipt (037), SSH target registration with per-user qualification policies and project sharing (038), administrator-managed webhook endpoints storing structured provider rules, the execution account and runtime target, intake limits, replay windows, and a write-only AES-256-GCM signing secret (039; it also adds the retained verified event payload used by administrator redispatch to `webhook_delivery_receipts`), session-recorded SSH target bindings that keep resumed sessions on the registered host and stop draft reservations from materializing under another target (040), the deployment control plane holding maintenance windows, writer-convergence epochs, an operation ledger, and the backup registry (041), and the endpoints' structured `owner/repo` repository filter (042). Qualification-policy changes ride the existing access-invalidation outbox so connected runtimes re-resolve without restart; webhook secrets decrypt only inside the Gateway and never appear in administrative views.
