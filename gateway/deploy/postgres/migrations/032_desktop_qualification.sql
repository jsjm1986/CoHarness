-- Desktop qualification is independent of model permission presets and resource leases.
CREATE TABLE harness.desktop_access_policies (
...
CREATE UNIQUE INDEX desktop_user_policy ON harness.desktop_access_policies(organization_id,user_id) WHERE user_id IS NOT NULL;
CREATE UNIQUE INDEX desktop_project_policy ON harness.desktop_access_policies(organization_id,project_id) WHERE project_id IS NOT NULL;

CREATE FUNCTION harness.capture_desktop_access_invalidation()
...
END;
$$;
CREATE TRIGGER desktop_access_changes AFTER INSERT OR UPDATE OR DELETE ON harness.desktop_access_policies
  FOR EACH ROW EXECUTE FUNCTION harness.capture_desktop_access_invalidation();
