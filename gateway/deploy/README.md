...
1. Copy the built `gateway/` directory to `/srv/harness/gateway` and `packages/llm/llm/` to `/srv/harness/packages/llm/llm`, preserving that relative layout; run `npm install && npm rebuild better-sqlite3 argon2` in the Gateway directory with the production Node. Gateway runs through `tsx` and maps `@deepseek-ai/dsh-llm/discovery` to that sibling package's source, so organization model discovery uses the same implementation as the Harness without depending on a separately published npm version. `public/admin` is gitignored, so copy from the checkout only after `build:production` has produced it.
