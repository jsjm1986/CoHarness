...
- File descriptors sized to the watched tree on hosts that run the file watcher (HMR profiles or `watch` sessions): the chokidar watcher is verified on macOS 26 (arm64) under a `1048575`-file soft limit with no `EMFILE` (errno -24); a startup `EMFILE` means raising `ulimit -n`, not a watcher defect.
- CPython 3.10 or newer on `PATH`, or an absolute `pythonBin` path, only when a profile mounts the experimental `@deepseek-ai/dsh-experimental-ptc-runtime-python`; the runtime probes the interpreter at load and rejects non-CPython or pre-3.10 binaries, so an older system `python3` cannot be picked up silently (verified: host `python3` 3.9.6 rejected, 3.12.13 passes the runtime suite).
...
1. Copy the built `gateway/` directory to `/srv/harness/gateway`, `packages/llm/llm/` to `/srv/harness/packages/llm/llm`, and `packages/session/session-format/` to `/srv/harness/packages/session/session-format`, preserving that relative layout; `build:production` emits the Gateway entry and its local modules under `gateway/lib/` and creates the relative `gateway/node_modules/@deepseek-ai/dsh-llm` and `gateway/node_modules/@deepseek-ai/dsh-session-format` links required by the compiled ESM graph. Run `npm install && npm rebuild better-sqlite3 argon2` in the Gateway directory with the production Node, then start `lib/index.js` with plain Node. The compiled Gateway keeps `@deepseek-ai/dsh-llm/discovery` as a package import, so the sibling package's built `lib/` and generated link are part of the release; no source `tsx` loader is used for a new activation. The release controller still accepts a legacy source-only release when it is the rollback target. `public/admin` is gitignored, so copy from the checkout only after `build:production` has produced it.
...
6. A database with no users causes first boot to create `/srv/harness/gateway-data/bootstrap-admin-password` (mode `0600`) and log only its path. Read it as root, sign in once, change the password, and remove the file with `shred -u` or an equivalent owner-only deletion. Existing deployments that exposed the password in the journal must rotate that administrator credential manually and review retained logs.

The Admin Archives channel is enabled by PostgreSQL migration 015. Set `HGW_ARCHIVE_RETENTION_DAYS` (default `30`) for the recoverable trash window. Runtime archive snapshots are reconciled through the private Gateway API; keep runtime homes and the Gateway archive index on the same backup plan. Personal transcript bodies remain in runtime-owned storage and are loaded on demand by an administrator view.

Managed desktop is opt-in per node. Install the Cua Driver application on the host and grant it the OS desktop permissions, provision the published `@deepseek-ai/dsh-experimental-computer-use-cua-driver-mcp` package directory (copy the built package to `packages/experimental/computer-use-cua-driver-mcp` under the release root, or point `HGW_DESKTOP_DRIVER_PACKAGE` at an absolute package directory), and set `HGW_DESKTOP_ID` to the node-local desktop identifier. Every launched runtime then mounts the computer-use service and that provider and publishes the managed desktop policy; `HGW_DESKTOP_DRIVER_COMMAND` and `HGW_DESKTOP_DRIVER_ARGS` replace the `cua-driver` lookup and `mcp` arguments when the deployment needs a pinned path or `--direct`. Administrators qualify users and projects on the Desktop coordination page, and each participant confirms the live root Session and desktop in the composer before a driver call runs. With `HGW_DESKTOP_ID` unset, runtimes mount no driver and the driver package is never read.
