...
1. Copy the built `gateway/` directory to `/srv/harness/gateway` and `packages/llm/llm/` to `/srv/harness/packages/llm/llm`, preserving that relative layout; `build:production` emits the Gateway entry and its local modules under `gateway/lib/`. Run `npm install && npm rebuild better-sqlite3 argon2` in the Gateway directory with the production Node, then start `lib/index.js` with plain Node. The compiled Gateway keeps `@deepseek-ai/dsh-llm/discovery` as a package import, so the sibling package's built `lib/` is part of the release; no source `tsx` loader is used in production. `public/admin` is gitignored, so copy from the checkout only after `build:production` has produced it.
