...
Working-language fragments and session separators are transcription residue. The Figma frame name stays: external attribution that resolves outside the repo by design.
...
The measurement pins the constant against uninformed retuning, and "measured" is the attribution that distinguishes data from a guess.
...
### Dropping attribution while keeping the number
