...
- **Measured bounds** — "(measured: 512 nests ≈ 0.15s)" calibrating a constant; the attribution word `measured` is load-bearing.
...
4. Before deleting anything, enumerate the passage's propositions (prose-standard) and check the [overcorrection traps](references/examples.md#overcorrection-traps): trims that flip an obligation into an endorsement, promote a hypothetical to a shipped feature, delete a true fact, or drop attribution.
