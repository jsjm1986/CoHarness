...
- workflow-ptc: `setTimeout(ms, undefined, { ref: false })` — exact semantics including not holding the event loop open.
...
- The `llm-retry`, `workflow-ptc`, and `terminal-bash` test suites pass unchanged (behavioral parity).
