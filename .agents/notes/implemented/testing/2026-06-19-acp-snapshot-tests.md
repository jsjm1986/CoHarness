...
Session V3 fixtures retain the durable `system/message` projection and compact assistant-attempt stream records. Snapshot helpers scrub prompt bodies only for structural header comparisons, retain full prompts in their dedicated sidecars, and zero compact stream clocks while preserving chunk order and payloads. Python runtime snapshots apply the same prompt and stream normalization so SDK consumers exercise the released event vocabulary.
