...
当前 Trajectory 验收上限为 850 ms，由 680 ms 目标与共享的 1.25× 余量表示。原先的 650 ms 上限（520 ms 目标）处于实测托管分布内部：运行 35095609422 记录中位数 666.651723 ms 和 630.843184 ms，分支运行 36208631023 和 36215749263 记录中位数 629.515400 ms 和 658.402183 ms，逐样本值跨度为 596.868403–664.735862 ms。该目标对最大实测中位数向上取整；记录样本覆盖接受区间，同一个判定断言拒绝 851 ms。其他终点预算与工作负载参数保持不变。
