...
[单次](../../../../examples/acp-agent/tests/snapshots/pwsh-tool-turn/session.v6.jsonl)与[持久](../../../../examples/acp-agent/tests/snapshots/persistent-pwsh-tool-turn/session.v6.jsonl) fixture 及其拥有的 header pin 使用真实 PowerShell 可执行文件和已录制模型回复，经构建后的 headless profile 刷新。策略事件和可用工具保留在预期中；工具结果与最终回复仍为 `PWSH_OK` 和 `DONE`。
