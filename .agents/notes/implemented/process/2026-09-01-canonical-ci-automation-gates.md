...
The canonical repository must configure `DSH_REAL_API_E2E_ENABLED`, `DSH_ISSUE_AUTOMATION_ENABLED`, `DEEPSEEK_API_KEY_EXTERNAL`, and the GitHub App credentials before enabling the corresponding workflows. The App must be installed for the `jsjm1986` account with access to `CoHarness` and the required Project permissions. The configured Project owner is the `jsjm1986` user account, so the policy uses the user Project query rather than assuming an organization-owned Project. Non-canonical repositories and untrusted pull-request events see those jobs skipped rather than failed. Repository and Project identities are intentionally explicit in configuration, workflow guards, and policy runtime validation, so changing either owner requires updating the configuration and regression tests together.

The [verification throughput decision](2026-09-22-pr-verification-throughput.md) makes a selected required PR proof an explicit E2E invocation independent of the scheduled-run opt-in. Repository and trusted-PR restrictions remain; an unavailable credentialed job makes the required proof summary fail. Issue automation and ordinary scheduled runs retain their repository-variable opt-ins.
