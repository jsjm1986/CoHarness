...
每个拉取请求还会启动一个名称为 `windows node 24 / native complete` 的常规独立 `windows-native` 作业。依据[可移植 Runner 默认值](2026-09-02-portable-ci-runner-defaults.zh.md)，它默认使用 `windows-2025`；`DSH_CI_ENTERPRISE_RUNNERS_ENABLED=true` 会选择组织自有的 `dsh-windows-2025-16core` 池，`DSH_CI_FAILOVER_WINDOWS=selfhosted` 则会选择 `[self-hosted, dsh-win-ci, windows]`。该作业为工作区符号链接启用开发人员模式，通过 `pnpm/action-setup` 提供仓库固定版本的 pnpm，在不传输 store 归档的情况下执行不可变安装，并在原生 PowerShell 下运行 `pnpm run check:ci:windows-complete`。门禁卡住时，120 分钟超时会为其设定上限，同时不把实测性能目标当作正确性截止时间。
...
**把企业 Windows 池继续作为隐式默认值。** 可移植 Runner 决策不采用这种方式，因为没有该组织级标签的仓库只会得到无限排队的作业。实测的 16 核配置仍可通过显式仓库配置启用。
...
维护者必须保留两种有意设计的执行拓扑：Wine 快照使用 Linux 安装加 hoisted 布局来触达 win32 二进制文件，而原生作业在仓库变量所选容量上使用不可变 Windows 工作区。任一作业独有的失败都必须依据该边界分类，不得削弱或静默跳过。
