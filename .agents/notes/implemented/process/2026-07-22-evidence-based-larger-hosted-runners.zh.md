...
**当前 Runner 放置。** [可移植 Runner 默认值](2026-09-02-portable-ci-runner-defaults.zh.md)取代本文把企业池作为主路径的配置。部署显式启用这些外部池时，下文的测量结果、聚合拓扑以及基准测试与故障切换依据仍然适用。
...
企业保留仅限本仓库使用的 Ubuntu 和 Windows x64 大型运行器池，作为需要显式启用的容量。普通拉取请求默认让 3 个 Linux 主作业使用 `ubuntu-latest`，让独立原生 Windows 信号使用 `windows-2025`；`DSH_CI_ENTERPRISE_RUNNERS_ENABLED=true` 才会选择实测的 16 核池。必需的 Wine 信号仍位于标准托管 Linux。企业池禁用公网 IP；工作流并发仍设有边界，因为自动扩缩容上限既不会分配闲置机器，也不意味着仓库工作可以无限扩展。

除非仓库显式启用企业池，必需主路径使用可移植的 GitHub 标准托管容量。无论采用哪种模式，Node 22.19、Node 26、Python SDK 和 Wine 约定都保留在标准托管运行器上。[串行参考流程](2026-07-21-serial-cross-platform-ci-reference.zh.md)把自托管完整聚合演练放在 `DSH_CI_SELF_HOSTED_STANDBY_ENABLED=true` 之后。`suite=larger-runner-benchmark` 比较已预配规格上相互独立的关键通道，`suite=consolidated-runner-benchmark` 则比较完整聚合流程；二者都要求显式启用企业运行器。每项基准测试都会先报告实测的处理器和内存容量，再运行仓库工作。
...
Linux 主流程使用 3 个相互独立的 job。标准托管运行器使用有界的可移植 worker 配置；显式选择的企业或自托管容量使用实测的较大配置。覆盖率单独运行，并按显式进程上限在该 job 内划分插桩工作；静态调度器负责不消费生成输出的源码和文档门禁。第 3 个 job 负责唯一一次 Linux 构建，随后让 lint、Node 24 运行时兼容性、依赖构建产物的快照、文档类型检查和所有产物消费方基于该目录树启动。这种[消费方独立构建](2026-07-30-independent-ci-consumer-build.zh.md)使 3 个 job 都能立即请求 runner，而无需重复编译或传输仅供本次运行使用的产物。生成的 NodeNext 消费方目录不会纳入 Oxlint 的文件发现范围，因为这些进程重叠执行时，产物检查会删除这些目录。pnpm store 会得到恢复，但缓存上传不会进入拉取请求关键路径；Oxlint 没有由仓库管理的结果缓存。性能报告采用每个 job 从 `startedAt` 到 `completedAt` 的区间；runner 排队延迟是容量证据，而非仓库执行时间。
...
[拉取请求双 Windows 拓扑](2026-08-08-native-windows-pull-request-ci.zh.md)把必需的构建与生产网站判定保留在标准托管 Linux 上的 Wine 中。独立且不阻断的原生作业默认使用 `windows-2025`，只有显式启用企业容量时才使用实测的 16 核池；它通过一次 Windows 设置共同执行工作区构建、生产网站验证、受支持源码覆盖率与完整的可移植性清单。重复的静态检查、文档、包、构建产物、lint 与快照检查由 Linux 提供阻断性判定，原生聚合流程则保留这些观测性检查。
...
自托管的 Linux 与 Windows 串行热备参考，以及被禁用的 `serial-macos` 任务仍然存在。拉取请求默认使用可移植标准路径，只有显式启用企业容量时才选择企业池；其他大型运行器规格也只通过已启用企业容量的手动触发运行。

当 `DSH_CI_SELF_HOSTED_STANDBY_ENABLED=true` 时，自托管串行 Linux 参考会在公司自有的 `vm-backup` 池上运行（该池是一台 64 核虚拟机，运行 6 个常驻的 systemd 管理运行器实例）。它是热备演练而非必需检查：启用后的 master 运行会重新证明这台持久化虚拟机能够执行完整的未分片聚合流程。实际切换机制已预先布线：三个必需 Linux 作业通过写入权限持有者可管理的仓库变量 `DSH_CI_FAILOVER_LINUX` 解析运行器池，因此故障响应就是设置一个变量并重跑——无需合并（合并本身会被正在失败的检查阻塞，形成死锁）（[切换手册](2026-07-26-ci-failover-runbook.zh.md)）。该热备通道由 push 触发，因此执行基础分支自身的工作流定义。但需要注意：故障切换期间，`pull_request` 作业确实会带着 PR merge 引用自带的工作流定义到达这些运行器；选择器会排除 Dependabot，详见切换手册。
...
**把企业池继续作为隐式的必需默认值。** 不予采用，因为没有这些组织级标签的仓库只会得到无限排队的必需 job，而不是可操作的结果。显式启用仍保留实测加速，同时不让外部容量成为正确性前置条件。
...
主拓扑中的每个通道只承担 1 轮设置开销，且不保留工作流级分片 job 或选择器。进程本地 coverage 分区共享这 1 轮设置与同一个工作区。普通拉取请求使用降低并发的标准托管容量；显式启用企业容量的仓库则保留实测的 16 核配置，只有在重新测量有价值时，手动基准测试才会加入其他规格。
...
企业级运行器标签缺失或改名只会在管理员显式启用它们时影响必需路径。未设置外部容量变量时，必需 job 使用可移植标准标签并产生有界结果。启用企业或自托管容量后，运行器分配成为管理员拥有的运维依赖，因此激活前必须验证标签和就绪状态。
