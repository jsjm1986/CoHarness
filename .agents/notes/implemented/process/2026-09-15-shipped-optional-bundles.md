...
The Web plugin page manages only the bundles a person installed into the profile. An official experimental layer such as Agent Teams had to be found on npm and installed by name before it could be switched on, and default-product isolation kept every experimental package out of the installation's runtime dependencies, so nothing shipped with dsh could offer it.
