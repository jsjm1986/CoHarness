...
[Execution evidence](../../../../scripts/gate-evidence.ts) separates stable identities and raw results from observations. [Publication readiness](../../../../scripts/release/readiness.ts) derives minimum checks from candidate changes, resolves exactly one upgrade record, rejects pending acceptance and unclaimed paths, and verifies report bytes against authenticated GitHub artifacts. Publish consumes the tested bytes. Missing required native, API or sandbox evidence prevents publication; a local success cannot substitute for those environments.
...
Evidence artifact names belong to producer jobs and distinguish matrix legs and attempts. A failed build still records its failure when outputs are absent; a passed build must match every declared output pattern. Source compatibility jobs provision a functioning sandbox, and the pinned Ubuntu payload comes from a dated snapshot so archive rotation cannot remove its download input.
...
Android is an optional feature: native validation runs only through the explicit android-audit suite, is absent from the required aggregate and default release requirements, and remains marked manual-only in selection evidence. Compilation, native bridge instrumentation and real push delivery are separate proofs. Strict Python typing remains deferred with reassessment on public SDK type changes or the next upstream SDK update. No global script coverage threshold or permanent generated bilingual gate catalog is introduced.
