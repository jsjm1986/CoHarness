...
[CI](../../../../.github/workflows/ci.yml) gives pull-request and master-push events complementary responsibilities. Pull requests run consolidated Linux and Wine-hosted Windows jobs plus the Node compatibility and Python contracts on standard GitHub-hosted capacity; an independent native Windows job reports the complete Windows inventory without participating in the required aggregate. The `serial / linux (self-hosted standby)` and `serial / windows (self-hosted standby)` definitions remain available on the in-house `vm-backup` and `dsh-win-ci` pools, but master pushes run them only when `DSH_CI_SELF_HOSTED_STANDBY_ENABLED=true`. There is no standard-hosted `serial / linux` definition; the standard-hosted `serial / macos` remains disabled under `TODO(hosted-serial-ci)` until its portable capacity can be restored. The separate job definitions intentionally keep their short checkout, runtime setup, and immutable install sequences visible instead of hiding operating systems behind a matrix or reusable workflow. `workflow_dispatch` is reserved for runner benchmarks, which require `DSH_CI_ENTERPRISE_RUNNERS_ENABLED=true`.
...
The self-hosted `vm-backup` (`serial / linux`) and `dsh-win-ci` (`serial / windows`) references are explicit standby drills rather than default master work; the one remaining disabled hosted serial reference (`serial-macos`) uses `macos-latest`, and there is no standard-hosted `serial / linux` label. The required pull-request Windows job runs under Wine on `ubuntu-latest`. The independent native job defaults to `windows-2025`, uses `dsh-windows-2025-16core` only when `DSH_CI_ENTERPRISE_RUNNERS_ENABLED=true`, and uses `[self-hosted, dsh-win-ci, windows]` under failover (see the [failover runbook](2026-07-26-ci-failover-runbook.md)); it remains absent from the required aggregate under the [dual Windows decision](2026-08-08-native-windows-pull-request-ci.md). Required pull-request jobs use portable standard capacity under the [portable runner defaults](2026-09-02-portable-ci-runner-defaults.md). Higher-core hosted runners remain opt-in benchmarks because a correctness path must remain runnable without repository-external runner configuration.
