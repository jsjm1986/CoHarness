...
In MPA builds VitePress may create route-named Markdown stubs and identical image copies before `buildEnd`; the twin pass may replace only those declared route paths or byte-identical image files, while other existing build files still fail closed.
