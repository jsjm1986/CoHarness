...
**Current runner placement.** The [portable runner defaults](2026-09-02-portable-ci-runner-defaults.md) supersede this note's enterprise-primary placement. The measurements, aggregate topology, and benchmark and failover rationale below remain applicable when a deployment explicitly enables those external pools.
...
The enterprise keeps repo-restricted x64 larger-runner pools for Ubuntu and Windows as explicitly enabled capacity. Ordinary pull requests default the three primary Linux jobs to `ubuntu-latest` and the independent native Windows signal to `windows-2025`; `DSH_CI_ENTERPRISE_RUNNERS_ENABLED=true` selects the measured 16-core pools instead. The required Wine signal remains on standard hosted Linux. Public IPs are disabled on the enterprise pools, and workflow concurrency remains bounded because an autoscaling ceiling neither allocates idle machines nor makes repository work scale without limit.

The required primary path uses portable standard GitHub-hosted capacity unless the repository explicitly enables the enterprise pools. Node 22.19, Node 26, Python SDK, and Wine contracts remain on standard hosted runners in either mode. The [serial reference](2026-07-21-serial-cross-platform-ci-reference.md) keeps optional self-hosted complete-aggregate drills behind `DSH_CI_SELF_HOSTED_STANDBY_ENABLED=true`. `suite=larger-runner-benchmark` compares isolated critical lanes across provisioned sizes, and `suite=consolidated-runner-benchmark` compares whole aggregates; both require the enterprise-runner opt-in. Each benchmark reports its observed processor and memory capacity before running repository work.
...
The self-hosted serial Linux and Windows standby references and the disabled `serial-macos` job exist. Pull requests use the portable standard path unless enterprise capacity is explicitly enabled; other larger-runner sizes run only through enterprise-enabled manual dispatch.

The self-hosted serial Linux reference uses the in-house `vm-backup` pool (a 64-core VM with six always-on systemd-managed runner instances) when `DSH_CI_SELF_HOSTED_STANDBY_ENABLED=true`. It is a hot-standby drill, not a required check: enabled master runs re-prove that the persistent VM can execute the complete unsharded aggregate. The actual switch is pre-wired: the three required Linux jobs resolve their pool through the writer-manageable `DSH_CI_FAILOVER_LINUX` repository variable, so an outage response is setting one variable and re-running — no merge, which would be deadlocked behind the failing checks themselves ([runbook](2026-07-26-ci-failover-runbook.md)). The standby lane is push-triggered, so it executes the base branch's workflow definition. Under failover, however, `pull_request` jobs do reach these runners with the PR merge ref's own workflow definition — the trust boundary is repository membership and the selectors exclude Dependabot, as the runbook records.
...
**Keep the enterprise pools as the implicit required default.** Rejected because repositories without those organization-scoped labels receive indefinitely queued required jobs rather than an actionable result. Explicit opt-in preserves the measured acceleration without making external capacity a correctness prerequisite.
...
The primary topology pays one setup wave per lane and retains no workflow-level shard jobs or selectors. Process-local coverage partitions share that one setup and workspace. Ordinary pull requests use standard hosted capacity with reduced concurrency; repositories that explicitly enable enterprise capacity retain the measured 16-core profile, and manual benchmarks add other sizes only when remeasurement is useful.
...
Missing or renamed enterprise labels affect the required path only when an administrator explicitly enables them. With no external-capacity variables, required jobs use portable standard labels and produce a bounded result. Enabling enterprise or self-hosted capacity makes runner assignment an administrator-owned operational dependency, so its labels and readiness must be verified before activation.
