...
**Using the packed install as the only release probe.** The release flow still unpacks tarballs into a tree and drives them with plain Node, which is the correct payload and executable-startup check. A separate upgrade gate now uses a metadata-only local registry to test two incompatible DSH versions and their physical placement; it is scoped to the synchronization risk and does not replace this packed-install probe. See [the dependency-gates note](2026-09-01-alpha23-dependency-gates.md).
