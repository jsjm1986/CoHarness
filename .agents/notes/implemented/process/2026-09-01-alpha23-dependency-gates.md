...
`scripts/verify-package-dependencies.ts` scans workspace manifests and both source faces. Client-faced packages share the module-graph rule with `verify-client-packages`: Cordis-family and `dsh-*` relationships keep matching peer/development declarations, statically linked inputs and vendored libraries stay in development or ordinary dependency sections, and every declared workspace peer is preserved as the shared-identity declaration it records. Host runtime imports require a production dependency section unless the imported export is reviewed as peer-required. The gate also requires workspace protocol ranges, no duplicate production declaration, and no dangling peer metadata. Bundle, app, vendor, and native policies remain with their existing specialized checks.
