...
只有人类来源的 `user/message` 事件中的文本块才符合条件；文本以已知注入信封开头的——运行时上下文快照、共享项目归属通知、goal 收尾、模型变更通知——也会被排除，即使旧版本构建曾把该注入记录为 `user` 来源。空提示词、仅含控制内容的提示词和非文本提示词会等待下一条合格消息。服务从提示词路径调度首个回退标题而不等待其完成，随后规范化空白和控制序列，应用已配置的单词数和 UTF-8 字节限制且不拆分代码点，并记录第一条消息的 seq。
...
`foldSessionTitle()` 会跳过由注入上下文生成的非用户来源标题事件——文本以信封开头，或其 `messageSeqs` 引用非人类的 `user/message`——因此 last-wins 重放不会复活强制过滤之前的损坏数据；折叠会回退到上一个有效标题。`session/created` 钩子对带事件宣告的会话执行一次前置 `ensureFallback`，因此持久标题缺失或被隐藏的恢复日志会获得与新会话相同的回退标题。损坏事件从不改写（不可变世代）；修复仅以追加方式进行，用户来源的重命名无论文本如何都保持权威。

Gateway 派生面应用相同的判定，而不是信任每个 `user/message` 事件：`dsh-archive-gateway` 仅从人类撰写的消息构建搜索行与消息计数，从第一条人类行推导回退标题，并直接使用 `foldSessionTitle()` 处理显式标题；Gateway 的 `conversation_search` 写入方跳过非人类消息，已存标题的读取与首条人类行回退都经过 `harness.human_session_title()`，该函数把注入信封值置空。`conversation_sessions.title` 参与 `assertSameHeader` 身份比对，因此其存储字节保持不变、读取时消毒；迁移 `025_human_authored_search_rows` 删除注入的搜索行并清空注入的归档标题，使下次同步重新发布干净的投影。
...
- **就地改写损坏的标题与搜索行**：不予采纳，因为 `conversation_sessions.title` 参与 `assertSameHeader` 身份比对且 Session 世代不可变；读取时消毒加仅追加修复可以修正所有消费方，同时让存储字节保持权威。
- **让损坏的标题事件保持可见直到显式重命名**：不予采纳，因为冷列表投影直接折叠持久日志，未打开的会话会持续显示注入文本。
...
- 归档与会话搜索只索引人类撰写的轮次；注入信封仍在日志中持久存在，但不再进入标题、预览或消息计数，遗留损坏会在会话进入时与下次归档同步时自愈。
