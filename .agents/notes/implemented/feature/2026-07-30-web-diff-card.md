...
The component shares the TUI’s single-column framing, line-terminator rule, and distinct-path file count. The [bounded comparison decision](../bug-fix/2026-09-22-bounded-diff-and-terminal-state.md) owns neutral context and changed-row classification; this note owns the render sites, geometry, and copying.
...
- **Line colors.** Removed rows use `- ` with the error token, added rows use `+ ` with the success token, and shared context stays neutral. Lines retain `white-space: pre` inside a horizontally scrolling box so source indentation remains readable. A create (`oldText: null`) has no removed side.
...
- **Footer and copy.** A dim `└ +A -R · N file(s)` footer counts additions and removals in the comparison result and distinct paths, independently of the height cap. Copy writes the complete prefixed diff with path headers, neutral context, changed rows, and gap markers so collapsed or multi-file copies remain attributable.
...
`DiffBlock` reads only the diff view’s fields, so replay does not depend on session lookups. A UI without the diff capability gets the bridge’s generic fallback. The tool result fields stay unchanged; the `diff` runtime dependency and bounded fallback are recorded in the [comparison decision](../bug-fix/2026-09-22-bounded-diff-and-terminal-state.md).
