...
The admin UI Vitest suite covers navigation, API request contracts, user confirmation and create flows, project path failures, project usage/month selection, explicit quota-source submission, project configuration summary rendering, and shared app rendering. Gateway tests pin the stable missing-directory diagnostic and project quota semantics. TypeScript and Vite builds pass. An authenticated production browser check covers all five routes plus project detail at `1440x900` and `390x844`, verifies the sidebar/bottom-navigation mode, confirms `scrollWidth` does not exceed the viewport, and captures the rendered pages for visual review.
