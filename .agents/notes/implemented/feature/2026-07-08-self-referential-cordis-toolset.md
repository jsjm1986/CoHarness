...
The model-facing execution interface described in this decision is superseded by [read-only Cordis discovery](../simplification/2026-09-22-retire-dynamic-cordis-model-tools.md). The runner lifecycle, generated API discovery rationale, and historical records remain in scope; the retained execution mechanics do not describe an available model tool.
