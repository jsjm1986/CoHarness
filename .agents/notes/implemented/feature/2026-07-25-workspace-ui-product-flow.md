...
On initial entry, the application waits until both the Workspace and Session baselines are ready. It restores a real Session selection that remains valid; otherwise, it opens the newest visible historical Session across Workspaces exactly once. When no Workspace has eligible history, it selects the most recent Workspace and falls back to its reusable or newly created blank Session. The most recent Workspace is determined by the maximum `updatedAt` of its member Sessions, falling back to `createdAt` for an empty Workspace. These derived values choose only the default target: they do not alter the Host Workspace order or trigger another selection after later hydration.
...
Top-level New Session and the plus button on a Workspace row invoke the blank New Session action. The Hero Workspace picker opens the selected Workspace's newest visible historical Session and falls back to the blank action when no history exists. An explicit Workspace id becomes the target directly; when none is specified, the blank action uses the current Session's Workspace, then the most recent Workspace, and enters the blank New Session page when no real Workspace exists. The Workspace picker's one Add workspace action ([one-route Note](../simplification/2026-07-31-one-route-to-add-a-workspace.md); it was a pair of Use-an-existing-folder and create-by-name actions when this was decided) immediately creates a real Workspace when the user confirms a directory, then retargets the frontend Session to it; an explicitly created empty Workspace remains even if the user sends no message. A Hero draft containing unsent text or browser-owned attachments requires confirmation before the picker discards it; cancellation leaves the current Session unchanged.
