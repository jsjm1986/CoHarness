...
English | [中文](2026-08-12-product-subagent-one-shot-background-jobs.zh.md)

## Problem

The Codex and Claude Code providers run one self-contained operation and return one final answer, while `dsh-tool-subagent` can already adapt any one-shot provider to the generic background Job runtime. The shipped product-tool rows disabled that route, so an agent could not schedule independent product work while continuing its own turn.

Background execution must not add a product session, product-specific state, another cancellation owner, or another result protocol. The same provider run remains responsible for one native process or query and one final answer; the existing Job registry remains responsible for ids, collection, cancellation, owner cleanup, and completion notices.

## Decision

Production `dsh` does not install the optional product providers. A Profile that opts in installs and mounts `dsh-subagent-codex`, `dsh-subagent-claude-code`, or both once on the host plane. The `standard`, `code`, and `cordis` Agent Presets configure the corresponding dormant `subagent_codex` and `subagent_claude_code` rows with `backgroundMode: one-shot`. Removing a row's `disabled` field exposes the existing `run_in_background` argument. Omission or `false` waits in the foreground; explicit `true` returns a parent-owned Job id after synchronous Job preflight and registration, without waiting for provider startup or completion.

The generic one-shot adapter owns background registration and settlement. It starts the same `SubagentRun`, uses a Job-owned cancellation signal across provider startup and execution, waits for `run.result` and `run.dispose()`, maps the terminal result into the Job, and lets `job_output`, `job_list`, `job_kill`, and the existing completion notice expose that state. The product-provider packages continue to own native protocols, answer selection, local cancellation, and process-tree quiescence.
...
**Add continuable product sessions at the same time.** Resume, follow-up, progress, and persisted product sessions require new product contracts and lifecycle ownership. This decision exposes only the existing one-shot background route.

## Consequences

Agents can continue useful work while Codex or Claude Code handles an independent one-shot operation, then collect the final answer or cancel it through the same Job controls used by other background producers. Foreground callers retain their existing result and error behavior.
