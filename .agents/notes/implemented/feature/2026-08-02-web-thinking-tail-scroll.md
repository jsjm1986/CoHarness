...
Expanded-body presentation is superseded by [complete reasoning Markdown](2026-09-22-complete-reasoning-markdown.md); the summary follower remains active.
...
The collapsed row now communicates provider cadence through content motion as well as the existing sweep, while the durable reasoning text remains byte-for-byte stable. The scroll update runs only on React renders the streaming accumulator already causes; it adds no timer, animation loop, subscription, durable state, or transport traffic. A long current reasoning line retains its full DOM text and programmatically clips the already-overflowing prefix, so expansion still reveals the complete block and assistive technology reads the same current summary text. A body beginning with blank lines or indentation has a non-empty collapsed summary while expansion interprets that formatting as Markdown.

## Testing

`packages/client/ui-conversation/tests/reasoning-row.client.spec.tsx` pins the latest-line selection, the calculated right-edge scroll position, the settlement reset to the first visible line and `scrollLeft = 0`, and complete Markdown content in the expanded body. The keyless assembled Chromium scenario in `apps/web/tests/lifecycle-chrome.e2e.ts` replays real recorded reasoning chunks at observable pacing, narrows the viewport until the summary overflows, and asserts that the live collapsed Think row reaches its actual browser scroll extent. Its settled replay golden remains unchanged for reasoning without leading formatting whitespace.
