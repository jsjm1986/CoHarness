...
评分器对每个候选项使用动态规划，时间复杂度为 `O(query length × name length)`，空间复杂度为 `O(name length)`。候选项评分只在客户端进行，同时检查规范命令名和可选的本地化标题；命令描述不影响匹配。一方命令通过稳定 definition identity 提供本地化标题、分组和共享图标。本地化 token 只作为草稿写法：菜单与 Enter claim 最终执行规范描述符名称，规范名称的精确匹配仍优先于别名。
