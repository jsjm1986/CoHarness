...
Policy is projected atomically to each personal or shared project runtime as mode-`0600` `$DSH_HOME/model-governance.json`, including a stable subject-bound bearer credential for the loopback intake. Personal policy uses the account role plus any user exception; project policy uses the ordinary-member role and has no per-user exception because one process is shared. A policy edit rewrites the projection, and the running plugin applies a validated replacement without restart under the [live policy reload decision](2026-08-14-gateway-model-governance-live-policy-reload.md). The mandatory tree-external `dsh-model-governance` bundle is materialized independently of the optional directory guard, so `HGW_GUARD_PATCH=off` cannot disable model authorization or accounting. Each profile receives the bundle's `package.json`, `lib/`, and patch files, while source and test trees remain outside the runtime. The built governance code resolves its declared shared `dsh-llm` and `dsh-model-provider-config` peer dependencies from the profile's `node_modules`; linking the source tree would make Node resolve from the checkout and fail a compiled CLI that depends on the profile's dependency graph. The plugin publishes the typed `ctx.modelAccess` face through the host's Cordis instance instead of loading another Cordis copy.
...
The admin SPA exposes Models and Usage pages for catalog, role defaults, user exceptions, prices, user quotas, and natural-month summaries. Project detail reuses the same usage presentation for project totals; its quota dialog defaults to project-independent unlimited limits and can still inherit the ordinary-member quota, and GET project includes the stored quota source with the limits currently in force. Authenticated users read only their personal summary through `/account/api/usage`; project usage remains an admin view because one total is shared by every member. A client plugin contributes durable personal threshold alerts to `shell.overlay` without recomputing quota policy in the browser.

## Testing

Gateway tests cover policy precedence and global disable, project member policy, price calculation and personal-cost exclusion, user/project UUID deduplication, threshold idempotency, malformed intake rejection, time-zone month boundaries, user quota inherit/unlimited/custom semantics, project inherit-or-explicit semantics, bearer intake authentication and denial audit, subject-bound policy projection/token reuse, mandatory governance when directory guard is disabled, and profile-local materialization of both policy packages with peer-import resolution. Plugin tests cover fail-closed policy parsing, final-stream denial and usage capture, attribution conflict, and outbox retry/removal. Host tests cover catalog filtering plus selection/prompt refusal. Adapter tests pin the non-secret source label. Admin tests cover project month selection, usage rendering, explicit quota-source submission, and project quota configuration on GET project. Type builds cover host, client, admin UI, model-access, the alert bundle, and the tree-external plugin.
...
**Bundle a second Cordis copy with the tree-external plugin.** Rejected because service identity and isolation must use the host's Cordis instance. The plugin keeps Cordis as a host-provided service and publishes the structural service face through that instance, while ordinary shared package imports remain profile dependencies.

**Link the source checkout into each profile.** Rejected because Node resolves peer imports from a symlink target's physical path, so compiled governance code would search the checkout's plugin directory instead of the profile's `node_modules`. The Gateway copies only the package manifest, built runtime, and bundle patches into the profile and refreshes them on each start.
