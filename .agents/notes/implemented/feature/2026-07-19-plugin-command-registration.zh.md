...
`CommandDefinition` 包含可选的、带插件命名空间的品牌 `definitionId`、不带 `/` 的小写名称、非空描述、可选的非结构化输入提示，以及可取消处理器。该标识独立于展示文案和每次执行的 `commandId`；有效描述符只携带选中的作用域定义的标识。注册会校验元数据并复制一份与调用方脱离的副本、冻结最终生效的定义，并返回对应 Cordis effect 的精确 disposer（资源释放器）。同一层中的重复名称会失败。每个消费方都能看到所有有效定义；若命令插件无法在某种部署中运行，它就不在该部署中注册，而不是把消费方身份编码进共享领域。
