...
The file capability follows the three-package seam: `@deepseek-ai/dsh-file-reference` owns `ctx.fileReferences`, the shared `@path` token grammar, candidate shape, and stable model guidance; `@deepseek-ai/dsh-file-reference-local` owns bounded per-agent Host-filesystem indexes, invalidation, and scoped prompt installation; `dsh-client-ui-reference` consumes the generated Remote namespaces and shared grammar. A file pick is an atomic composer reference with a file glyph and filename; its serialized form remains path-only prompt text. A directory pick settles as an atomic folder reference, while its explicit drill action keeps editable path text, retriggers completion below the trailing slash, and publishes a reversible breadcrumb trail.
...
Autocomplete labels use live session projections or durable projection-cache checkpoints when those faces are mounted; sessions without a projection fall back to their stable id until opened. This avoids folding a complete cold log on every `@` keystroke while retaining the transport-neutral fallback in minimal compositions.

The local file index treats an unreadable workspace root as an index failure so a later query can retry; unreadable descendants remain best-effort omissions. It does not exclude `lib` by default because some ecosystems keep source files there; deployments can add it explicitly.
...
File lookup is advisory and cancellable; selection itself performs no read. Session autocomplete uses projection-backed labels when available and keeps title-log fallback only for compositions that do not mount projection faces. Session preparation is all-or-nothing for one accepted model step. A queued message captures each source when the message is claimed, so queue edits and queue-to-steer relocation use the same path without gateway coordination.
