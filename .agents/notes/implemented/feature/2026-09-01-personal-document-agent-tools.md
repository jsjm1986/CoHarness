...
The Consumer is mounted as a plugin row in the shipped `standard`, `ptc`, and `cordis` Agent presets. It injects `tools`, `systemPrompt`, and `userDocs`, adds one stable instruction that tells the model to list before asking for an upload, and never places the complete inventory in the system prompt. The `ptc` preset therefore receives the same capability through its generated SDK and does not require an agent-loop change.
