...
`@deepseek-ai/dsh-command-feedback` in `packages/feedback/command-feedback/` registers one global `feedback` command over `ctx.commands`. `/feedback <text>` acknowledges with the receiving session id and the shared harness-home anonymous user id; bare or whitespace-only input returns a direct usage error. The command handler is synchronous; the Remote uses the Session service, and neither has deployment configuration. [The shared-id decision](../architecture/2026-08-07-shared-feedback-telemetry-user-id.md) records why feedback and OpenTelemetry use the same `$DSH_HOME/.anonymous-user-id` value.

The package declares the log-only `feedback/record { text?, category? }` event and exports `recordFeedback(session, entry)` as its command-independent producer. Text is trimmed and omitted when blank; an empty entry still records the request for review. The command requires text, while the `sessionFeedback.record` Remote accepts an optional category and text. Gateway authorization requires write access to the addressed Session.
...
The Web client decorates a bare `/feedback` with the upstream category-and-description dialog. It consumes only the current trigger token; attachments and other drafts stay intact. `/feedback <text>` retains Host command execution. Each retained Session owns its dialog, so four Workbench panes do not share drafts. Scope disposal drops drafts and ignores late acknowledgements. The existing per-message controls retain their independent sidecar behavior. Replacing that sidecar with Session feedback would silently change telemetry consent and is not an equivalent UI adaptation.
...
Feedback has no model consumer, amendment, withdrawal, or explicit durability barrier; a recorded entry can be lost with any other unflushed tail. The optional telemetry consumer treats the event as an export-policy trigger. Editable per-message ratings remain in their independent sidecar and do not authorize log sharing.

Package and assembled Web tests cover command capture, category submission, failure retention and retry, and absence of new model turns. The Web dialog has an independently replayed accessibility snapshot.
