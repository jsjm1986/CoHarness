...
两个落定侧守卫把并非单进程退出的结果挡在 terminal 路径之外。`isSpilledShellCall` 识别文本以持久化 spill 通知结尾的前台 `bash`/`pwsh` 结果：`presentBashResult` 靠末尾的 `[exit code: N]` 标记渲染状态徽标，而 spill 页脚不是这种标记，缺了该守卫，卡片就会在有界预览之上报告一个伪造的干净退出。识别助手是浏览器安全入口 `@deepseek-ai/dsh-spill-policy/notice` 的 `hasSpillNotice`——即生产方自己的 `formatSpillNotice` 约定——而不是 UI 里第二份通知模式副本。`isSettledPersistentShellCall` 通过无 `description` 的 `bash`/`pwsh` schema 识别 persistent shell 家族：persistent 提供方报告的是重置与部分输出而非进程退出状态，因此其落定调用同样保持 generic。聊天行的可展开通用正文随后为两类守卫承载已记录的原始文本——spill 预览与 persistent shell 转录都仍可查看，而不必虚构一个退出状态。这两个守卫共用的解析助手（`parsedToolCall`、`validEscalationFields`、`singleResultText`）位于 card model 旁的 `raw-tool-call.ts`。
...
- **退出状态与复制。** 非零退出码、显式未知退出码或信号渲染一枚状态徽章，与 bash 工具自身渲染器所作的退出状态区分一致；干净退出不渲染徽章，落定后的空输出渲染一处变暗的占位文字——该判定读的是卡片实际渲染的解析行，而非原始文本，因为只含转义或控制字节的输出能通过 `trim()` 却解析不出任何可见内容，否则就会画出一片空行外加一个把不可见字节写进剪贴板的复制控件。复制控件复制的是原始输出文本而非渲染后的树，因此提示符行与徽章不会进入剪贴板。 运行中输出的可见性和未知退出码处理遵循[终端状态决策](../bug-fix/2026-09-22-bounded-diff-and-terminal-state.zh.md)。
