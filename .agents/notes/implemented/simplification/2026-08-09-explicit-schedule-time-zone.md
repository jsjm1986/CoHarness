...
**Let Schedule inspect the latest time-context message.** A prose snapshot is model-visible evidence, not a typed package seam. Consuming it would couple Schedule to AgentLoop history and duplicate validation against the original zone records.
...
**Require the model to ask on every unqualified time.** This is safe but unnecessarily interrupts the common browser-local case. The request-local instruction provides the intended assumption while mixed or missing zone records still ask.
...
- Travel and concurrent tabs affect only their own prompts; a turn with mixed zone records asks instead of mutating shared state.
