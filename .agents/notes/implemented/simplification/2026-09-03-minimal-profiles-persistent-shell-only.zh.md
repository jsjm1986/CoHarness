...
本决策部分取代[极简裸运行时](../feature/2026-08-11-minimal-profiles-bare-two-tool-runtime.zh.md)中的工具选择。该 Agent Note 继续负责提示词所有权与无 compaction 行为；共享 base patch 仍附带 `str_replace_editor`，因此基于 base 的 profile 保留第二个文件编辑接口。[应用架构](../../../../docs/architecture.zh.md)负责 profile 启动与 bundle 分层。
