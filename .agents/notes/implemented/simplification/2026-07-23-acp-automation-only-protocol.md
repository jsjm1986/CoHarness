...
`@deepseek-ai/dsh-acp` is an automation transport under [`packages/acp/acp`](../../../../packages/acp/acp/README.md), outside the `ui` package group. Its public protocol is intentionally limited to version negotiation, primary-session creation/listing/resume/close, model and reasoning configuration, session-scoped stdio or HTTP MCP mounts, one in-flight prompt per session, committed assistant text/image updates, per-session cancellation, concurrent sessions, and connection-owned teardown. Prompts preserve text and supported raster images in wire order, while resource links flatten to bracketed textual references; the bridge rejects additional directories, unsupported MCP transports, audio, embedded resources, malformed or empty prompts, unknown sessions, and overlapping prompts.
...
The app composition contains the agent spine, JSONL persistence, the SQLite session-query index used by the host, checkpoint policy, and ACP transport. It does not mount command, session-reference, plan-mode, permission-picker, or user-questions services for ACP.
...
Automation clients receive complete committed text/images rather than token deltas or structured tool UI. They inspect durable logs or another API when they need reasoning, tool traces, titles, or richer state. ACP can browse and resume persisted primary sessions through its standard list/resume controls, while callers that need host checkpoint navigation, deletion, or fork use a host API.
