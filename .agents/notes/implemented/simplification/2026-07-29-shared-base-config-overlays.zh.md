...
`examples/tui-agent`、`examples/cordis-agent`、`examples/code-mode` 与 `packages/examples/tui-demo` 均被删除。TUI 测试迁往 `apps/cli/tests/`，cordis 工具集的 e2e 迁入 `packages/extensions/tool-cordis/tests/`，受支持的 PTC mode demo 则保留为 `examples/acp-agent/ptc.cordis.yml` 中的 ACP（Agent Client Protocol）overlay。
