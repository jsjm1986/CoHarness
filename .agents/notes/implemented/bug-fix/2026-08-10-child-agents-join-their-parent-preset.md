...
Recomposition emits the scoped `agent-preset/recomposed` event after moving the parent link. Agent-owned consumers that reconcile registrations on scope changes use that event to remove rows left by the previous preset before a child joins the new standing composition. This keeps the live parent view and the child's inherited view identical when a blank parent switches presets.
