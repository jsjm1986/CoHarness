...
`LlmRuntime` wraps the complete `llm/stream` waterfall with a provider-neutral fail-closed guard. It publishes ordinary text unchanged, but withholds and rejects a tagged or incomplete known prefix with `UNSAFE_MODEL_OUTPUT` when an adapter or middleware leaves it unnormalized. The guard runs before session consumers receive chunks, so the same protection covers stale adapter registrations, managed-provider routes, and middleware-owned streams.
...
Tagged thinking from private OpenAI-compatible gateways renders as a separate reasoning block when the adapter normalizes it. A prefix that reaches the provider-neutral guard is rejected before any buffered text is published; the failure is outside the default retryable set. An intentionally tag-prefixed answer can therefore be classified as reasoning by the adapter, while a bypassed or incomplete known prefix fails closed and an unknown delimiter remains ordinary text. Converted responses lose provider-native replay signatures and response ids for that turn, but their durable Harness blocks remain complete and safe to replay as provider-neutral history. Empty native text blocks remain emitted when no conversion occurs so replay entries stay aligned. Client projections strip a tagged prefix from historical and live text, and the Gateway persistence repository rejects an assistant surface event that still carries one. Reasoning hidden by the provider, or available only as textual `reasoning_details` that pi-ai discards, remains unavailable to this adapter.
...
`packages/llm/llm/tests/text-thinking-guard.spec.ts` and the `LlmRuntime` service tests cover the provider-neutral fail-closed boundary, including split prefixes, middleware output, terminal-only blocks, case variants, deferred tool chunks, missing block starts, and cleanup. The managed-provider integration test exercises an `org-glm` route through pi-ai and the shared guard. `gateway/tests/conversation-safety.spec.ts` and the PostgreSQL repository suite reject unsafe assistant surface events. Client runtime, chat-node, and trajectory tests cover historical and live projection suppression.
