...
Reclaiming idle records, a resource-owned failure type decoupled from the Remote face, and the `chat` and `terminal` protocols are open; each waits for a consumer. The developer-facing reference is [packages/client/runtime](../../../../packages/client/runtime/README.md); the Sidebar that consumes the model is described in [docs/subsystems/web-client.md](../../../../docs/subsystems/web-client.md).
