...
SRC 运行时由 decorator 在 Service prototype 上记录带版本的 descriptor。descriptor 保存方法名、调用模式、可选导出名和可选 stream 模式；`remoteMethods()` 返回脱离存储的 marker。它不向 Service 实例、constructor 或方法函数写入状态，多个插件模块可以读取同一版本 descriptor。
...
Client 仅在生成描述符声明 `acceptsUndefined` 时接受省略尾部业务参数。必需位置和作用域身份注入保持不变，取消信号不会跨过省略的业务参数位置。Host 编解码器仍验证缺失的具名值；调用不合成业务默认值。
...
SRC 面向本地源码启动。`@Remote` 和 `@RemoteScope()` 的 prototype descriptor 给出方法名和调用模式，运行时从 JavaScript 函数签名读取顺序参数名，并结合已注册 lookup/Context provider 生成弱 descriptor。
