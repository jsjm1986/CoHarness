...
`EpochHeader` records the request's non-history state: call config and tool schemas, with empty values canonicalized to absence. The rendered system prompt is derived history instead — a `system/message` surface event the loop writes before user messages enter the surface, so the prompt is priced, compacted, and replayed like any other surface node. `request/header` always writes a full snapshot: the first loop instance uses reason `initial`, later instances use `resume`, and an in-instance change uses `change`. `foldRequestHeader` selects the latest snapshot. Legacy `request/header-delta` events and the removed `fallback` reason are rejected when appended or loaded.
