...
面板通过匿名容器查询在共享的 480/560/720 档位上自适应：CSS Modules 会按模块对 `container-name` 做哈希，跨包的命名容器永远无法匹配；外壳三列刻意不声明 `container-type`，因为 layout containment 会改变树内 `position: fixed` 后代的包含块。设置浮层、图片 lightbox 与拖放遮罩 portal 到 `document.body`（[设置浮层 portal](2026-08-19-settings-overlay-portal.zh.md)）；三列仍省略 containment，使任何剩余的树内 fixed 层继续以视口为包含块。`ui-theme/styles/metrics.css` 新增与主题无关的度量 token——`--dsw-space-1..8`（4px 步进）、`--dsw-radius-*`、`viewport-fit=cover` 下基于 `env(safe-area-inset-*)` 的 `--dsw-safe-*`，以及 `--dsw-touch-target`（44px）——并把 90 个客户端样式表中的 619 处间距/圆角字面量做了取值不变的改写。触控基线落在 ui-primitives（底部面板化的 Modal、compact 内联子菜单、粗指针行高、防 iOS 聚焦缩放的 16px Input 文字、隐藏的 Tooltip 气泡），另加详情拖拽胶囊与工具 inspect 胶囊的粗指针常显；portal 表面用 `useMediaQuery` 按同一阈值分支，或在已 portal 的样式表里用 `@media (max-width: 767px)`。规范以 docs/web-styling.md 为准。
...
- **在外壳三列上声明 `container-type`**——被拒：layout containment 会改变每个树内 `position: fixed` 后代的包含块；三列不加 containment，使任何剩余的树内 fixed 层继续以视口为包含块。容器保持面板自有且匿名。
