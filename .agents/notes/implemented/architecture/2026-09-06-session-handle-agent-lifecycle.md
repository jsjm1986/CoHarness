...
**Keep `agentLoop.create` synchronous and in-memory only.** Initially retained because hundreds of assembled tests and in-memory examples used its synchronous contract; later made async on the same handle seam once upstream adopted the async signature.
