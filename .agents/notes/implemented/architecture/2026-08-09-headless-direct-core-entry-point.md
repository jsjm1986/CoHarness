...
`headless-runner` is a direct core entry point. After Loader settlement, it reads `ctx.agentDefaultModel.currentSelection()`, resolves the Agent identity — a fresh `session-<uuid>` by default, or the exact persisted Session that `--session-id` names, observed through `ctx.sessionQuery.observeSession` and refused when no log exists, when that Session is already live in the process, or when its recorded cwd or agent preset does not match this run — installs the `ModelSelection` in the Agent scope, waits for startup quiescence, anchors the Session sequence, submits one ordinary user message, and waits for quiescence again. It awaits `ctx.sessions.flush`, folds its durable event interval for the last non-empty assistant text and final `turn/end` reason, writes the text plus one newline to stdout, and requests bounded launcher shutdown with exit 0 exactly when the reason is `completed`. A terminal `error` reason writes its durable code and message to stderr; unexpected driver failures also use stderr and exit 1. The runner's `progress` option defaults to `false`; when enabled, reasoning chunks from the owned Session are streamed to stderr without changing the durable result or stdout contract.

The task text is the positional argument, or stdin when the argument is omitted or a lone `-`; a blank task is rejected before the runner activates. `--json` replaces the final-text stdout line with a bounded newline-delimited event stream projected at commit points — `session`, `status`, `text`, `thinking`, `tool_call`, `tool_result`, `final` — while stderr keeps only `dsh:` diagnostics; in that mode every usage error, including the command line's own grammar rejections, also writes an `error` event to stdout.
...
`dsh --profile headless` provides a local Agent task rather than browser observation, Host APIs, or HTTP. Users who need those capabilities choose `dsh web`. Successful stderr is empty by default; an explicit headless progress setting may add reasoning output without adding model-visible data. Completion follows durable flush, and the persisted Session remains available to later tooling — including a later `--session-id` invocation, which continues the same conversation under the adoption checks. Its initial user message records `source.kind: 'user'` and therefore carries no ApiProxy `rpcId`. The `--json` stream is a projection, not the log: strings outside the terminal `final` are bounded and unmodeled events are omitted.
