...
- The retired batch API did not delete on contact: consumers still on `migrate`/`createStream` read through the `legacy-*` cluster (`legacy-types`, `legacy-json`, `legacy-chain`, `legacy-catalog`, `catalog-default`) and the public `@deepseek-ai/dsh-session-format/legacy` subpath until each consumer migrates to codec/restore reads.
