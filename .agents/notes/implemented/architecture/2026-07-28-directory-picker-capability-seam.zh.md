...
- **授权文件 browse 栅栏，不做 roots Config。** `workspace.create` 接受任意路径且 API 本就提供驱动 bash 的方法，浏览根只会是 UX 范围而非边界。当 `$DSH_DIRECTORY_GRANTS` 或 `$DSH_HOME/directory-grants.json` 含至少一条有效 path 时，browse 后端列出这些根并拒绝根外的 `list`／`createDirectory`；没有该文件时（独立 `dsh web`）列举仍从操作系统家目录起。依据：[项目制管理端](2026-08-14-project-centric-admin.zh.md)。
