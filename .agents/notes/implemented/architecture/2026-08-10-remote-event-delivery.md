...
- The official `cordis/*` and rescoped `@deepseek-ai/cordis/*` names for the six dynamic Cordis event families are one compatibility exception. Both names are selectable and forwarded unchanged; the Client dispatcher matches each pair as one subscription family without making the Host emit twice. The [official plugin compatibility baseline](2026-08-18-official-plugin-compatibility-baseline.md) owns the limit and rationale.
...
When an `Events` entry's signature reaches a Host-only symbol (a Service, `Agent`, a Context), the answer is to **split the code until the entry lands cleanly in `./types`** — never a declaration half-left in `index.ts`, and never a structurally equivalent shadow type in `./types`. The original five owner packages reach only pure types, and `dsh-cordis-host-runner` already exposes its Remote payloads through `./types`. The agent-presets package renames its previous vocabulary module to `preset.ts`, leaving the exported `types.ts` dedicated to the client-safe event declaration.

The original five events ride this path, and their dedicated `HostFrame` variants or Client aliases are gone. Model consumers subscribe directly to both owner inputs, `llm/adapters-updated` and `settings/document-updated`; preset-derived consumers subscribe to `agent-preset/selected`. The six dynamic Cordis event families use the same wrapper with their two accepted names. Frames that actually project or deduplicate data stay dedicated: `host/workspace-changed`/`-removed`/`host/archived-sessions-changed` (view derivation plus per-connection dedup state), and `host/session-added`/`-removed`/`host/session-status`/`host/agent-error` (live-object projection or frame-time derived fields).
...
client/runtime — the owner of the host frame sink — calls it directly, so the frame reaches the subscription table without an intermediate event to relay it. The `event` parameter is `string`, not `TypertRemoteEvent`: this is a wire boundary. Dynamic Cordis aliases are matched as one family, and any other name nobody subscribed to is dropped silently.

Delivery shares no implementation with the cordis event system: one-way only, no waterfall/bail/parallel/serial modes and no `@mode` concept (`ReturnType extends void` is the static expression of that rule), no `this` binding, no `EventOptions`, `prepend`, or priority. Listeners run in global registration order. One listener registered under both Cordis aliases is invoked once per frame, while repeated registrations under one exact name remain independent. A listener that throws is contained and logged — it must never take down the frame pump (the same posture `ConnectionController` already applies to its sinks).
...
| `api/gateway` Client half | `ClientRemoteService` implements `$on` (subscriptions addressed by registration, `ctx.effect` ownership for the calling fiber) and `$dispatch` (snapshot delivery in global registration order, Cordis alias matching, and containment for listeners that throw or reject) |
| `api/remotes` | `src/remote-events.ts` owns the allowlist value and `src/types.ts` its type projection and selection seat; both files belong to both compiler faces. The Host face asserts every entry and imports each owner `./types`; the Client half re-exports those owners plus `@deepseek-ai/dsh-api-gateway/client`. The dynamic Cordis families list both supported prefixes |
...
- `$on`'s disposer belongs to the calling fiber, and two registrations of one function object under one exact event name retire independently. Cordis alias tests additionally pin two-way matching, global ordering, and one invocation when the same listener is registered under both aliases.
