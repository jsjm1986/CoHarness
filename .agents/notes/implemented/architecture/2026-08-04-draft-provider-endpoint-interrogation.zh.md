...
`packages/llm/llm/tests/topology.spec.ts` 覆盖注册表：每个 namespace 一份、随 fiber dispose（资源释放）、丢弃重复与不可用 id 且不凭空补容量的归一化，以及 `NO_DISCOVERY`/`INVALID_DISCOVERY` 两种拒绝。`packages/llm/llm/tests/discovery.spec.ts` 驱动共享 HTTP helper，覆盖 OpenAI 与 Anthropic 路径、协议专用标头、部署路径处理、候选回退、缓存失效、密钥校验、状态与 JSON 失败、取消、响应体失败，以及「声明长度」与「流式」两种尺寸上限。`packages/llm/llm-pi-ai/tests/discovery.spec.ts` 覆盖 pi-ai 的 catalog 快捷路径、已存密钥解析、委托共享 helper，以及首次模型请求复用已解析端点。`packages/llm/llm-pi-ai/tests/adapter.spec.ts` 覆盖两个 OpenAI URL 方向、405 与非 SSE 回退、缓存复用与 profile 失效、Anthropic 规范化，以及必须保持单次尝试的负面情形。`gateway/tests/organization-model-settings.spec.ts` 覆盖 Gateway 委托和同样的 OpenAI/Anthropic 请求。`packages/host/apiproxy/tests/api-proxy-config.spec.ts` 在真实 proxy 上覆盖该 RPC：草稿完整抵达其 namespace、缺席字段保持缺席、没有 namespace 或凭据被写入，以及失败以 `model-discovery-failed` 呈现且序列化后的错误里不含凭据。
