...
The streaming face is available without forcing every existing migration or provider to change at once. The v0→v1 and v1→v2 stages normalize the legacy payloads CoHarness stamped under those versions, and v2→v3 performs the system-history conversion, while JSONL and Gateway provider integration remains a separate implementation step because their physical readers own decoding and publication.
