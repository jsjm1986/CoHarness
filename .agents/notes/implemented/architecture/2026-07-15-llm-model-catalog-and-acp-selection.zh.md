...
请求头与 `model/selection` 事件共同承担持久化：为下一次请求所接受的选择会作为持久化意图追加，消费它的 `request/header` 快照随之退役 pending 值。前端的内存选择依次由投影的 pending 值、折叠后的最后一个请求头、创建选项播种——见[持久化模型选择](2026-09-13-durable-model-selection.zh.md)。
...
**立即持久化一个新的模型选择会话事件。** 当初被否，理由是未被使用的 UI 选择尚未影响任何模型请求；当冷恢复使该待生效意图变成重建相关状态后立场改变——[持久化模型选择](2026-09-13-durable-model-selection.zh.md)采用的正是这个事件。
...
- 请求头与基于提供方路由的会话形态保持兼容；后来新增的选择意图事件由[持久化模型选择](2026-09-13-durable-model-selection.zh.md)覆盖。
