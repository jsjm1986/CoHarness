...
The generic Gateway client returns raw outputs without validating them. The existing [SessionEventStream](../../../../packages/client/runtime/src/client/sessions/session.ts) therefore checks follow snapshots, live durable entries, and history pages before publishing them. Its private [wire-event checker](../../../../packages/client/runtime/src/client/sessions/session.ts) validates the exact envelope and delegates event-local rules to the browser-safe core validators. It does not add a generic Gateway schema or validate unrelated plugin payloads. Surface membership and source existence remain Host-owned because a browser window may omit earlier events.
