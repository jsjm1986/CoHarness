...
**为仓库自有的旧名称保留别名。**不予采纳。没有已发布的消费方需要这些别名。别名会保留两套内部词汇，使首次发布携带一项从未有用户需要的迁移。外部上游标识符由兼容基线单独评估。

## 验证

- 清单中的每项映射都出现在仓库中。每个系列只有一套公开词汇；同一个 Cordis 上下文中没有兼容包、重新导出别名、重复的 `ctx` 键、双重插件 id、清单自有的双重事件 id、旧工具别名或回退解析器。
