...
- 旧批量 API 现已随替换删除：`legacy-*` 簇（`legacy-types`、`legacy-json`、`legacy-chain`、`legacy-catalog`、`catalog-default`）与公开子路径 `@deepseek-ai/dsh-session-format/legacy` 均已移除。协调器、Gateway 与 SQLite 经由 `session-format-catalog` 的 `sessionLogicalFormatCatalog` 读取已解码存储行——它把逻辑 header 投影到发布版要求上，并把 v0、v1、v2 三条边经声明的 CoHarness 方言 stage 路由到同一套发布版原语。v0/v1 准入词汇表见 proposed 笔记 `2026-09-25-coharness-v0-v1-session-dialect`。
