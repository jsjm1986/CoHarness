...
- At startup the runtime subscribes to the first complete baseline: a successfully restored current session is kept in place; otherwise it opens the latest visible historical Session across Workspaces, falling back to `recentWorkspaceId` and its `connectWorkspace` blank path when no history exists anywhere. The policy settles only once; a later user-initiated clear is never overridden by auto-selection again, and an open failure waits for the next baseline projection to retry.
- Re-picking the Workspace in the blank Hero goes through `openWorkspace`, which selects the target's latest visible history or delegates to `connectWorkspace` for a blank fallback. An unsent draft requires explicit confirmation; after the target resolves, the browser-owned draft is discarded before `sessions.open(nextId)`, while cancellation or target failure keeps the current session and draft.
