...
`measure(session, requestHeader?)` synchronizes the fold once and returns scalar pressure together with positional per-node prices. `totalTokens` remains request-and-response pressure; `surfaceTokens` is the surface total under the measured route's request pricing and equals the sum of `nodes[].tokens`. A `requestHeader` override changes pressure and node pricing only, while the node set always describes the current session. `estimateMessage(message)` applies the fixed heuristic without session state. Each result is one detached, deeply immutable snapshot carrying one `logRevision`. Every measurement clones the current nodes and is therefore O(surface).

The routed adapter's optional synchronous `imageRequestPricing(provider, model)` hook — resolved through `ctx.llm.imageRequestPricing()` — prices each retained image occurrence at its route's declared visual price plus model-visible text; routes without the hook keep the fixed heuristic. Every `TokenSurfaceNode` therefore carries two prices: `tokens`, the route-priced request pressure read by trigger, retention, range selection, and the summary-shrink comparison; and `heuristicTokens`, the fixed-heuristic price that the shadow-price protocol uses so the O(1) projection fold stays in agreement with its own appends. Logged `shadowedTokenCount` on `compaction/summary` and `compaction/prune` carries the heuristic total, while the compactor's shrink comparison reads the route-priced `shadowedRouteTokenCount`.

Provider usage is reused only when the measured canonical request envelope equals the latest successful-call anchor. Any provider, model, system, prefix, tool, or call-config change causes complete repricing. The anchor holds raw materials — the pre-assistant surface nodes, the provider output's heuristic price, and the reported usage — so a matching header derives its baseline per measurement and reprices the anchored surface under the same route pricing as the current surface; the usage-versus-estimated choice is made per measurement against that route-priced anchor. Surface changes remain a signed delta from the anchor, including negative values after a shrinking replacement. A later successful request replaces the earlier anchor, including across provider or model switches.
...
Unit tests cover fixed estimation, envelope invalidation and anchor replacement, replay boundaries, immutable snapshots, routed pressure, route-priced measurement (pricer-driven trigger, pricer-less fallback, and provider-usage increments), heuristic shadow claims against route-priced shrink comparisons, convergence, overflow generation proof, and rollback. A real Loader/Include fixture verifies the zero-config token-meter and compaction-basic load path in dependency order.
