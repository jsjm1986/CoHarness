...
客户端标识符和附件编码拥有一个经过测试的实现，并能在支持的浏览器环境中使用。现在宿主侧强制标识符失败的测试直接拦截该所有者，因此生命周期失败覆盖不会悄悄退回全局 Web API。dsh release 的 packed-install 演练包含该工具的本地 tarball，确保整个 family 发布前不会因缺少成员而回退到 registry 查询。新增 CSS 改善混合文字正文，同时通过显式字面量选择器保持源文本的渲染不变。该工具要求环境提供可用的 Web Crypto `getRandomValues`；不具备该能力时会明确失败，而不是静默降低安全性。

## 测试

工具测试覆盖 v4 形状、唯一性、不具备安全上下文方法的情况，以及空数据、二进制和大块 base64 输入。packed-install 闭包包含 `packages/util/crypto`，因此发布路径演练会安装与 dsh release family 相同的本地依赖。Web 样式测试覆盖正文规则和字面量输出排除项。连接、运行时、会话和附件消费方测试继续通过共享工具覆盖这些调用。
