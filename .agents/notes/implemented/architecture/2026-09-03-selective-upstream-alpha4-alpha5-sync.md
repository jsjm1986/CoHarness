...
- `ptc-runtime-python` is replaced wholesale by the upstream experimental package at `packages/experimental/ptc-runtime-python`, adapted to fork imports (`snapshotJsonValue` from `dsh-session`), kept `private`, and given the fork's `./invariant` companion. It requires CPython 3.10 or newer.
