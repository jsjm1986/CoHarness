...
The committed guest module is generated from this repository’s sources, shared helpers, and pinned toolchain; its bytes may differ from the upstream generated module. The upstream sync manifest therefore records the package as adapted, with local generation as the replay step. `pnpm run verify-workflow-guest` checks freshness, while guest, schema, and real-runtime tests verify the embedded program; synchronization cannot replace locally derived output with upstream generated bytes.
