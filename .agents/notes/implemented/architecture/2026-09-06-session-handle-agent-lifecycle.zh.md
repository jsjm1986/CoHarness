...
`SessionPersistence` 提供读句柄和写句柄。每个持久化实例对同一个 Session id 只允许一个写句柄；读句柄彼此独立，并且读句柄拒绝追加。对于新的持久化 Session，`AgentFactory.createAgent` 在发布前取得写句柄；`resume` 会在读取或修复存储 Session 之前打开写句柄。句柄会在 registry 脱离之前由同一个幂等的 Agent 销毁流程关闭，该流程负责排空循环、释放持久化所有权并注销 Session。配置驱动的恢复或创建也走同一条路径；`agentLoop.create` 是异步的，并在发布前取得同一个写句柄。
...
**保持 `agentLoop.create` 为同步的纯内存入口。** 最初保留，因为大量组装测试和内存示例依赖其同步约定；上游采用异步签名后，该入口也在同一句柄接缝上改为异步。
