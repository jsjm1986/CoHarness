...
The PostgreSQL conversation repository implements `readHeader` and `readPage` with a read-only repeatable-read transaction and the `(session_id, seq)` keyset index. Queries cast bigint sequence values to text only in result columns and qualify the underlying bigint in `ORDER BY`, preserving numeric keyset order. Page groups use turn/step coordinates for assistant stream chunks and call ids for tool lifecycles; a long token stream therefore consumes one logical step group instead of one group per chunk. The Gateway runtime exposes metadata and page endpoints, authenticates before buffering internal request bodies, applies response budgets, and preserves typed page failures. A compatibility repository without an indexed page method keeps the existing complete-read path.
...
`SessionPersistence.readHistoryIndex` is an optional acceleration for navigation metadata. The PostgreSQL provider reads only `turn/start` boundaries and bounded `conversation_search` previews inside a repeatable-read transaction, returning at most 2,000 revision-associated markers without event payloads. The Host exposes this as `session.historyIndex`; the browser requests it after the first tail page, retains the last successful result during refresh, and fetches only the older pages needed for an unloaded marker. Live turns remain visible through the resident Conversation index, so metadata refresh never blocks model admission or streaming.
...
Cold Gateway history now reads a bounded indexed range instead of crossing the 64 MiB runtime response ceiling for ordinary large sessions. Navigation metadata is a separate small read, so showing a complete turn rail does not expand the event response or duplicate stream chunks in browser memory. A single event larger than the page budget fails closed with a stable safe message, and a changed source invalidates a continuation rather than mixing revisions. The Gateway-backed write-behind default is 48 MiB, reserving envelope headroom below the 64 MiB request limit. Providers that cannot seek still have a compatibility fallback, so deployments using sequential storage retain their previous read cost until they add an indexed implementation.
...
Focused Host tests prove a large detached history uses `readPage` without calling `inspect`; persistence tests cover cursor direction/session/revision binding, byte/event/group limits, stream-chunk grouping, oversized indivisible events, navigation-index parsing, and cancellation; Gateway tests cover indexed keyset pages, turn-index boundaries and previews, numeric ordering, stream-chunk grouping, cursor invalidation, runtime page/index responses, early body authentication, and supervisor survivor re-attachment. A production-sized read-only probe over the 137,382-event session reduced the complete bounded walk from 2,771 pages to 165 and returned 30 turn markers without selecting event payloads. Client tests cover incremental assistant accumulation, reconnect resync admission, index loading/cancellation, unloaded-marker paging, scope disposal, response-body cancellation, and listener cleanup. `pnpm run typecheck`, Gateway artifact/typecheck, export-JSDoc, contract lint, and the affected Vitest suites pass on the source tree.
