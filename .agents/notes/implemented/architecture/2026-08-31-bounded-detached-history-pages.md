...
Focused Host tests prove a large detached history uses `readPage` without calling `inspect`; persistence tests cover cursor direction/session/revision binding, byte/event/group limits, stream-chunk grouping, oversized indivisible events, and cancellation; Gateway tests cover indexed keyset pages, numeric ordering, stream-chunk grouping, cursor invalidation, runtime page responses, early body authentication, and supervisor survivor re-attachment. A production-sized read-only probe over the 137,382-event session reduced the complete bounded walk from 2,771 pages to 165 and kept each page within the configured byte/event limits. Client tests cover incremental assistant accumulation, reconnect resync admission, scope disposal, response-body cancellation, and listener cleanup. `pnpm run typecheck`, Gateway artifact/typecheck, export-JSDoc, contract lint, and the affected Vitest suites pass on the source tree.
