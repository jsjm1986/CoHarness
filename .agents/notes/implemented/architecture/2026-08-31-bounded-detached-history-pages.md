...
The PostgreSQL conversation repository implements `readHeader` and `readPage` with a read-only repeatable-read transaction and the `(session_id, seq)` keyset index. The Gateway runtime exposes metadata and page endpoints, authenticates before buffering internal request bodies, applies response budgets, and preserves typed page failures. A compatibility repository without an indexed page method keeps the existing complete-read path.
