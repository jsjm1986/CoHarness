...
`Agent.inbox` exposes only the structural `Inbox` interface for reading and mutating pending work; loop-only `hasPending` and claim operations are absent from that public face. dsh-agent-loop constructs one `ReactLoopInbox` and uses it for both structural commands and driver operations. The concrete constructor receives `SessionProjectionRegistry` directly instead of the wider Cordis `Context` and applies the configured `InboxLimits` admission quotas (`maxMessages`, `maxBytes`) before each durable commit. `AgentLoop` requires the registry service at activation and registers the `agentInbox` projection definition for its own service lifetime, so cold reads work before any Agent exists and after all Agents unload.

The two event surfaces have separate consumers. Observers following one message use `agent/inbox/inserted`, `claimed`, and `discarded`. Each `ReactLoopInbox` reads the host-only `agentInbox` projection folded over the durable `agent/inbox/spliced` stream; UI edits and removals route through an Inbox mutation method so the same projection records every change. When that projection reconstructs durable history, it rejects unsafe or out-of-range coordinates and duplicate `MessageId` values across both lists, and reports the offending event seq. Browser queue views read the same durable stream through ApiProxy's separate `inbox` projection and the client projection store; the subagent admission path reads the agent-scoped `agentInbox` state for `hasPending`.
...
The archived [addressable queue occurrence decision](../../archived/feature/2026-07-29-addressable-queue-operations.md) describes the superseded occurrence-wrapper design. `MessageId` owns addressability, while `ReactLoopInbox` contributes `agentInbox` as the host-side session projection over durable splices. ApiProxy's `inbox` projection serves live updates, history-tail reconnect baselines, and cold process-restart recovery without a live Agent mirror.
